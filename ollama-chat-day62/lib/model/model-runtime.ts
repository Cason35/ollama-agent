...
export function buildModelRuntime(
  ...
  if (provider === "mimo") {
    ...
    const key = configManager.getString("model.mimoApiKey", "").trim(); // 第62天：从配置中心读取 MiMo API Key，后续 Day63 可迁移到 Secrets。
    if (!key) {
      return {
        rt: null,
        errorResponse: apiJsonReasonError(
          API_REASON.MIMO_KEY_MISSING,
          "未配置 model.mimoApiKey：请在配置中心或环境变量 XIAOMI_MIMO_API_KEY 中填入密钥"
        ),
      };
    }
  }
  ...
  const rt: ModelRuntime = {
    ...
    mimoApiKey: configManager.getString("model.mimoApiKey", "").trim(), // 第62天：从配置中心读取 MiMo API Key。
    mimoModel,
  };
