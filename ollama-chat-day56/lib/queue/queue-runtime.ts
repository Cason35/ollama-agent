...
export class QueueRuntime { // 定义第35天队列运行时。
  ...
  async restartJob(jobId: string): Promise<JobStoreSnapshot & { restarted: Job }> { // 第37天：定义克隆旧任务并作为新 Job 重启的方法。
    ...
    this.queue.enqueue(restarted); // 确保追加时间线后的任务在内存队列中可被认领。
    const snapshot = await this.snapshot(); // 读取最新快照。
...
const globalForQueue = globalThis as typeof globalThis & { __day37QueueRuntime?: QueueRuntime }; // 扩展 globalThis 保存第37天单例。

export function getQueueRuntime(): QueueRuntime { // 定义获取运行时单例的方法。
  if (!globalForQueue.__day37QueueRuntime) { // 判断是否已有第37天单例。
    globalForQueue.__day37QueueRuntime = new QueueRuntime(); // 没有则创建新的运行时。
  } // 结束单例判断。
  return globalForQueue.__day37QueueRuntime; // 返回运行时单例。
} // 结束 getQueueRuntime。
