...
export type PromptDashboardSnapshot = { /* 绗?2澶╋細瀹氫箟 Prompt Explorer锛堟彁绀鸿瘝娴忚鍣級鎺ュ彛蹇収銆?*/
  ...
  optimizationPreview: PromptOptimizationPreview; /* 第55天：保存 PromptOptimizer 生成的策略对比、推荐和优化指标。 */
  generatedAt: number; /* 绗?2澶╋細淇濆瓨蹇収鐢熸垚鏃堕棿銆?*/
}; /* 绗?2澶╋細缁撴潫 Prompt Explorer 蹇収绫诲瀷銆?*/
import type { PromptOptimizationPreview } from "@/lib/prompts/prompt-optimization-types"; /* 第55天：引入动态提示词优化预览类型，用于 Prompt Strategy Explorer。 */
