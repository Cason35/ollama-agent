...
import type { KnowledgeMetricsSnapshot, RetrievalMode, RetrievedChunkHit } from "@/lib/knowledge-types"; // 第26天：RAG V3 类型
import { ChatComposer } from "@/app/components/ChatComposer"; // 第26天：聊天输入区
import { ChatToolbar } from "@/app/components/ChatToolbar"; // 第26天：聊天顶部控制栏
import { Header } from "@/app/components/Header"; // 顶部标题与状态徽标
import { KnowledgeSidebar } from "@/app/components/KnowledgeSidebar"; // 第26天：右侧知识库与调试侧栏
...
export default function HomePage() {
  ...
  const [ragDebugQuery, setRagDebugQuery] = useState("Workflow Runtime"); // 第26天：RAG 调试查询
  const [ragDebugHits, setRagDebugHits] = useState<RetrievedChunkHit[]>([]); // 第26天：最近检索命中
  const [ragDebugRecallK, setRagDebugRecallK] = useState(20); // 第26天：第一阶段召回数量
  const [ragDebugTopK, setRagDebugTopK] = useState(5); // 第26天：TopK（默认 5）
  const [ragDebugMinScore, setRagDebugMinScore] = useState(0.3); // 第26天：最终分阈值
  const [ragDebugMode, setRagDebugMode] = useState<RetrievalMode>("hybrid"); // 第26天：检索模式
  ...
  /** 第26天：RAG Debug — 手动触发多阶段检索并展示多种 score / rank / mode。 */
