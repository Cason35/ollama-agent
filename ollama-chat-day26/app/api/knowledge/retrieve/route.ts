/**
 * 第26天：POST /api/knowledge/retrieve — RecallK + TopK + mode + minScore 检索（RAG Debug UI）。
 */
...
export async function POST(req: Request) {
  try {
    ...
    const hits = await knowledgeStore.search(query, { recallK, topK, minScore, mode }); // 多阶段检索
    const metrics = await knowledgeStore.getMetrics(); // 刷新指标
    const lastRetrieval = knowledgeStore.getLastRetrieval(); // 最近检索快照
    return apiJsonSuccess({ query, recallK, topK, minScore, mode, hits, metrics, lastRetrieval }); // 返回可观测数据
  } catch (err) {
