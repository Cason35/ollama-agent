/**
 * 第26天：RAG Runtime V3 检索层 —— Recall + Keyword + Hybrid + Rerank。
 */
...
export async function retrieveTopChunks(
  ...
  const queryEmbedding = await embedText(q); // 生成查询向量
  ...
  for (const doc of documents) {
    for (const chunk of doc.chunks) {
      ...
      const literalScore = keywordScore(q, chunk.text); // 计算关键词分
      const hybridScore = vectorScore * 0.7 + literalScore * 0.3; // 计算混合分
      rows.push({ chunk, document: doc, vectorScore, keywordScore: literalScore, hybridScore, rerankScore: 0 }); // 收集候选
    }
  }
