...
import { executeRagAnswer } from "@/lib/knowledge-rag"; // 第26天：RAG V3 + fallback
...
} from "@/lib/knowledge-retrieval"; // 第26天：默认 recallK / topK / minScore / mode
...
/** 第26天：retrieval — RecallK + TopK + minScore + Hybrid + Rerank 多阶段检索。 */
const retrievalTool: Tool = {
  name: "retrieval", // 工具名
  description: "从知识库检索与问题最相关的文本块（RecallK + Hybrid Search + Rerank + minScore）", // 描述
  capabilities: ["knowledge-retrieval"], // 能力：知识检索
  inputSchema: { input: { type: "string", required: true, description: "检索查询" } }, // 查询必填
  outputSchema: { query: "string", hits: "array", recallK: "number", topK: "number", minScore: "number" }, // 命中列表
  async execute(input) {
    const query =
    ...
    const hits = await knowledgeStore.search(query, { recallK, topK, minScore, mode }); // 多阶段检索 + 过滤
    return { query, hits, recallK, topK, minScore, mode }; // 返回可观测结构
  },
};

/** 第26天：ragAnswer — 多阶段检索 + 低幻觉 Prompt + 无结果 fallback。 */
const ragAnswerTool: Tool = {
  name: "ragAnswer", // 工具名
  description: "基于知识库 RAG V3：Hybrid Search、Rerank、minScore 过滤、无合格片段时不硬编", // 描述
  ...
  async execute(input) {
    ...
    const result = await executeRagAnswer(question, hits, input.rt); // RAG V3 执行
    return {
