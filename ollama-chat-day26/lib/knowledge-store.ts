...
let lastRetrieval: {
  query: string; // 查询词
  recallK: number; // 第26天：第一阶段召回数量
  topK: number; // TopK 参数
  minScore: number; // 相似度阈值
  mode: RetrievalMode; // 第26天：检索模式
...
/**
 * 第26天：多阶段检索并更新 RAG metrics / lastRetrieval。
 * @param query 查询文本
...
export async function searchKnowledge(
  ...
  const hits = await retrieveTopChunks(query, docs, { recallK, topK, minScore, mode }); // 召回 + 重排 + 过滤
  ...
  lastRetrieval = { query, recallK, topK, minScore, mode, hits, at: Date.now() }; // 记录最近检索供 UI
