...
type WorkflowStep = {
  ...
  dependsOn?: string[];
  status: WorkflowStepStatus; // 第16天：扩展为含 queued / blocked 的状态机
...
/**
 * 第16天：并行 DAG 调度执行 workflow：在 validate 通过后按「可运行层」分批 `Promise.all`；
 * 保留第15天的依赖注入、线性链容错、步骤级 retry 与 Timeline；并补充 queued/blocked 状态、失败传播与 executionBatches。
 */
async function executeWorkflow(
  ...
  /** 第16天：单步执行（含重试），读取批次开始时的链快照，避免同批并行互相污染线性前缀。 */
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (useWorkflow) {
      ...
      pushTimeline("执行器 executeWorkflow 启动（第16天：并行 DAG 调度 + Promise.all 分批 + blocked 传播 + executionBatches）", undefined); // Timeline：executor 边界（并行版）
