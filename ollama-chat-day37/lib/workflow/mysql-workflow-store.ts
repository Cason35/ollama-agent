...
/** 第22天：save 返回值（是否新插入 + 库内 createdAt）。 */
export type MySQLWorkflowSaveResult = {
  created: boolean; // 新行 true；更新已有行 false
  createdAt: number; // 与 workflows.created_at 一致的毫秒时间戳
}; // MySQLWorkflowSaveResult 结束

/** 第21–22天：MySQL 版 Workflow 存储实现。 */
export class MySQLWorkflowStore {
  /**
   * 写入或更新一条 WorkflowState（INSERT ... ON DUPLICATE KEY UPDATE）。
   * 第22天：UPDATE 分支不写 created_at，仅新插入行由表 DEFAULT 写入 created_at。
   */
  async save(workflow: WorkflowState): Promise<MySQLWorkflowSaveResult> {
    if (workflow.version !== WORKFLOW_STATE_VERSION) {
      const now = Date.now(); // 拒绝写入时的兜底时间
      return { created: false, createdAt: workflow.createdAt ?? now }; // 版本不对仍返回结构
    } // 版本校验
    ...
    ); // execute 结束；注意 SQL 未列出 created_at，更新时保留原值
    const header = result as { affectedRows?: number }; // mysql2 结果头
    const created = (header.affectedRows ?? 0) === 1; // MySQL：插入=1，重复键更新=2
    const row = await this.get(workflow.workflowId); // 读回权威 created_at / updated_at
    const createdAt = row?.createdAt ?? workflow.createdAt ?? Date.now(); // 优先库内时间
    return { created, createdAt }; // 供 API envelope 与前端侧栏使用
  } // save 结束
