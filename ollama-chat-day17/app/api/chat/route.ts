...
type WorkflowStepAction = "chat" | "summary" | "todo" | "weather" | "judge"; // judge：第17天结构化判断节点

/** 第17天：条件运算符（与 evaluateCondition 分支一一对应）。 */
type WorkflowConditionOperator = "equals" | "includes" | "truthy"; // equals 精确匹配 judge.result；includes 子串；truthy 真值

/** 第17天：单步可选条件（相对前驱输出的控制流）。 */
type WorkflowStepCondition = {
  fromStepId: string; // 读取哪一步的输出作为判定来源
  operator: WorkflowConditionOperator; // 判定算子
  value: string; // 期望比对值（truthy 下可为空字符串，仅 Boolean(sourceOutput)）
}; // WorkflowStepCondition 结束

/** 第16天：步骤生命周期状态（含排队与因上游失败被阻塞）；第17天增加 skipped（条件未命中，非失败）。 */
type WorkflowStepStatus =
  | "pending" // 尚未被调度执行
  | "queued" // 已入选当前并行批次，等待进入 running
  | "running" // 正在执行（含重试过程中的运行态）
  | "success" // 成功完成
  | "failed" // 本步用尽重试仍失败
  | "blocked" // 任一依赖 failed/blocked 或工作流全局中断导致本步不再执行
  | "skipped"; // 第17天：条件不满足而跳过的正常分支
...
type WorkflowStep = {
  ...
  dependsOn?: string[];
  /** 第17天：可选条件；执行前由 evaluateCondition 决定是否进入工具链。 */
  condition?: WorkflowStepCondition; // undefined 表示无条件，恒为可执行（在依赖满足前提下）
  status: WorkflowStepStatus; // 含 queued / blocked / skipped 的状态机
  ...
  /** 第17天：skipped 时可读原因（例如 condition not matched）。 */
  skipReason?: string; // 仅 skipped 时使用；success/failed 等不必填写
};
...
type Workflow = {
  ...
  /** 第16天：按调度批次记录「同批并行」的步骤 id 列表与时间戳，供前端 Batch Timeline 与 DAG 观测。 */
  executionBatches?: WorkflowExecutionBatch[]; // 可选：并行执行器写入
}; // Workflow 类型结束（第15天 executionTimeline + 第16天 executionBatches）

/** 第16天：单次调度批次（层内并行边界）的可序列化摘要。 */
type WorkflowExecutionBatch = {
  batchIndex: number; // 从 1 递增的批次序号
  stepIds: string[]; // 本批并行执行的步骤 id（顺序与拓扑稳定序一致）
  ts: number; // 本批开始调度时的 Unix 毫秒时间戳
...
async function executeWorkflow(
  ...
  if (workflow.status !== "failed") workflow.status = "success"; // 未触发全局 failed 则整体成功
  return workflow; // 就地变异后的同一引用：调用方继续把它当作 wfDone 使用
} // executeWorkflow 函数结束（第16天并行 DAG 版）
    ...
    if (useWorkflow) {
      ...
      let workflow: Workflow = {
        ...
        steps: planItems.map((p) => ({
          ...
          status: "pending" as const, // 初始未执行；executeWorkflow 会写入 running/success/failed/skipped
      ...
      pushTimeline("执行器 executeWorkflow 启动（第16天：并行 DAG + Promise.all；第17天：condition/skipped/judge）", undefined); // Timeline：executor 能力边界说明
