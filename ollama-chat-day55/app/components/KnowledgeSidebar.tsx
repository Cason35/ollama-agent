...
export function KnowledgeSidebar({
  ...
  const [activePanel, setActivePanel] = useState<"runtime" | "experiment" | "prompt" | "model" | "memory" | "cache" | "usage" | "knowledge" | "records">("experiment"); /* 第53天：默认打开实验面板，突出 Prompt Experiment Platform。 */
      ...
      <div className="z-10 shrink-0 border-b border-zinc-200/80 bg-white px-3 py-3 dark:border-zinc-800 dark:bg-zinc-900">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-0.5 text-[11px] text-zinc-500 dark:text-zinc-400">按主题查看提示词实验、版本、模型健康、运行、用量、知识与历史记录。</p> {/* 第53天：更新控制台标签页说明，突出提示词实验平台。 */}
          </div>
          <span className="rounded-full bg-emerald-500/15 px-2.5 py-1 text-[10px] font-semibold text-emerald-800 ring-1 ring-emerald-500/20 dark:text-emerald-200">
            Day 53 {/* 第53天：更新控制台日期徽标。 */}
          </span>
        </div>
        <div className="mt-3 grid grid-cols-9 gap-1" role="tablist" aria-label="Day 53 侧边栏面板"> {/* 第53天：扩展为包含提示词实验平台的九个标签页。 */}
          ...
          <button type="button" className={tabClass("model")} onClick={() => setActivePanel("model")}> {/* 第51天：模型健康标签页入口。 */}
            健康 {/* 第51天：显示紧凑的模型健康标签名称。 */}
          </button>
