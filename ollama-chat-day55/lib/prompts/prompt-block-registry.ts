...
function cloneBlock(block: PromptBlock): PromptBlock { /* 第54天：定义提示词块浅克隆函数，防止外部修改注册表内部状态。 */
  return { ...block, weight: block.weight ?? block.order, requiredVariables: [...(block.requiredVariables ?? [])] }; /* 第55天：复制数组字段并为旧块兜底补齐 weight。 */
} /* 第54天：结束提示词块浅克隆函数。 */
...
export function sortBlocks(left: PromptBlock, right: PromptBlock): number { /* 第54天：定义提示词块稳定排序函数。 */
  return right.weight - left.weight || left.order - right.order || left.id.localeCompare(right.id, "zh-CN"); /* 第55天：优先按 weight 降序排序，再按 order 和 ID 保证可复现。 */
} /* 第54天：结束提示词块稳定排序函数。 */
