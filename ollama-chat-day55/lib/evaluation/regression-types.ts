...
import type { PromptRegressionLink } from "@/lib/prompts/prompt-types"; /* 第52天：引入 Prompt Regression Link（提示词回归关联）类型。 */
...
export type RegressionDashboardSnapshot = { /* 第46天：定义回归评估看板接口返回结构。 */
  ...
  promptRegressionLink: PromptRegressionLink; /* 第52天：保存基线与候选提示词版本的回归关联。 */
