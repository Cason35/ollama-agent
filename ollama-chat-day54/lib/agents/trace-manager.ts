import type { Trace, TraceMetrics, TraceSpan, TraceSpanStatus, TraceSpanType } from "@/lib/agents/agent-types"; /* 第44天：引入 Trace（追踪记录）、TraceSpan（追踪跨度）和 TraceMetrics（追踪指标）类型。 */
...
export class TraceManager { /* 第44天：定义 TraceManager（追踪管理器），集中维护 Trace 和 Span。 */
  ...
  startSpan(traceId: string, input: StartSpanInput): string { /* 第44天：在指定 Trace（追踪记录）下开始一个 Span（跨度）。 */
    ...
    const span: TraceSpan = { spanId: this.nextId("span"), parentSpanId: input.parentSpanId, name: input.name, type: input.type, startedAt: Date.now(), status: "running", metadata: input.metadata }; /* 第44天：创建运行中的 Span（跨度）。 */
  ...
  endSpan(traceId: string, spanId: string, status: TraceSpanStatus = "success", metadata?: Record<string, unknown>): TraceSpan | undefined { /* 第44天：结束指定 Span（跨度）并更新状态。 */
    ...
    span.metadata = { ...(span.metadata ?? {}), ...(metadata ?? {}) }; /* 第44天：合并结束时追加的元数据。 */
  ...
  getMetrics(): TraceMetrics { /* 第44天：计算 Trace Metrics（追踪指标）。 */
    ...
    return { totalTraces: traces.length, avgTraceDuration: this.averageTraceDuration(traces), avgAgentDuration: this.averageSpanDuration(spans, ["agent"]), avgToolDuration: this.averageSpanDuration(spans, ["tool", "retrieval"]), avgReflectionDuration: this.averageSpanDuration(spans, ["reflection"]) }; /* 第44天：返回完整追踪指标快照。 */
