...
export class AgentRuntime { /* 定义第40天基于 Supervisor 的智能体运行时。 */
  ...
  private resolvePrompt(componentId: string, fallback: string, variables: Record<string, string | number | boolean | undefined | null>): ResolvedPrompt { /* 第54天：定义从注册表解析 active Prompt 并通过 PromptBuilder 组合最终提示词的方法。 */
    ...
    const result = promptBuilder.buildPromptWithReport(blocks, variables); /* 第54天：按 order 排序、按条件跳过并渲染全部可用块。 */
    const fallbackText = promptBuilder.buildPrompt(blocks.filter((block) => block.type === "system"), variables) || fallback; /* 第54天：当上下文块都不可用时仍保留系统块兜底。 */
