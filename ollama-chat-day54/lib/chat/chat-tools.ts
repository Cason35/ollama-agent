 ...
 * 单步工具：天气、总结、待办、闲聊兜底。
 */
import type { ModelRuntime } from "@/lib/model/model-runtime";
import { modelExecutor } from "@/lib/model/model-executor"; /* 第51天：引入模型执行器，统一处理超时、重试、备用模型和熔断。 */
...
export async function generateFallbackChat(
  ...
  const result = await modelExecutor.call({ modelId: "small-chat", messages: [
    {
      role: "system", // 第二轮聊天人设
      content: "你是一个简洁、友好的中文助手。请直接回答用户，不要输出 JSON。", // 禁止 JSON
    },
    ...messages, // 带上完整上下文（含 system 记忆块）
  ], options: { runtime: rt, degradedOutput: "抱歉，我现在暂时无法正常回答，请稍后再试。" } }); // 第51天：通过模型执行器调用普通聊天模型并启用降级响应。
  if (!result.success) return result.output || "抱歉，我现在暂时无法正常回答，请稍后再试。"; // 第51天：模型链路失败时返回降级响应。
  return result.output || "抱歉，我现在暂时无法正常回答，请稍后再试。"; // 第51天：成功但空内容时仍保留兜底文案。
}
...
${content}
`; // 总结提示词
  const result = await modelExecutor.call({ modelId: "small-chat", messages: [{ role: "user", content: prompt }], options: { runtime: rt, degradedOutput: "总结失败：模型暂时不可用，请稍后重试。" } }); // 第51天：通过模型执行器调用总结模型并启用备用链。
  if (!result.success) {
    return "总结失败：模型暂时不可用，请稍后重试。"; // 模型不可用
  }
  return result.output || "总结失败：未获取到有效结果。"; // 第51天：返回执行器输出或空正文兜底。
}
...
`; // 待办生成提示词；依赖模式用独立结构，否则沿用线性前置段
  const result = await modelExecutor.call({ modelId: "json-structured", messages: [{ role: "user", content: prompt }], options: { runtime: rt, degradedOutput: "" } }); // 第51天：通过模型执行器调用结构化模型，失败时允许后续静态待办兜底。
  if (!result.success) {
    return [
  ...
  const raw = result.output ?? ""; // 第51天：读取执行器返回的模型文本，供 JSON 解析使用。
