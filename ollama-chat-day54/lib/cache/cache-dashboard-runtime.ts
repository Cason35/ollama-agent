...
async function runCacheDemo(): Promise<void> { /* 第48天：定义运行完整缓存命中演示链路的函数。 */
  ...
  for (const query of DEMO_QUERIES) { /* 第48天：按顺序依次执行四条演示查询。 */
    ...
    events.push({ query: answer.query, status: answer.cacheStatus, similarity: answer.similarity, traceId: answer.traceId, durationMs: answer.durationMs, savedCost: answer.savedFromCache ? estimateSavedCostForHit() : 0 }); /* 第48天：记录每条查询的命中状态、相似度、耗时与节省费用。 */
  } /* 第48天：结束演示查询循环。 */
  latestEvents = events; /* 第48天：保存最近一次演示事件序列供快照复用。 */
} /* 第48天：结束缓存演示运行函数。 */

function estimateSavedCostForHit(): number { /* 第48天：定义读取最近一次命中节省费用的辅助函数。 */
  const summaries = semanticCache.summaries(); /* 第48天：读取当前缓存条目摘要。 */
...
export async function getCacheDashboardSnapshot(force = false): Promise<CacheSnapshot> { /* 第48天：定义读取或强制重跑 Cache Explorer 快照的入口。 */
  if (force) { semanticCache.clear(); latestEvents = []; } /* 第48天：强制运行时清空缓存与事件，保证新快照独立。 */
  if (semanticCache.list().length === 0 || latestEvents.length === 0) await runCacheDemo(); /* 第48天：缓存为空或缺少事件时生成一次完整演示链路。 */
  return { entries: semanticCache.summaries(), metrics: semanticCache.getMetrics(), events: latestEvents, threshold: semanticCache.getThreshold(), generatedAt: Date.now() }; /* 第48天：返回缓存条目、指标、事件与命中阈值。 */
} /* 第48天：结束 Cache Explorer 快照入口。 */

export function invalidateCacheEntry(id: string): boolean { /* 第48天：定义供接口调用的手动失效单条缓存方法。 */
  return semanticCache.invalidate(id); /* 第48天：失效指定缓存条目并返回是否成功。 */
} /* 第48天：结束手动失效方法。 */

export function clearCache(): void { /* 第48天：定义供接口调用的清空全部缓存方法。 */
  semanticCache.clear(); /* 第48天：清空全部缓存条目与指标。 */
