    ...
    <section className="shrink-0 border-b border-teal-200/70 px-4 py-3 dark:border-teal-900/40"> {/* 第52天：定义提示词浏览器外层容器。 */}
      <div className="flex items-start justify-between gap-3"> {/* 第52天：排列标题说明与刷新按钮。 */}
        <div> {/* 第52天：定义标题说明区域。 */}
          ...
          <p className="mt-1 text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">Day 54 · Prompt Registry 继续管理版本，Prompt Block Explorer 展示可组合、可排序、可条件跳过的提示词块。</p> {/* 第54天：展示提示词版本与提示词块组合的关系。 */}
        </div> {/* 第52天：结束标题说明区域。 */}
      ...
      {snapshot ? ( /* 第52天：快照存在时渲染看板主体。 */
        <div className="mt-3 space-y-3"> {/* 第52天：定义看板主体垂直间距。 */}
          ...
          <div className="flex gap-1 overflow-x-auto rounded-lg bg-zinc-100 p-1 dark:bg-zinc-900" role="tablist" aria-label="Day 54 Prompt 组件标签页"> {/* 第54天：定义组件标签页导航。 */}
          ...
          <div className="rounded-lg border border-indigo-200 bg-indigo-50/60 p-2.5 text-[10px] text-indigo-950 dark:border-indigo-900/50 dark:bg-indigo-950/20 dark:text-indigo-100"> {/* 第54天：定义 Prompt Block Explorer 卡片。 */}
            <div className="flex items-start justify-between gap-2"> {/* 第54天：排列块视图标题和启用率指标。 */}
              <div> {/* 第54天：定义块视图标题区域。 */}
                ...
                <p className="mt-1 text-[9px] leading-relaxed text-indigo-800 dark:text-indigo-200">按 order 组合 system、memory、workspace、tool、task、output，并在上下文缺失时跳过条件块。</p> {/* 第54天：说明块排序和条件跳过能力。 */}
              </div> {/* 第54天：结束块视图标题区域。 */}
            ...
            <ul className="mt-2 grid gap-1.5 md:grid-cols-2"> {/* 第54天：定义提示词块列表。 */}
              {snapshot.blockMetrics.blocks.map((block) => <li key={block.blockId} className="rounded bg-white/75 p-2 dark:bg-zinc-950/30"><div className="flex items-center justify-between gap-2"><p className="truncate font-semibold">{block.order}. {block.blockName}</p><span className="font-mono text-[8px]">{block.type}</span></div><p className="mt-1 font-mono text-[8px] text-indigo-700 dark:text-indigo-200">{block.blockId}</p><p className="mt-1 text-[8px] text-indigo-800 dark:text-indigo-200">length {block.length} · tokens {block.estimatedTokens} · hit {(block.hitRate * 100).toFixed(0)}% · {block.enabled ? "enabled" : "disabled"}</p></li>)} {/* 第54天：逐个展示块顺序、类型、长度、token、命中率和启用状态。 */}
            </ul> {/* 第54天：结束提示词块列表。 */}
          ...
          <div className="rounded-lg border border-zinc-200 bg-zinc-50 p-2.5 text-[10px] dark:border-zinc-800 dark:bg-zinc-950/25"> {/* 第54天：定义 Prompt Composition Preview 卡片。 */}
            <p className="text-[11px] font-semibold text-zinc-900 dark:text-zinc-50">Prompt Composition Preview（提示词组合预览）</p> {/* 第54天：展示组合预览标题。 */}
