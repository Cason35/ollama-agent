...
import { DEFAULT_MIN_SCORE, DEFAULT_RECALL_K, DEFAULT_RETRIEVAL_MODE, DEFAULT_RETRIEVAL_TOP_K, retrieveTopChunks } from "@/lib/knowledge/knowledge-retrieval"; // 检索默认值与单 query 对照
...
export class IncrementalIndexer {
  ...
  async indexDocument(title: string, content: string, forceReindex = false): Promise<KnowledgeDocument> {
    ...
    const doc: KnowledgeDocument = { id: docId, title: normalizedTitle, content: normalizedContent, contentHash, version: existing ? existing.version + 1 : 1, chunks, createdAt: existing?.createdAt ?? now, updatedAt: now }; // 组装文档
...
export async function searchKnowledge(query: string, options: Partial<RetrieveOptions> = {}, rt?: ModelRuntime): Promise<RetrievedChunkHit[]> {
  await ensureLoaded(); // 确保加载
  const recallK = options.recallK ?? DEFAULT_RECALL_K; // 读取召回数
  const topK = options.topK ?? DEFAULT_RETRIEVAL_TOP_K; // 读取 TopK
  const minScore = options.minScore ?? DEFAULT_MIN_SCORE; // 读取最低分
