export const REGISTRY_ITEM_TYPES = ["agent", "tool", "model", "prompt", "memory", "knowledge", "workflow", "evaluation", "observability", "governance"] as const; // 第73天：扩展统一注册中心以支持生产治理能力类型。
...
export type RegistryMetrics = { // 第66天：定义统一注册中心规模、启用状态和版本复杂度指标。
  ...
  observabilityCount: number; // 第72天：记录日志、指标、链路与告警等可观测能力数量。
  governanceCount: number; // 第73天：记录身份认证、权限、配额与审计等治理能力数量。
