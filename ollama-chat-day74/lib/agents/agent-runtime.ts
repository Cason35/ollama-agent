...
import { promptBuilder } from "@/lib/prompts/prompt-builder"; /* 第54天：引入 PromptBuilder（提示词构建器），让运行时使用组合块生成最终提示词。 */
import { buildRuntimePromptBlocks } from "@/lib/prompts/default-prompt-blocks"; /* 第54天：引入运行时提示词块组装函数，把 active Prompt 和上下文块合并。 */
import { promptOptimizer } from "@/lib/prompts/prompt-optimizer"; /* 第55天：引入 PromptOptimizer，让运行时根据上下文动态选择提示词块。 */
import type { PromptOptimizationContext, PromptTaskType } from "@/lib/prompts/prompt-optimization-types"; /* 第55天：引入提示词优化上下文类型。 */
import { inferRuntimeContextFromText, runtimeDecisionEngine, type RuntimeDecisionEngine } from "@/lib/runtime/runtime-decision-engine"; /* 第57天：引入自适应运行时决策引擎和上下文推导工具。 */
import { runtimeDecisionStore } from "@/lib/runtime/runtime-decision-store"; /* 第57天：引入运行时决策回放仓库，用于记录 Agent Runtime 的决策历史。 */
import type { RuntimeCollaborationStrategy } from "@/lib/runtime/runtime-types"; /* 第57天：引入协作策略类型，用于根据决策生成轻量计划。 */
import { exportWorkspaceToStorage, uploadTraceAttachment } from "@/lib/storage/storage-runtime"; /* 第61天：引入 Workspace Export 和 Trace Attachment 上传到 Object Storage 的能力。 */
...
export class AgentRuntime { /* 定义第40天基于 Supervisor 的智能体运行时。 */
  ...
  constructor(private readonly registry: AgentRegistry, private readonly workspaceStore: WorkspaceStore = new MemoryWorkspaceStore(), private readonly usageStore: UsageManager = usageManager, private readonly cacheStore: SemanticCache = semanticCache, private readonly memoryStore: LongTermMemoryStore = longTermMemory, private readonly modelRouterStore: ModelRouter = modelRouter, private readonly promptRegistryStore: PromptRegistry = promptRegistry, private readonly runtimeDecisionEngineStore: RuntimeDecisionEngine = runtimeDecisionEngine) {} /* 第57天：在第52天依赖注入基础上追加 RuntimeDecisionEngine，让 Agent Runtime 先做自适应决策。 */
  ...
  async answerWithCache(query: string, context?: AgentContext, rt?: ModelRuntime): Promise<CacheAwareAnswer> { /* 第48天：定义先查语义缓存、命中直接返回、未命中再执行智能体的入口。 */
    ...
    const search = await this.cacheStore.searchAsync(query); /* 第58天：在语义缓存中执行异步相似度检索，Redis 可用时先从共享缓存水合。 */
    if (search.hit && search.entry) { /* 第48天：相似度达到阈值时判定缓存命中。 */
      this.traceManager.endSpan(trace.traceId, cacheSpanId, "success", { cacheStatus: "hit", similarity: search.similarity, cacheEntryId: search.entry.id, cacheBackend: search.backend }); /* 第58天：结束 cache span 并记录命中状态、条目 ID 与 Redis/Memory 后端。 */
    ...
    this.traceManager.endSpan(trace.traceId, cacheSpanId, "success", { cacheStatus: "miss", similarity: search.similarity, cacheBackend: search.backend }); /* 第58天：未命中时结束 cache span 并记录 Redis/Memory 后端。 */
    ...
    await this.cacheStore.addAsync({ query, answer: collaboration.result.output, traceId: trace.traceId, score: collaboration.evaluationMetrics?.averageScore ?? 0, genDurationMs }); /* 第58天：把首次生成结果写入内存索引并同步写入 RedisCacheStore。 */
  ...
  async executeAgentPlan(plan: AgentPlan, context?: AgentContext, rt?: ModelRuntime): Promise<AgentCollaborationSnapshot> { /* 定义按 AgentPlan DAG 并行执行智能体的方法。 */
    ...
    const workspaceContext: AgentContext = { memory: context?.memory, workflow: context?.workflow, tools: context?.tools, workspace, trace: { traceId: trace.traceId, parentSpanId: workflowSpanId }, runtimeContext: context?.runtimeContext, runtimeDecision: context?.runtimeDecision }; /* 第57天：把共享工作空间、Trace 和运行时决策一起注入 AgentContext。 */
    ...
    await this.attachPromptSnapshot(finalTrace, promptABTest); /* 第61天：把 Prompt A/B Test 快照作为 Trace Attachment 上传到 Object Storage。 */
    const workspaceExport = await this.exportWorkspaceSnapshot(finalWorkspace); /* 第61天：把最终 Workspace（工作区）导出为 Markdown 并上传 Object Storage。 */
    return { result, callGraph: this.callGraph, timeline: this.timeline, metrics: this.getRuntimeMetrics(), dagMetrics, resultStore, workspace: finalWorkspace, workspaceMetrics, workspaceExport, reflectionAttempts: this.reflectionAttempts, reflectionMetrics: this.getReflectionMetrics(), evaluations: this.evaluationRecords, evaluationMetrics: this.getEvaluationMetrics(), promptABTest, trace: finalTrace, traces: this.traceManager.listTraces(), traceMetrics: this.traceManager.getMetrics(), plan: safePlan, validation: safeValidation, runtimeContext: context?.runtimeContext, runtimeDecision: context?.runtimeDecision }; /* 第61天：返回包含运行时上下文、对象存储附件、Workspace、Reflection、Evaluation 和 Trace 指标的 DAG 快照。 */
  ...
  async runSupervisorCollaboration(goal: string, context?: AgentContext, rt?: ModelRuntime): Promise<AgentCollaborationSnapshot> { /* 定义第40天 Supervisor 协作入口。 */
    ...
    const runtimeContext = context?.runtimeContext ?? inferRuntimeContextFromText({ text: goal, hasMemory: Boolean(context?.memory), hasWorkspace: Boolean(context?.workspace), hasKnowledge: Boolean(context?.tools), requiresJson: /json|结构化|只返回|schema/i.test(goal) }); /* 第57天：从目标和上下文推导 RuntimeContext。 */
    const decisionSpanId = this.traceManager.startSpan(trace.traceId, { name: "runtime-decision", type: "decision", metadata: { runtimeContext } }); /* 第57天：创建运行时决策 Span。 */
    const runtimeDecision = context?.runtimeDecision ?? this.runtimeDecisionEngineStore.decide(runtimeContext); /* 第57天：复用上游决策或由引擎生成新决策。 */
    const decisionRecord = runtimeDecisionStore.record({ context: runtimeContext, decision: runtimeDecision, source: "agent-runtime", traceId: trace.traceId }); /* 第57天：把决策写入 Decision Replay 仓库。 */
    this.traceManager.endSpan(trace.traceId, decisionSpanId, "success", { decisionId: decisionRecord.decisionId, promptStrategy: runtimeDecision.promptStrategy, modelStrategy: runtimeDecision.modelStrategy, collaborationStrategy: runtimeDecision.collaborationStrategy, cacheStrategy: runtimeDecision.cacheStrategy, retrievalStrategy: runtimeDecision.retrievalStrategy, memoryStrategy: runtimeDecision.memoryStrategy, estimatedCost: runtimeDecision.estimatedCost, estimatedLatencyMs: runtimeDecision.estimatedLatencyMs }); /* 第57天：结束运行时决策 Span 并写入核心策略。 */
    const decisionContext: AgentContext = { memory: context?.memory, workflow: context?.workflow, tools: context?.tools, workspace: context?.workspace, trace: context?.trace, runtimeContext, runtimeDecision }; /* 第57天：构造带运行时决策的 AgentContext。 */
    ...
    try { /* 第44天：捕获 Supervisor 规划异常并写入 Trace（追踪记录）。 */
      const plan = this.shouldUseDecisionLitePlan(runtimeDecision.collaborationStrategy) ? this.createRuntimeDecisionLitePlan(goal, runtimeDecision.collaborationStrategy) : await this.planAgents(goal, rt); /* 第57天：根据 RuntimeDecision 决定走轻量单 Agent 计划或 Supervisor 动态计划。 */
      ...
      return this.executeAgentPlan(plan, decisionContext, rt); /* 第57天：执行并返回带 RuntimeDecision 的计划协作快照。 */
  ...
  private shouldUseDecisionLitePlan(strategy: RuntimeCollaborationStrategy): boolean { /* 第57天：判断 RuntimeDecision 是否要求跳过重型 Supervisor 规划。 */
    return strategy === "direct" || strategy === "single-agent"; /* 第57天：直答或单 Agent 策略使用轻量计划。 */
  } /* 第57天：结束轻量计划判断方法。 */

  private createRuntimeDecisionLitePlan(goal: string, strategy: RuntimeCollaborationStrategy): AgentPlan { /* 第57天：根据运行时决策生成轻量 AgentPlan。 */
    const reason = `RuntimeDecision 选择 ${strategy}，因此跳过复杂 Agent DAG，只用 Writer Agent 汇总回答。`; /* 第57天：生成可解释计划原因。 */
    const task = strategy === "single-agent" ? `按当前运行时决策整理结构化且可执行的回答：${goal}` : `按当前运行时决策快速回答：${goal}`; /* 第57天：根据策略生成单 Agent 任务。 */
    return { goal, selectedAgents: ["writer"], reason, steps: [{ id: "day57-runtime-decision-writer", agentId: "writer", task, dependsOn: [] }] }; /* 第57天：返回只包含 Writer Agent 的轻量计划。 */
  } /* 第57天：结束轻量 AgentPlan 生成方法。 */
  ...
  private async attachPromptSnapshot(trace: Trace, promptABTest: PromptABTestResult): Promise<void> { /* 第61天：把提示词实验快照作为 Trace Attachment（追踪附件）写入对象存储。 */
    try { /* 第61天：对象存储失败不应打断主协作链路。 */
      const result = await uploadTraceAttachment(trace, "prompt-ab-test", promptABTest); /* 第61天：上传 Prompt A/B Test 快照到 Object Storage。 */
      this.traceManager.addAttachment(trace.traceId, { id: `${trace.traceId}-prompt-ab-test`, name: "prompt-ab-test", storage: result.attachment, createdAt: Date.now() }); /* 第61天：把对象存储引用挂回 Trace。 */
    } catch (err) { /* 第61天：捕获对象存储异常。 */
      console.warn("[Storage] trace attachment upload failed", err); /* 第61天：记录警告但保持运行时可用。 */
    } /* 第61天：结束异常处理。 */
  } /* 第61天：结束 attachPromptSnapshot 方法。 */

  private async exportWorkspaceSnapshot(workspace: Workspace): Promise<AgentCollaborationSnapshot["workspaceExport"]> { /* 第61天：把最终 Workspace（工作区）导出为对象存储文件。 */
    try { /* 第61天：对象存储失败不应打断主协作链路。 */
      return await exportWorkspaceToStorage(workspace); /* 第61天：上传 Markdown 工作区导出文件并返回引用。 */
    } catch (err) { /* 第61天：捕获对象存储异常。 */
      console.warn("[Storage] workspace export upload failed", err); /* 第61天：记录警告但保持运行时可用。 */
      return undefined; /* 第61天：失败时返回空引用。 */
    } /* 第61天：结束异常处理。 */
  } /* 第61天：结束 exportWorkspaceSnapshot 方法。 */
  ...
  private resolvePrompt(componentId: string, fallback: string, variables: Record<string, string | number | boolean | undefined | null>): ResolvedPrompt { /* 第54天：定义从注册表解析 active Prompt 并通过 PromptBuilder 组合最终提示词的方法。 */
    const template = this.promptRegistryStore.getActive(componentId); /* 第54天：读取组件当前 active（启用中）提示词模板。 */
    const blocks = buildRuntimePromptBlocks(componentId, template, fallback); /* 第54天：把 active 模板包装成 system block，并追加 memory、workspace、tool、task、output 块。 */
    const optimizationContext = this.inferPromptOptimizationContext(componentId, variables); /* 第55天：根据组件和变量推导动态提示词优化上下文。 */
    const strategy = optimizationContext.complexity === "high" ? "quality" : "balanced"; /* 第55天：高复杂度任务优先质量策略，其余任务使用平衡策略。 */
    const optimized = promptOptimizer.optimize(blocks, optimizationContext, strategy); /* 第55天：在 PromptBuilder 前先由优化器开关块并按权重排序。 */
    const result = promptBuilder.buildPromptWithReport(optimized.blocks, variables); /* 第55天：按优化后的块列表渲染最终提示词。 */
    const fallbackText = promptBuilder.buildPrompt(optimized.blocks.filter((block) => block.type === "system"), variables) || fallback; /* 第55天：当上下文块都不可用时仍保留系统块兜底。 */
    return { promptId: template?.id, promptVersion: template?.version, text: result.text || fallbackText }; /* 第54天：返回提示词元数据和组合后的最终正文。 */
  } /* 第54天：结束提示词解析方法。 */

  private inferPromptOptimizationContext(componentId: string, variables: Record<string, string | number | boolean | undefined | null>): PromptOptimizationContext { /* 第55天：定义从运行时变量推导 PromptOptimizationContext 的方法。 */
    const taskText = String(variables.task ?? ""); /* 第55天：读取当前任务文本。 */
    const taskType = this.inferPromptTaskType(componentId, taskText); /* 第55天：根据组件和任务文本推导任务类型。 */
    const hasMemory = this.hasPromptValue(variables.memory, ["暂无记忆", "无额外记忆"]); /* 第55天：判断是否存在真实记忆上下文。 */
    const hasWorkspace = this.hasPromptValue(variables.workspace, ["暂无工作空间", "尚未创建工作空间", "无额外工作空间"]); /* 第55天：判断是否存在真实工作空间上下文。 */
    const hasKnowledge = this.hasPromptValue(variables.knowledge, ["暂无知识", "无知识"]); /* 第55天：判断是否存在真实知识库证据。 */
    const requiresJson = /json|结构化|schema|只返回/i.test(taskText); /* 第55天：根据任务文本判断是否要求结构化输出。 */
    const requiresCitation = /引用|来源|证据|citation|论文|研究报告/i.test(taskText) || hasKnowledge; /* 第55天：根据任务文本和知识证据判断是否要求引用。 */
    const complexity = taskText.length > 80 || taskType === "research" || taskType === "evaluation" || taskType === "reflection" ? "high" : taskText.length > 30 ? "medium" : "low"; /* 第55天：用任务长度和任务类型推导复杂度。 */
    return { taskType, hasMemory, hasWorkspace, hasKnowledge, requiresJson, requiresCitation, complexity, userIntent: taskText.slice(0, 80) }; /* 第55天：返回可被 PromptOptimizer 消费的上下文。 */
  } /* 第55天：结束动态提示词优化上下文推导方法。 */

  private inferPromptTaskType(componentId: string, taskText: string): PromptTaskType { /* 第55天：定义任务类型推导方法。 */
    if (componentId === "reflection" || /反思|复盘|reflection/i.test(taskText)) return "reflection"; /* 第55天：反思组件或反思文本识别为 reflection。 */
    if (componentId === "evaluation" || /评估|打分|evaluate|evaluation/i.test(taskText)) return "evaluation"; /* 第55天：评估组件或评估文本识别为 evaluation。 */
    if (componentId === "planner" || /计划|规划|拆解|planning/i.test(taskText)) return "planning"; /* 第55天：规划组件或规划文本识别为 planning。 */
    if (componentId === "research" || /研究|分析|检索|论文|报告|research/i.test(taskText)) return "research"; /* 第55天：研究组件或研究文本识别为 research。 */
    return "chat"; /* 第55天：无法命中特定类型时按普通聊天处理。 */
  } /* 第55天：结束任务类型推导方法。 */

  private hasPromptValue(value: string | number | boolean | undefined | null, emptyMarkers: string[]): boolean { /* 第55天：定义提示词变量是否有真实内容的判断方法。 */
    const text = String(value ?? "").trim(); /* 第55天：把变量规范化为去空白字符串。 */
    return text.length > 0 && !emptyMarkers.some((marker) => text.includes(marker)); /* 第55天：非空且不包含空占位提示时视为真实上下文。 */
  } /* 第55天：结束提示词变量内容判断方法。 */
