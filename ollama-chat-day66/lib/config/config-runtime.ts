...
const globalForConfig = globalThis as typeof globalThis & { __day63ConfigManager?: ConfigManager }; // 第63天：扩展 globalThis，避免 Next.js 热重载时重复创建配置中心。
...
export function getConfigManager(): ConfigManager { // 第62天：定义读取配置中心单例的方法。
  if (!globalForConfig.__day63ConfigManager) globalForConfig.__day63ConfigManager = createConfigManager(); // 第63天：首次访问时创建配置中心。
  return globalForConfig.__day63ConfigManager; // 第63天：返回共享配置中心实例。
