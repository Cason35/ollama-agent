...
import { configManager } from "@/lib/config/config-runtime"; // 第62天：引入配置中心，运行时读取 RAG 检索参数。
...
export function getDefaultRecallK(): number { // 第62天：定义运行时读取默认召回数量的方法。
  return configManager.getNumber("retrieval.recallK", DEFAULT_RECALL_K); // 第62天：从配置中心读取 recallK，未配置时使用旧默认值。
} // 第62天：结束默认召回数量读取方法。
export function getDefaultRetrievalTopK(): number { // 第62天：定义运行时读取默认 TopK 的方法。
  return configManager.getNumber("retrieval.topK", DEFAULT_RETRIEVAL_TOP_K); // 第62天：从配置中心读取 topK，支持热更新立即生效。
} // 第62天：结束默认 TopK 读取方法。
export function getDefaultMinScore(): number { // 第62天：定义运行时读取默认最低分的方法。
  return configManager.getNumber("retrieval.minScore", DEFAULT_MIN_SCORE); // 第62天：从配置中心读取最低分阈值。
} // 第62天：结束默认最低分读取方法。
export function getDefaultMaxQueries(): number { // 第62天：定义运行时读取默认查询改写数量的方法。
  return configManager.getNumber("retrieval.maxQueries", 5); // 第62天：从配置中心读取 maxQueries。
} // 第62天：结束默认查询改写数量读取方法。
...
async function scoreChunksForQuery(query: string, documents: KnowledgeDocument[], options: Partial<RetrieveOptions>): Promise<ScoredChunkRow[]> {
  const recallK = Math.max(1, Math.min(50, Math.floor(options.recallK ?? getDefaultRecallK()))); // 第62天：从配置中心规整召回数。
...
export async function retrieveTopChunks(query: string, documents: KnowledgeDocument[], options: Partial<RetrieveOptions> = {}): Promise<RetrievedChunkHit[]> {
  ...
  if (!q) return []; // 空查询直接返回
  const topK = Math.max(1, Math.min(20, Math.floor(options.topK ?? getDefaultRetrievalTopK()))); // 第62天：从配置中心限制 TopK。
  const minScore = typeof options.minScore === "number" ? options.minScore : getDefaultMinScore(); // 第62天：从配置中心读取阈值。
...
export async function retrieveWithQueries(query: string, rewrittenQueries: string[], documents: KnowledgeDocument[], options: Partial<RetrieveOptions> = {}): Promise<MultiQueryRetrieveResult> {
  ...
  const topK = Math.max(1, Math.min(20, Math.floor(options.topK ?? getDefaultRetrievalTopK()))); // 第62天：从配置中心限制最终 TopK。
  const minScore = typeof options.minScore === "number" ? options.minScore : getDefaultMinScore(); // 第62天：从配置中心读取最低分。
  const mode = normalizeRetrievalMode(options.mode); // 规整模式
...
export async function multiQueryRetrieve(query: string, documents: KnowledgeDocument[], options: Partial<RetrieveOptions> = {}): Promise<MultiQueryRetrieveResult> {
  const q = query.trim(); // 规整查询文本
  if (!q) return { hits: [], rewrite: { originalQuery: "", rewrittenQueries: [], rewriteCount: 0 } }; // 空查询返回空结构
  const maxQueries = Math.max(1, Math.min(8, Math.floor(options.maxQueries ?? getDefaultMaxQueries()))); // 第62天：从配置中心限制 query 数。
