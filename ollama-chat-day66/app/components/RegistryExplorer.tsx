...
const TYPE_LABELS: Record<RegistryFilter, string> = { all: "全部", agent: "Agent", tool: "Tool", model: "Model", prompt: "Prompt", memory: "Memory", workflow: "Workflow", evaluation: "Evaluation" }; // 第66天：定义注册类型过滤按钮的中英文紧凑标签。
...
export function RegistryExplorer() { // 第66天：定义展示统一能力库存、发现结果和指标的注册浏览器组件。
  ...
  return ( // 第66天：返回 Registry Explorer 的指标、搜索、过滤和注册项列表视图。
    <section className="space-y-3 p-3 text-xs"> {/* 第66天：定义统一注册浏览器主体容器。 */}
      ...
      ) : <p className="rounded-lg border border-dashed border-zinc-300 p-4 text-zinc-500 dark:border-zinc-700">正在创建 Day66 Unified Registry 能力目录...</p>} {/* 第66天：首次加载时展示统一注册中心占位提示。 */}
