...
export function KnowledgeSidebar({
  ...
  const [activePanel, setActivePanel] = useState<"registry" | "events" | "context" | "secrets" | "config" | "storage" | "lock" | "queue" | "runtime" | "redis" | "experiment" | "prompt" | "collaboration" | "model" | "memory" | "cache" | "usage" | "knowledge" | "records">("registry"); /* 第66天：默认打开统一注册面板，突出 Unified Registry 和能力发现。 */
      ...
      <div className="z-10 shrink-0 border-b border-zinc-200/80 bg-white px-3 py-3 dark:border-zinc-800 dark:bg-zinc-900">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-0.5 text-[11px] text-zinc-500 dark:text-zinc-400">按主题查看统一注册、能力发现、统一事件、上下文、模型、提示词、知识与历史记录。</p> {/* 第66天：更新控制台说明并突出统一能力库存和发现入口。 */}
          </div>
          <span className="rounded-full bg-emerald-500/15 px-2.5 py-1 text-[10px] font-semibold text-emerald-800 ring-1 ring-emerald-500/20 dark:text-emerald-200">
            Day 66 {/* 第66天：更新控制台日期徽标。 */}
          </span>
        </div>
        <div className="mt-3 grid grid-cols-12 gap-1" role="tablist" aria-label="Day 66 侧边栏面板"> {/* 第66天：扩展为包含 Registry Explorer 的统一能力目录标签页。 */}
