...
import type { Job, JobType, QueueMetrics } from "@/lib/queue/queue-types"; // 第32天：引入队列任务与指标类型
import { QueueDashboard } from "@/app/components/QueueDashboard"; // 第32天：引入 Queue Dashboard 组件
...
type KnowledgeSidebarProps = {
  ...
  queueJobs: Job[]; // 第32天：队列任务列表
  queueMetrics: QueueMetrics | null; // 第32天：队列指标
  queueLoading: boolean; // 第32天：队列请求加载状态
  handleCreateQueueJob: (type: JobType) => Promise<void>; // 第32天：创建队列任务动作
  handleRequeueQueueJob: (jobId: string) => Promise<void>; // 第32天：死信任务重新入队动作
};
    ...
    <aside className="flex min-h-0 w-full shrink-0 flex-col overflow-y-auto overflow-x-hidden rounded-xl border border-zinc-200/80 bg-white/78 shadow-lg shadow-zinc-900/5 ring-1 ring-white/50 backdrop-blur-md dark:border-zinc-800/90 dark:bg-zinc-900/65 dark:shadow-black/30 dark:ring-zinc-700/40 lg:h-full lg:w-[320px] xl:w-[340px]">
      ...
      <div className="shrink-0 border-b border-emerald-200/70 px-4 py-3 dark:border-emerald-900/40">
        ...
        <p className="mt-1 text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">
          第32天：继承 retrieval / ragAnswer / reindexKnowledge，并新增 Queue Runtime V2 观察 retry / backoff / DLQ。
        </p>
      ...
      <div className="shrink-0 border-b border-sky-200/70 px-4 py-3 dark:border-sky-900/40">
        ...
        <p className="mt-1 text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">
          第32天：继承 RAG V7 · Knowledge Store · Local Vector Store · Metadata Filter · Vector Explorer。
        </p>
        {knowledgeMetrics ? (
      ...
      <div className="shrink-0 border-b border-amber-200/70 px-4 py-3 dark:border-amber-900/40">
        ...
        <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
          第32天：RAG 仍先走 Local Vector Store 召回，再回查 KnowledgeStore；失败长任务可进入 retry / DLQ。
        </p>
