...
import { getQueueRuntime } from "@/lib/queue/queue-runtime"; // 引入第32天队列运行时单例
import type { CreateJobInput, JobType } from "@/lib/queue/queue-types"; // 引入创建任务输入类型

export const dynamic = "force-dynamic"; // 声明该路由总是动态执行

const JOB_TYPES: JobType[] = ["workflow", "retrieval", "embedding", "reindex", "unstable", "alwaysFail"]; // 定义允许创建的任务类型
