...
import type { Job, JobType, QueueMetrics } from "@/lib/queue/queue-types"; // 第32天：Queue Runtime V2 任务与指标类型
...
export default function HomePage() {
  ...
  const [queueJobs, setQueueJobs] = useState<Job[]>([]); // 第32天：Queue Dashboard 任务列表
  const [queueMetrics, setQueueMetrics] = useState<QueueMetrics | null>(null); // 第32天：Queue Metrics V2 指标
  const [queueLoading, setQueueLoading] = useState(false); // 第32天：队列 API 加载状态
  ...
  /** 第32天：加载队列快照（Retry + Backoff + DLQ + Queue Metrics V2）。 */
  ...
  /** 第32天：创建后台 Job 并交给 Worker 异步执行与失败恢复。 */
  const handleCreateQueueJob = async (type: JobType) => {
    ...
    try {
      ...
      const res = await fetch("/api/queue", {
        ...
        body: JSON.stringify({ type, payload: payloadByType[type] }), // 请求体
      }); // fetch 调用结束
  ...
  /** 第32天：从 Dead Letter Queue 手动重新入队。 */
