...
export class AgentRuntime { /* 定义第40天基于 Supervisor 的智能体运行时。 */
  ...
  constructor(private readonly registry: AgentRegistry, private readonly workspaceStore: WorkspaceStore = new MemoryWorkspaceStore(), private readonly usageStore: UsageManager = usageManager, private readonly cacheStore: SemanticCache = semanticCache, private readonly memoryStore: LongTermMemoryStore = longTermMemory) {} /* 第49天：在第48天依赖注入基础上追加共享长期记忆存储，让运行时具备跨任务经验积累能力。 */
  ...
  private recordComponentUsage(traceId: string, spanId: string, componentType: UsageComponentType, componentId: string, inputText: string, outputText: string, startedAt: number): UsageRecord { /* 第47天：定义各运行阶段共享的用量写入方法。 */
    return this.usageStore.addRecord({ traceId, spanId, componentType, componentId, inputTokens: estimateTokenCount(inputText), outputTokens: estimateTokenCount(outputText), durationMs: Math.max(1, Date.now() - startedAt) }); /* 第47天：估算词元、费用和耗时后写入统一 UsageManager。 */
  } /* 第47天：结束运行阶段用量写入方法。 */
