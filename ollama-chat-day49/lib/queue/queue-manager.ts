...
export class QueueManager { // 定义第35天本地队列管理器。
  ...
  cancelQueuedJob(job: Job): Job | undefined { // 第36天：取消仍在内存队列中等待的任务。
    const index = this.queue.findIndex((item) => item.id === job.id); // 在内存队列中查找目标任务。
    if (index < 0) return undefined; // 如果任务已经被 Worker 认领，则交给运行中取消流程处理。
    const now = Date.now(); // 获取当前时间戳。
    const cancelled: Job = { // 构造已取消任务快照。
      ...this.queue[index], // 继承队列里的最新任务字段。
      status: "cancelled", // 将状态直接改为已取消。
      cancelRequestedAt: job.cancelRequestedAt ?? now, // 记录取消请求时间。
      ...
      timeline: [...this.queue[index].timeline, timeline("CancelRequested", "用户取消了尚未运行的任务"), timeline("Cancelled", "任务仍在队列中等待，因此已直接取消")], // 追加取消生命周期事件。
    }; // 结束已取消任务构造。
    this.queue.splice(index, 1); // 从可认领队列中移除该任务。
    return cancelled; // 返回已取消任务，交给运行时持久化。
  } // 结束 cancelQueuedJob。
