...
import type { UnifiedRegistry } from "@/lib/registry/unified-registry"; /* 第66天：引入可选统一注册中心类型供默认模型工厂注入。 */
...
export function createDefaultModelRegistry(unifiedRegistry?: UnifiedRegistry): ModelRegistry { /* 第66天：允许默认模型工厂可选接入统一注册中心并兼容旧调用。 */
  const registry = new ModelRegistry(unifiedRegistry); /* 第66天：创建可同步统一能力目录的模型注册表实例。 */
