...
import { configManager } from "@/lib/config/config-runtime"; // 第62天：引入配置中心，统一读取本地对象存储 Bucket。
...
export class LocalObjectStorageProvider implements StorageProvider { // 第61天：定义本地对象存储 Provider 类。
  ...
  constructor(readonly defaultBucket = configManager.getString("storage.bucket", "agent-platform"), private readonly rootDir = path.join(process.cwd(), ".data", "object-storage")) {} // 第62天：从配置中心初始化默认 Bucket 和本地根目录。
