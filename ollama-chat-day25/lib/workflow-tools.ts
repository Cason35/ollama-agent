...
import { executeRagAnswer } from "@/lib/knowledge-rag"; // 第25天：RAG V2 + fallback
import { knowledgeStore } from "@/lib/knowledge-store"; // 第24天：本地知识库
import { DEFAULT_RETRIEVAL_TOP_K, DEFAULT_MIN_SCORE } from "@/lib/knowledge-retrieval"; // 默认 TopK / minScore
...
/** 第25天：retrieval — TopK + minScore 语义检索。 */
const retrievalTool: Tool = {
  name: "retrieval", // 工具名
  description: "从知识库检索与问题最相关的文本块（TopK + minScore 过滤）", // 描述
  capabilities: ["knowledge-retrieval"], // 能力：知识检索
  inputSchema: { input: { type: "string", required: true, description: "检索查询" } }, // 查询必填
  outputSchema: { query: "string", hits: "array", topK: "number", minScore: "number" }, // 命中列表
  async execute(input) {
    const query =
    ...
    const hits = await knowledgeStore.search(query, { topK, minScore }); // 向量检索 + 过滤
    return { query, hits, topK, minScore }; // 返回可观测结构
  },
};

/** 第25天：ragAnswer — 检索 + 低幻觉 Prompt + 无结果 fallback。 */
const ragAnswerTool: Tool = {
  name: "ragAnswer", // 工具名
  description: "基于知识库 RAG V2：minScore 过滤、严谨 Prompt、无合格片段时不硬编", // 描述
  ...
  async execute(input) {
    ...
    const result = await executeRagAnswer(question, hits, input.rt); // RAG V2 执行
    return {
