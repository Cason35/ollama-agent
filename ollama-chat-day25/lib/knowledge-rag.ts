/**
 * 第25天：RAG Prompt V2 — 严谨低幻觉 Prompt + 无合格 chunk 时的 fallback。
 */
...
/**
 * 将 TopK 知识片段格式化为 Prompt 中的「知识片段」区块（含 score / 来源 / 块序号）。
 * @param hits 检索结果
 */
...
export function buildRagPrompt(question: string, hits: RetrievedChunkHit[]): string {
  ...
  const usedChunkIds = hits.map((h, i) => `Chunk ${i + 1} (${h.chunkId}, score ${h.score})`).join("; "); // 引用列表
  return `
你是一个严谨的知识库问答助手。
