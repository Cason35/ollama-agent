  ...
  const handleCreateQueueJob = async (input: CreateJobInput) => {
    ...
    try {
      const payloadByType: Record<JobType, Record<string, unknown>> = {
        chat: { prompt: "第41天 chat 任务，用于在 Agent DAG Runtime 项目中继续测试 llm 资源速率限制与生命周期控制" }, // chat 任务载荷
        ...
        reminder: { text: "第41天 Reminder Job 已按 scheduledAt 自动触发" }, // reminder 定时提醒任务载荷
      }; // 任务载荷映射结束
