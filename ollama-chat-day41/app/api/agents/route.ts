...
export async function GET() { /* 定义 GET /api/agents 接口。 */
  ...
  const demoResult = await executeAgent("supervisor", { id: "day41-demo-task", goal: "演示第41天 Agent DAG Planning 执行入口", assignedAgentId: "supervisor" }); /* 执行一个轻量 Supervisor 示例任务。 */
  const collaboration = await executeSupervisorAgentCollaboration("研究 LangGraph，总结核心概念，制定三天学习路线，并输出最终报告"); /* 执行第41天 Supervisor DAG 多智能体协作链。 */
