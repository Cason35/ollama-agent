export type PromptBlockType = "system" | "memory" | "workspace" | "knowledge" | "tool" | "task" | "reflection" | "evaluation" | "citation" | "schema" | "output"; /* 第55天：扩展提示词块的业务类型，覆盖知识、引用、结构化输出、反思和评估等动态优化块。 */
export type PromptBlock = { /* 第54天：定义 PromptBlock（提示词块）基础结构。 */
  ...
  weight: number; /* 第55天：保存提示词块权重，高权重块会在动态组合时更靠前。 */
...
export type PromptBlockFieldChange = { /* 第54天：定义块级字段变化记录。 */
  field: "name" | "type" | "template" | "enabled" | "order" | "weight"; /* 第55天：保存发生变化的字段名称，并纳入块权重差异。 */
...
export type PromptBlockMetric = { /* 第54天：定义单个提示词块的指标。 */
  ...
  weight: number; /* 第55天：保存提示词块权重，便于观察动态优化排序依据。 */
