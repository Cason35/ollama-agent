...
export class MinIOStorageProvider implements StorageProvider { // 第61天：定义 MinIO 对象存储 Provider 类。
  ...
  constructor(options: { endPoint?: string; port?: number; useSSL?: boolean; accessKey?: string; secretKey?: string; bucket?: string } = {}) { // 第61天：允许通过环境变量或测试参数创建 Provider。
    this.defaultBucket = options.bucket ?? process.env.OBJECT_STORAGE_BUCKET ?? "agent-platform"; // 第61天：读取默认 Bucket。
    this.client = new Client({ endPoint: options.endPoint ?? process.env.MINIO_ENDPOINT ?? "127.0.0.1", port: options.port ?? Number(process.env.MINIO_PORT ?? 9000), useSSL: options.useSSL ?? process.env.MINIO_USE_SSL === "true", accessKey: options.accessKey ?? process.env.MINIO_ACCESS_KEY ?? "minioadmin", secretKey: options.secretKey ?? process.env.MINIO_SECRET_KEY ?? "minioadmin" }); // 第61天：创建 MinIO 客户端。
  } // 第61天：结束构造函数。
