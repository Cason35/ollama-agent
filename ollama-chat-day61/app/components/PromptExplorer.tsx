...
import { PromptStrategyExplorer } from "@/app/components/PromptStrategyExplorer"; /* 第55天：引入提示词策略浏览器，展示动态优化结果。 */
    ...
    <section className="shrink-0 border-b border-teal-200/70 px-4 py-3 dark:border-teal-900/40"> {/* 第52天：定义提示词浏览器外层容器。 */}
      <div className="flex items-start justify-between gap-3"> {/* 第52天：排列标题说明与刷新按钮。 */}
        <div> {/* 第52天：定义标题说明区域。 */}
          ...
          <p className="mt-1 text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">Day 61 · Prompt Registry 继续管理版本，为 Distributed Queue 项目中的运行时决策提供 Fast、Balanced、Quality 和 JSON 策略素材。</p> {/* 第60天：展示提示词版本与分布式队列基础设施的关系。 */}
        </div> {/* 第52天：结束标题说明区域。 */}
      ...
      {snapshot ? ( /* 第52天：快照存在时渲染看板主体。 */
        <div className="mt-3 space-y-3"> {/* 第52天：定义看板主体垂直间距。 */}
          ...
          <div className="flex gap-1 overflow-x-auto rounded-lg bg-zinc-100 p-1 dark:bg-zinc-900" role="tablist" aria-label="Day 61 Prompt Strategy 组件标签页"> {/* 第60天：定义组件标签页导航。 */}
          ...
          <div className="rounded-lg border border-indigo-200 bg-indigo-50/60 p-2.5 text-[10px] text-indigo-950 dark:border-indigo-900/50 dark:bg-indigo-950/20 dark:text-indigo-100"> {/* 第54天：定义 Prompt Block Explorer 卡片。 */}
            <div className="flex items-start justify-between gap-2"> {/* 第54天：排列块视图标题和启用率指标。 */}
              <div> {/* 第54天：定义块视图标题区域。 */}
                <p className="text-[11px] font-semibold">Prompt Block Explorer（提示词块浏览器）</p> {/* 第54天：展示提示词块浏览器标题。 */}
                <p className="mt-1 text-[9px] leading-relaxed text-indigo-800 dark:text-indigo-200">按 weight 和 order 组合 system、memory、workspace、knowledge、citation、task、schema、output，并由 PromptOptimizer 动态开关条件块。</p> {/* 第55天：说明块权重排序、动态开关和条件跳过能力。 */}
              </div> {/* 第54天：结束块视图标题区域。 */}
              <span className="shrink-0 rounded-full bg-white/70 px-2 py-1 font-mono text-[9px] font-semibold text-indigo-800 dark:bg-zinc-950/30 dark:text-indigo-200">{Math.round(snapshot.blockMetrics.enabledRate * 100)}% enabled</span> {/* 第54天：展示提示词块启用率。 */}
            </div> {/* 第54天：结束块视图标题和启用率布局。 */}
            <div className="mt-2 grid grid-cols-3 gap-1 text-center"> {/* 第54天：定义提示词块聚合指标网格。 */}
              <div className="rounded bg-white/70 p-1.5 dark:bg-zinc-950/30"><p className="text-[8px] text-indigo-700">Blocks</p><p className="font-mono text-[10px] font-semibold">{snapshot.blockMetrics.totalBlocks}</p></div> {/* 第54天：展示提示词块总数。 */}
              <div className="rounded bg-white/70 p-1.5 dark:bg-zinc-950/30"><p className="text-[8px] text-indigo-700">Avg Token</p><p className="font-mono text-[10px] font-semibold">{snapshot.blockMetrics.averageTokens}</p></div> {/* 第54天：展示平均估算 token 数。 */}
              <div className="rounded bg-white/70 p-1.5 dark:bg-zinc-950/30"><p className="text-[8px] text-indigo-700">Used</p><p className="font-mono text-[10px] font-semibold">{snapshot.compositionPreview.usedBlockIds.length}</p></div> {/* 第54天：展示组合预览命中的块数量。 */}
            </div> {/* 第54天：结束提示词块聚合指标网格。 */}
            <ul className="mt-2 grid gap-1.5 md:grid-cols-2"> {/* 第54天：定义提示词块列表。 */}
              {snapshot.blockMetrics.blocks.map((block) => <li key={block.blockId} className="rounded bg-white/75 p-2 dark:bg-zinc-950/30"><div className="flex items-center justify-between gap-2"><p className="truncate font-semibold">{block.order}. {block.blockName}</p><span className="font-mono text-[8px]">{block.type}</span></div><p className="mt-1 font-mono text-[8px] text-indigo-700 dark:text-indigo-200">{block.blockId}</p><p className="mt-1 text-[8px] text-indigo-800 dark:text-indigo-200">weight {block.weight} · length {block.length} · tokens {block.estimatedTokens} · hit {(block.hitRate * 100).toFixed(0)}% · {block.enabled ? "enabled" : "disabled"}</p></li>)} {/* 第55天：逐个展示块顺序、权重、类型、长度、token、命中率和启用状态。 */}
            </ul> {/* 第54天：结束提示词块列表。 */}
          </div> {/* 第54天：结束 Prompt Block Explorer 卡片。 */}
          <div className="rounded-lg border border-violet-200 bg-violet-50/60 p-2.5 text-[10px] text-violet-950 dark:border-violet-900/50 dark:bg-violet-950/20 dark:text-violet-100"> {/* 第54天：定义 Block Diff 卡片。 */}
            <p className="text-[11px] font-semibold">Block Diff（提示词块差异） · {snapshot.blockComparison.baselineBlockId} → {snapshot.blockComparison.candidateBlockId}</p> {/* 第54天：展示 Block Diff 标题。 */}
            <div className="mt-2 grid gap-2 xl:grid-cols-2"> {/* 第54天：定义字段变化和模板行变化布局。 */}
              <div><p className="font-semibold">字段变化</p><ul className="mt-1 space-y-1">{snapshot.blockComparison.changes.map((change) => <li key={change.field} className="rounded bg-white/70 px-2 py-1 font-mono text-[9px] dark:bg-zinc-950/30">{change.field}: {String(change.before)} → {String(change.after)}</li>)}</ul></div> {/* 第54天：展示名称、类型、正文、启用状态或顺序变化。 */}
              <div><p className="font-semibold">模板变化</p><ul className="mt-1 space-y-1">{[...snapshot.blockComparison.addedLines.map((line) => `+ ${line}`), ...snapshot.blockComparison.removedLines.map((line) => `- ${line}`)].map((line) => <li key={line} className="rounded bg-white/70 px-2 py-1 font-mono text-[9px] dark:bg-zinc-950/30">{line}</li>)}</ul></div> {/* 第54天：展示块模板新增和删除行。 */}
            </div> {/* 第54天：结束字段变化和模板行变化布局。 */}
          </div> {/* 第54天：结束 Block Diff 卡片。 */}
          <PromptStrategyExplorer snapshot={snapshot} /> {/* 第55天：展示 Fast、Balanced、Quality 三种动态提示词策略。 */}
          <div className="rounded-lg border border-zinc-200 bg-zinc-50 p-2.5 text-[10px] dark:border-zinc-800 dark:bg-zinc-950/25"> {/* 第55天：定义 Dynamic Prompt Preview 卡片。 */}
            <p className="text-[11px] font-semibold text-zinc-900 dark:text-zinc-50">Dynamic Prompt Preview（动态提示词预览）</p> {/* 第55天：展示动态提示词预览标题。 */}
            <p className="mt-1 font-mono text-[9px] text-zinc-500">used: {snapshot.compositionPreview.usedBlockIds.join(" → ")} · skipped: {snapshot.compositionPreview.skippedBlockIds.join(" / ") || "none"}</p> {/* 第54天：展示命中块和跳过块。 */}
            <pre className="mt-2 max-h-56 overflow-auto whitespace-pre-wrap rounded-md bg-white p-2 font-mono text-[9px] leading-relaxed text-zinc-600 dark:bg-zinc-900 dark:text-zinc-300">{snapshot.compositionPreview.text}</pre> {/* 第54天：展示 PromptBuilder 组合后的最终提示词。 */}
          </div> {/* 第54天：结束 Prompt Composition Preview 卡片。 */}
        </div> /* 第52天：结束看板主体。 */
      ) : null} {/* 第52天：结束快照条件渲染。 */}
