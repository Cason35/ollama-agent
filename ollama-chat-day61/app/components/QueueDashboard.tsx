...
export function QueueDashboard({ // 导出队列看板组件。
  ...
  const createTenEmbeddingJobs = () => { // 第36天：定义 10 个 embedding 资源并发与取消压测入口。
    Array.from({ length: 10 }, (_, index) => index + 1).forEach((index) => { // 循环创建 10 个 embedding 长任务。
      void handleCreateQueueJob({ type: "embedding", priority: 1, timeoutMs: 30000, payload: { durationMs: 5000, document: `Day61 Redis Queue embedding 生命周期压测任务 ${index}` } }); // 第60天：创建 5 秒 embedding 任务。
    }); // 结束循环。
  ...
  const createFiveChatJobs = () => { // 第36天：定义 llm 速率限制压测入口。
    Array.from({ length: 5 }, (_, index) => index + 1).forEach((index) => { // 循环创建 5 个 chat 任务。
      void handleCreateQueueJob({ type: "chat", priority: 10, payload: { prompt: `Day61 Redis Queue llm rate limit burst ${index}` } }); // 第60天：创建 chat 任务观察每秒速率限制。
    }); // 结束循环。
  }; // 结束 createFiveChatJobs。
  const createTimeoutEmbeddingJob = () => handleDebouncedCreateJob({ type: "embedding", priority: 10, timeoutMs: 1200, payload: { durationMs: 5000, document: "Day61 Redis Queue visibility timeout 测试任务" } }); // 第60天：创建一个会触发超时的 embedding 任务。
  const createManualWorkflowJob = () => { // 第37天：定义无需聊天入口也能创建的最小 WorkflowJob。
    const workflowId = `manual_wf_${Date.now()}`; // 生成手动工作流 ID。
    handleDebouncedCreateJob({ type: "workflow", priority: 8, timeoutMs: 120000, payload: { workflowId, workflow: { id: workflowId, goal: "手动创建 Day61 Redis Queue WorkflowJob 示例", status: "queued", steps: [], executionTimeline: [{ ts: Date.now(), message: "手动 WorkflowJob 已在 Queue Explorer 创建" }] }, memory: { shortTerm: [], items: [] }, provider: "local" } }); // 第60天：创建包含完整 workflow 与 memory 的合法 WorkflowJob。
  }; // 结束 createManualWorkflowJob。
  return ( // 返回队列看板 UI。
    <div className="shrink-0 border-b border-cyan-200/70 px-4 py-3 dark:border-cyan-900/40"> {/* 队列看板容器。 */}
      <h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">Queue Explorer（队列浏览器）Day 61</h2> {/* 第60天：看板标题。 */}
      ...
      <div className="mt-3 overflow-hidden rounded-lg border border-cyan-200/70 dark:border-cyan-900/40"> {/* 任务表格外框。 */}
        ...
        <ul className="max-h-44 overflow-y-auto"> {/* 任务列表。 */}
          {jobs.length === 0 ? ( // 判断是否暂无任务。
            <li className="px-2 py-3 text-center text-[11px] text-zinc-400">暂无任务；点击上方按钮创建 Day 61 Redis Queue Job（任务）。</li> // 第60天：空状态。
          ) : ( // 有任务时渲染列表。
            jobs.map((job) => ( // 遍历任务。
