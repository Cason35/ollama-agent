...
import { StorageExplorer } from "@/app/components/StorageExplorer"; // 第61天：引入 Object Storage Explorer（对象存储浏览器）组件。
...
export function KnowledgeSidebar({
  ...
  const [activePanel, setActivePanel] = useState<"storage" | "lock" | "queue" | "runtime" | "redis" | "experiment" | "prompt" | "collaboration" | "model" | "memory" | "cache" | "usage" | "knowledge" | "records">("storage"); /* 第61天：默认打开 Storage 面板，突出 Object Storage。 */
  ...
  return (
    <aside className="flex min-h-0 w-full shrink-0 flex-col overflow-hidden rounded-lg border border-zinc-200/80 bg-white shadow-sm shadow-zinc-900/5 dark:border-zinc-800 dark:bg-zinc-900/80 dark:shadow-black/30 lg:h-full lg:w-[400px] xl:w-[430px]">
      <div className="z-10 shrink-0 border-b border-zinc-200/80 bg-white px-3 py-3 dark:border-zinc-800 dark:bg-zinc-900">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-0.5 text-[11px] text-zinc-500 dark:text-zinc-400">按主题查看 Storage、Lock、Queue、Redis、运行时决策、模型协作、提示词实验、模型健康、用量、知识与历史记录。</p> {/* 第61天：更新控制台标签页说明，突出 Object Storage。 */}
          </div>
          <span className="rounded-full bg-emerald-500/15 px-2.5 py-1 text-[10px] font-semibold text-emerald-800 ring-1 ring-emerald-500/20 dark:text-emerald-200">
            Day 61 {/* 第61天：更新控制台日期徽标。 */}
          </span>
        </div>
        <div className="mt-3 grid grid-cols-12 gap-1" role="tablist" aria-label="Day 61 侧边栏面板"> {/* 第61天：扩展为包含 Storage 浏览器的标签页。 */}
          <button type="button" className={tabClass("storage")} onClick={() => setActivePanel("storage")}> {/* 第61天：Storage 对象存储标签页入口。 */}
            存储 {/* 第61天：显示紧凑的 Storage 标签名称。 */}
          </button> {/* 第61天：结束 Storage 标签页按钮。 */}
          ...
          <button type="button" className={tabClass("prompt")} onClick={() => setActivePanel("prompt")}> {/* 第52天：提示词版本管理标签页入口。 */}
            提示 {/* 第52天：显示紧凑的提示词标签名称。 */}
          </button> {/* 第52天：结束提示词标签页按钮。 */}
      ...
      <div className={activePanel === "storage" ? "contents" : "hidden"}> {/* 第61天：仅在存储标签页展示 Object Storage Explorer。 */}
        <StorageExplorer /> {/* 第61天：展示 Bucket、Object Key、Size、Signed URL、Storage Metrics 与对象操作。 */}
      </div> {/* 第61天：结束 Storage 标签页内容。 */}
      ...
      <div className={activePanel === "prompt" ? "contents" : "hidden"}> {/* 第52天：仅在提示词标签页展示 Prompt Explorer。 */}
        <PromptExplorer /> {/* 第52天：展示提示词注册表、版本、diff、rollback 与回归关联。 */}
      </div> {/* 第52天：结束提示词标签页内容。 */}
      ...
      <div className={activePanel === "memory" ? "contents" : "hidden"}> {/* 第49天：仅在记忆标签页展示 Memory Explorer V2。 */}
        <MemoryExplorerV2 /> {/* 第49天：展示长期记忆经验提取、综合检索、整合压缩与重要性衰减。 */}
      </div> {/* 第49天：结束记忆标签页内容。 */}
