...
import type { CreateJobInput, Job, JobType, QueueMetrics, WorkerPoolSnapshot } from "@/lib/queue/queue-types"; // 第34天：Queue Runtime V4 任务、创建输入、指标与 WorkerPool 快照类型
...
export default function HomePage() {
  ...
  const [queueJobs, setQueueJobs] = useState<Job[]>([]); // 第34天：Queue Dashboard 任务列表
  const [queueMetrics, setQueueMetrics] = useState<QueueMetrics | null>(null); // 第34天：Queue Metrics V4 指标
  const [workerPool, setWorkerPool] = useState<WorkerPoolSnapshot | null>(null); // 第34天：WorkerPool 并发状态快照
  const [queueLoading, setQueueLoading] = useState(false); // 第34天：队列 API 加载状态
  ...
  /** 第34天：加载队列快照（Priority + Schedule + Retry + DLQ + WorkerPool + Concurrency Metrics）。 */
  const refreshQueuePanel = async () => {
    try {
      const res = await fetch("/api/queue"); // GET 队列快照
      const payload = await readApiData<{ jobs: Job[]; metrics: QueueMetrics; workerPool?: WorkerPoolSnapshot }>(res); // 解析队列响应
  ...
  /** 第34天：创建后台 Job，并携带 priority / scheduledAt 交给 WorkerPool 并发调度。 */
  const handleCreateQueueJob = async (input: CreateJobInput) => {
    ...
    try {
      const payloadByType: Record<JobType, Record<string, unknown>> = {
        ...
        embedding: { durationMs: 5000, document: "模拟 5000ms embedding 长任务" }, // embedding 长任务载荷
        ...
        reminder: { text: "第34天 Reminder Job 已按 scheduledAt 自动触发" }, // reminder 定时提醒任务载荷
      }; // 任务载荷映射结束
      ...
      const payload = await readApiData<{ jobs: Job[]; metrics: QueueMetrics; workerPool?: WorkerPoolSnapshot }>(res); // 解析创建结果
  ...
  /** 第34天：从 Dead Letter Queue 手动重新入队并交给 WorkerPool 重新认领。 */
  const handleRequeueQueueJob = async (jobId: string) => {
    ...
    try {
      ...
      const payload = await readApiData<{ jobs: Job[]; metrics: QueueMetrics; workerPool?: WorkerPoolSnapshot }>(res); // 解析恢复结果
      setQueueJobs(payload.jobs ?? []); // 立即更新任务列表
      setQueueMetrics(payload.metrics ?? null); // 立即更新指标
      setWorkerPool(payload.workerPool ?? null); // 立即更新 WorkerPool 快照
    } catch (err) {
