...
import type { CreateJobInput, Job, JobType, QueueMetrics, WorkerPoolSnapshot } from "@/lib/queue/queue-types"; // 引入队列任务、创建输入、指标与 WorkerPool 类型。
...
type QueueDashboardProps = { // 定义队列看板组件参数。
  ...
  workerPool: WorkerPoolSnapshot | null; // 第34天：接收 WorkerPool 快照。
...
function formatLockedAt(job: Job): string { // 第34天：定义任务锁时间格式化函数。
...
function formatHeartbeat(at?: number): string { // 第34天：定义 Worker 心跳格式化函数。
...
export function QueueDashboard({ // 导出队列看板组件。
  ...
  const createFiveSleepJobs = () => { // 第34天：定义创建五个 5 秒长任务的测试入口。
    Array.from({ length: 5 }, (_, index) => index + 1).forEach((index) => { // 循环创建 5 个 embedding 长任务。
      void handleCreateQueueJob({ type: "embedding", priority: 1, payload: { durationMs: 5000, document: `Day34 并发测试任务 ${index}` } }); // 创建 5 秒模拟任务。
    }); // 结束循环。
  }; // 结束 createFiveSleepJobs。
  return ( // 返回队列看板 UI。
    <div className="shrink-0 border-b border-cyan-200/70 px-4 py-3 dark:border-cyan-900/40"> {/* 队列看板容器。 */}
      <h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">Queue Runtime V4</h2> {/* 看板标题。 */}
      <p className="mt-1 text-xs leading-relaxed text-zinc-500 dark:text-zinc-400"> {/* 看板说明。 */}
        第34天：Concurrency + Worker Pool，在 Priority Scheduler 基础上观察多 Worker 并发认领、job lock、heartbeat、stale lock 与吞吐指标。{/* 说明文字。 */}
      </p> {/* 结束看板说明。 */}
      <div className="mt-2 grid grid-cols-2 gap-2 text-[10px]"> {/* 队列指标网格。 */}
        ...
        <div className="rounded-lg border border-cyan-200/70 bg-cyan-50/60 px-2 py-1.5 dark:border-cyan-900/40 dark:bg-cyan-950/20">concurrency: {workerPool?.metrics.concurrency ?? 0}</div> {/* 并发数指标。 */}
        <div className="rounded-lg border border-emerald-200/70 bg-emerald-50/60 px-2 py-1.5 dark:border-emerald-900/40 dark:bg-emerald-950/20">throughput/min: {workerPool?.metrics.throughputPerMinute ?? 0}</div> {/* 吞吐量指标。 */}
      </div> {/* 结束队列指标网格。 */}
      <p className="mt-2 font-mono text-[10px] text-cyan-900 dark:text-cyan-100">active: {workerPool?.metrics.activeWorkers ?? 0} | idle: {workerPool?.metrics.idleWorkers ?? 0} | avg job: {workerPool?.metrics.avgJobDuration ?? 0}ms</p> {/* WorkerPool 指标摘要。 */}
      ...
      <div className="mt-2 grid grid-cols-2 gap-2"> {/* 创建任务按钮区域。 */}
        ...
        <button type="button" disabled={queueLoading} onClick={createFiveSleepJobs} className={`${createJobButtonClass} col-span-2`}>Create 5 Sleep Jobs</button> {/* 创建五个 5 秒并发测试任务。 */}
      </div> {/* 结束创建任务按钮区域。 */}
      <div className="mt-3 overflow-hidden rounded-lg border border-cyan-200/70 dark:border-cyan-900/40"> {/* 任务表格外框。 */}
        <div className="grid grid-cols-[1fr_0.65fr_0.75fr_0.75fr_0.8fr_0.7fr] bg-cyan-50/80 px-2 py-1 font-mono text-[9px] text-cyan-900 dark:bg-cyan-950/25 dark:text-cyan-100"> {/* 表头。 */}
          <span>Job</span> {/* Job 表头。 */}
          <span>Type</span> {/* Type 表头。 */}
          <span>Priority</span> {/* Priority 表头。 */}
          <span>Worker</span> {/* Worker 表头。 */}
          <span>Status</span> {/* Status 表头。 */}
          <span>Lock</span> {/* Lock 表头。 */}
        </div> {/* 结束表头。 */}
        <ul className="max-h-44 overflow-y-auto"> {/* 任务列表。 */}
          {jobs.length === 0 ? ( // 判断是否暂无任务。
            <li className="px-2 py-3 text-center text-[11px] text-zinc-400">暂无任务；点击上方按钮创建 Day 34 Job。</li> // 空状态。
          ) : ( // 有任务时渲染列表。
            jobs.map((job) => ( // 遍历任务。
              <li key={job.id} className="grid grid-cols-[1fr_0.65fr_0.75fr_0.75fr_0.8fr_0.7fr] items-center gap-1 border-t border-cyan-100 px-2 py-1.5 text-[10px] dark:border-cyan-900/30"> {/* 单行任务。 */}
                ...
                <span className="truncate font-mono" title={job.workerId}>{job.workerId ?? DASH}</span> {/* Worker ID。 */}
                ...
                <span className="truncate font-mono" title={`schedule ${formatScheduledAt(job)} wait ${formatWait(job)}`}>{formatLockedAt(job)}</span> {/* 锁定时间。 */}
              </li> // 结束单行任务。
            )) // 结束任务遍历。
          )} {/* 结束条件渲染。 */}
        </ul> {/* 结束任务列表。 */}
      ...
      {latestJob ? ( // 判断是否有最新任务。
        <div className="mt-3 rounded-lg border border-cyan-200/70 bg-white/60 px-2 py-2 dark:border-cyan-900/40 dark:bg-zinc-950/25"> {/* 时间线卡片。 */}
          <h3 className="text-xs font-semibold text-cyan-950 dark:text-cyan-100">Worker Timeline | {latestJob.id}</h3> {/* 时间线标题。 */}
          <p className="mt-1 font-mono text-[10px] text-cyan-800 dark:text-cyan-200">duration: {formatDuration(latestJob)} | worker: {latestJob.workerId ?? DASH} | attempts: {latestJob.attempts}/{latestJob.maxAttempts}</p> {/* 最新任务摘要。 */}
