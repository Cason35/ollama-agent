...
import { QueueManager } from "@/lib/queue/queue-manager"; // 引入第34天队列管理器。
...
import { WorkerPool } from "@/lib/queue/worker-pool"; // 引入第34天 WorkerPool。

const NORMAL_PRIORITY = 5; // 定义普通优先级。
const DEFAULT_CONCURRENCY = 3; // 第34天：默认启动 3 个并发 Worker。
const STALE_LOCK_MS = 30 * 1000; // 第34天：定义 30 秒过期锁检测窗口。
...
function createJob(input: CreateJobInput): Job { // 定义任务对象创建函数。
  ...
  const items: JobTimelineItem[] = [timeline("Created", "用户请求已创建 Job，并进入 Queue Runtime V4 WorkerPool 队列")]; // 写入创建时间线。
...
export class QueueRuntime { // 定义第34天队列运行时。
  ...
  async requeue(jobId: string): Promise<JobStoreSnapshot & { requeued: Job }> { // 定义手动重新入队方法。
    ...
    const requeued: Job = { // 构造重新入队任务。
      ...
      updatedAt: now, // 写入更新时间。
      timeline: [...current.timeline, timeline("Requeued", "用户从死信队列手动重新入队，尝试次数已重置并立即重新调度")], // 追加重新入队节点。
    }; // 结束 requeued 任务。
  ...
  async detectStaleJobs(now = Date.now()): Promise<Job[]> { // 第34天：检测并恢复过期任务锁。
    ...
    for (const job of staleJobs) { // 遍历所有过期锁任务。
      const next: Job = { // 构造恢复任务。
        ...job, // 继承原任务字段。
        status: "retrying", // 恢复为等待重试，避免马上无限重复。
        workerId: undefined, // 清空 Worker 归属。
  ...
  async snapshot(): Promise<JobStoreSnapshot> { // 定义读取队列快照的方法。
    ...
    return { jobs, metrics: calculateQueueMetrics(jobs), workerPool: this.workerPool.getStats(jobs) }; // 返回任务、队列指标和 WorkerPool 指标。
  } // 结束 snapshot。
} // 结束 QueueRuntime。

const globalForQueue = globalThis as typeof globalThis & { __day34QueueRuntime?: QueueRuntime }; // 扩展 globalThis 保存第34天单例。
export function getQueueRuntime(): QueueRuntime { // 定义获取运行时单例的方法。
  if (!globalForQueue.__day34QueueRuntime) { // 判断是否已有第34天单例。
    globalForQueue.__day34QueueRuntime = new QueueRuntime(); // 没有则创建新的运行时。
  } // 结束单例判断。
  return globalForQueue.__day34QueueRuntime; // 返回运行时单例。
} // 结束 getQueueRuntime。
