...
import { rewriteQueryWithFallback } from "@/lib/query-rewrite"; // 第27天：Query Rewrite 工具逻辑
import {
  DEFAULT_MIN_SCORE,
  DEFAULT_RECALL_K,
...
/** 第27天：queryRewrite — 把用户问题改写成多个知识库检索 query。 */
const queryRewriteTool: Tool = {
  name: "queryRewrite", // 工具名
  description: "将用户问题改写成多个用于知识库检索的查询表达（LLM 优先，规则兜底）", // 工具描述
  capabilities: ["query-rewrite", "retrieval-optimization"], // 能力标签
  inputSchema: { input: { type: "string", required: true, description: "待改写的用户问题" } }, // 入参 schema
  ...
  async execute(input) {
    ...
    return rewriteQueryWithFallback(query, input.rt, 5); // LLM 优先，失败回退规则版
  },
};

/** 第27天：retrieval — Query Rewrite + Multi-Query + RecallK + TopK + minScore + Rerank。 */
const retrievalTool: Tool = {
  name: "retrieval", // 工具名
  description: "从知识库检索与问题最相关的文本块（Query Rewrite + Multi-Query + Rerank）", // 描述
  capabilities: ["knowledge-retrieval", "multi-query-retrieval"], // 能力：知识检索
  ...
  async execute(input) {
    ...
    const recallK = DEFAULT_RECALL_K; // 默认召回数量
    const topK = DEFAULT_RETRIEVAL_TOP_K; // 默认 TopK
    const minScore = DEFAULT_MIN_SCORE; // 默认相似度阈值
    const mode = DEFAULT_RETRIEVAL_MODE; // 默认混合检索模式
    const hits = await knowledgeStore.search(query, { recallK, topK, minScore, mode }); // Multi-Query 检索 + 过滤
    const rewrite = knowledgeStore.getLastRetrieval()?.rewrite; // 读取最近一次 rewrite 调试信息
    return { query, hits, recallK, topK, minScore, mode, rewrite }; // 返回可观测结构
  },
};

/** 第27天：ragAnswer — Multi-Query 检索 + 低幻觉 Prompt + 无结果 fallback。 */
const ragAnswerTool: Tool = {
  name: "ragAnswer", // 工具名
  description: "基于知识库 RAG V4：Query Rewrite、Multi-Query Retrieval、Rerank、无合格片段时不硬编", // 描述
  capabilities: ["knowledge-answer", "knowledge-retrieval", "multi-query-retrieval"], // 检索 + 问答
