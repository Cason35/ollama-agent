...
export function normalizeWorkflowAction(raw: unknown): WorkflowStepAction {
  ...
  if (raw === "research") return "research"; // 第23天：组合工具
  if (raw === "note") return "note"; // 第23天：笔记
  if (raw === "searchHistory" || raw === "search_history") return "searchHistory"; // 历史搜索
  if (raw === "generatePlan" || raw === "generate_plan") return "generatePlan"; // 学习计划
  if (raw === "critic") return "critic"; // 自评
  if (raw === "queryRewrite" || raw === "query_rewrite") return "queryRewrite"; // 第27天：查询改写
  if (raw === "retrieval") return "retrieval"; // 第24天：知识检索
  if (raw === "ragAnswer" || raw === "rag_answer" || raw === "rag") return "ragAnswer"; // 第24天：RAG 问答
  return "chat"; // 默认走普通对话
}
...
function parsePlannerPlanOutput(modelOutput: string): PlannerPlanItem[] {
  try {
    ...
    for (const row of parsed) {
      ...
      const capRaw = (row as { capabilities?: unknown }).capabilities; // 第23天：能力列表
      const capabilities = Array.isArray(capRaw)
        ? capRaw.filter((c): c is string => typeof c === "string" && c.trim().length > 0).map((c) => c.trim())
        : []; // 规范化能力数组
      const actionRaw = (row as { action?: unknown }).action; // 显式 action
      const mappedFromCaps =
        capabilities.length > 0 ? resolveActionFromCapabilities(capabilities) : undefined; // 能力路由
      const action = normalizeWorkflowAction(
        actionRaw != null && String(actionRaw).trim() !== ""
          ? actionRaw
          : mappedFromCaps ?? "chat"
      ); // 优先 action，否则 capability 映射，最后 chat
...
export async function planWorkflowSteps(
  ...
  const capabilityPrompt = formatCapabilitiesForPlanner(workflowToolRegistry); // 第23天：Capability Routing
  const plannerPrompt = `
你是一个 Workflow Planner。

请把用户需求拆解成 2-5 个可执行步骤（若需求很简单则只返回 1 个步骤）。

第23天 Capability Routing（Tool ≠ Capability）：
- 规划时先思考需要哪些「能力」，再选择 action（工具名）；二者可二选一写在 JSON 里。
- 可只写 capabilities 数组（字符串），服务端会映射为 action；也可直接写 action。
- 需要「总结 + 待办」时优先 capabilities: ["text-summary","task-generation"] 或 action: "research"。
...
research 补充说明：组合执行 summary 再 todo，适合「研究某主题并列出行动项」
note / searchHistory / generatePlan / critic：见工具 description
...
要求：
1. 只返回 JSON 数组
2. 每个步骤必须包含：id（稳定唯一）、name、input（字符串）；以及 action 或 capabilities 至少其一；天气步骤写城市名短句；不要嵌套 JSON 对象充当 input
