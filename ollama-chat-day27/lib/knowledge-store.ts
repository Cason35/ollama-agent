/**
 * 第27天：本地知识库 Store — overlap 导入 / Multi-Query Retrieval / Query Rewrite Metrics。
 */
...
/**
 * 第27天：Multi-Query 检索并更新 RAG metrics / Query Rewrite metrics / lastRetrieval。
 * @param query 查询文本
...
export async function searchKnowledge(
  ...
  const result = await multiQueryRetrieve(query, docs, { ...options, recallK, topK, minScore, mode }); // 第27天：改写 + 多路召回
  const hits = result.hits; // 取最终命中列表
  ...
  queryRewriteMetrics.generatedQueriesSum += result.rewrite.rewriteCount; // 累加生成 query 数
  ...
  lastRetrieval = { query, recallK, topK, minScore, mode, rewrite: result.rewrite, hits, at: Date.now() }; // 记录最近检索供 UI
