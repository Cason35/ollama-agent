...
import { buildSamplePromptVariables, promptVariableContracts, validatePromptTemplate } from "@/lib/prompts/prompt-contracts"; /* 第53天增强：引入统一样例变量、变量契约和模板校验能力。 */
...
function buildRenderedPreview(): string { /* 第52天：定义提示词渲染预览构造函数。 */
  ...
  return renderPrompt(activeResearch, buildSamplePromptVariables(activeResearch)); /* 第53天增强：使用统一样例变量渲染预览，避免新增 agentId 等变量后快照刷新失败。 */
} /* 第52天：结束提示词渲染预览构造函数。 */
