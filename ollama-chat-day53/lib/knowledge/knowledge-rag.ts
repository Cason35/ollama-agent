...
`.trim(); // 去掉首尾空白
  const promptTemplate = promptRegistry.getActive("ragAnswer"); // 第52天：读取 ragAnswer 工具当前 active 提示词模板
  return safeRenderPrompt(promptTemplate, { task: question.trim(), workspace: `${chunksBlock}\n\n参考片段：${usedChunkIds || "无"}` }, fallbackPrompt); // 第52天：使用注册表提示词渲染 RAG Prompt，失败时回退旧模板
}
