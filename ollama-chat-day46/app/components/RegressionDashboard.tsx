...
export function RegressionDashboard() { /* 第46天：定义持续评估回归看板。 */
  ...
  const loadSnapshot = useCallback(async (force: boolean) => { /* 第46天：定义读取或重新运行回归评估的函数。 */
    ...
    try { /* 第46天：捕获接口加载异常。 */
      ...
      setError(loadError instanceof ApiClientError ? loadError.message : "加载 Day 46 回归评估失败"); /* 第46天：展示统一错误文本。 */
  ...
  return ( /* 第46天：返回回归评估看板视图。 */
    <section className="shrink-0 border-b border-cyan-200/70 px-4 py-3 dark:border-cyan-900/40"> {/* 第46天：定义回归看板外层容器。 */}
      ...
      {!snapshot ? <p className="mt-3 rounded-lg border border-dashed border-cyan-200 py-5 text-center text-[11px] text-cyan-700 dark:border-cyan-900/50 dark:text-cyan-300">正在准备 Day 46 固定数据集与回归报告...</p> : null} {/* 第46天：快照未就绪时展示加载占位。 */}
