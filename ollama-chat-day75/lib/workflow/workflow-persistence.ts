...
export function loadStorageModePreference(): "local" | "backend" {
  ...
  if (raw === "backend") window.localStorage.setItem("workflow:storageMode", "local"); // 第37天：避免刷新首屏自动连接未配置的 MySQL backend。
  return "local"; // 第37天：默认始终用 local，backend 仍可由用户手动切换。
