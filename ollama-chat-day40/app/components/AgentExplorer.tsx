  ...
  useEffect(() => { /* 在组件挂载后加载智能体注册表与协作快照。 */
    ...
    (async () => { /* 定义并立即执行异步加载函数。 */
      try { /* 捕获接口加载异常。 */
        ...
        if (!cancelled) setError(err instanceof ApiClientError ? err.message : "加载 Day 40 Supervisor Runtime 失败"); /* 写入用户可读错误。 */
      } /* 结束 catch。 */
    })(); /* 立即执行异步函数。 */
    return () => { /* 返回清理函数。 */
  ...
  return ( /* 返回 Supervisor Runtime Dashboard 视图。 */
    <div className="shrink-0 border-b border-fuchsia-200/70 px-4 py-3 dark:border-fuchsia-900/40"> {/* 定义第40天模块容器。 */}
      <div className="flex items-start justify-between gap-3"> {/* 定义标题与徽标的横向布局。 */}
        <div> {/* 定义标题文本容器。 */}
          <h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">Supervisor Runtime Dashboard</h2> {/* 展示模块标题。 */}
          <p className="mt-1 text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">第40天：Supervisor Agent 先分析目标，再动态选择 Research、Planner、Critic、Writer 执行协作。</p> {/* 展示模块说明。 */}
        </div> {/* 结束标题文本容器。 */}
        <span className="shrink-0 rounded-full bg-fuchsia-500/15 px-2.5 py-1 text-[10px] font-semibold text-fuchsia-800 ring-1 ring-fuchsia-500/25 dark:text-fuchsia-200">Day 40</span> {/* 展示第40天标记。 */}
      </div> {/* 结束标题与徽标布局。 */}
      ...
      {payload ? ( /* 判断是否已经加载到智能体数据。 */
        <> {/* 使用片段包裹已加载内容。 */}
          ...
          <AgentCallGraph edges={payload.collaboration.callGraph} /> {/* 展示第40天智能体调用图。 */}
          <AgentTimeline events={payload.collaboration.timeline} /> {/* 展示第40天智能体计划时间线。 */}
        ...
        <p className="mt-3 rounded-lg border border-dashed border-zinc-200 px-2 py-3 text-center text-[11px] text-zinc-400 dark:border-zinc-700">Day 40 Supervisor Runtime 加载中...</p> /* 展示加载占位。 */
      )} {/* 结束加载状态判断。 */}
