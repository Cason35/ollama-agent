...
export function QueueDashboard({ // 导出队列看板组件。
  ...
  return ( // 返回队列看板 UI。
    <div className="shrink-0 border-b border-cyan-200/70 px-4 py-3 dark:border-cyan-900/40"> {/* 队列看板容器。 */}
      <h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">Queue Runtime V7</h2> {/* 看板标题。 */}
      <p className="mt-1 text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">第40天：继续保留 Workflow as Job 基础设施，供 Supervisor Multi-Agent Runtime 复用 Queue、Worker、Retry、Cancel 和 Unified Timeline。</p> {/* 看板说明。 */}
      ...
      <div className="mt-2 grid grid-cols-2 gap-2"> {/* 创建任务按钮区域。 */}
        ...
        <button type="button" disabled={queueLoading} onClick={createManualWorkflowJob} className={createJobButtonClass}>Workflow Job P8</button> {/* 创建 workflow 任务。 */}
      ...
      <div className="mt-3 overflow-hidden rounded-lg border border-cyan-200/70 dark:border-cyan-900/40"> {/* 任务表格外框。 */}
        ...
        <ul className="max-h-44 overflow-y-auto"> {/* 任务列表。 */}
          {jobs.length === 0 ? ( // 判断是否暂无任务。
            <li className="px-2 py-3 text-center text-[11px] text-zinc-400">暂无任务；点击上方按钮创建 Day 40 继承队列 Job。</li> // 空状态。
          ) : ( // 有任务时渲染列表。
            jobs.map((job) => ( // 遍历任务。
      ...
      {restartableJobs.length ? ( // 判断是否存在可重启任务。
        <div className="mt-3 rounded-lg border border-red-200/80 bg-red-50/60 px-2 py-2 dark:border-red-900/50 dark:bg-red-950/20"> {/* 死信任务区域。 */}
          <h3 className="text-xs font-semibold text-red-950 dark:text-red-100">Restartable Jobs</h3> {/* 第37天：可重启任务标题。 */}
              ...
              <li key={job.id} className="rounded border border-red-200/80 bg-white/70 px-2 py-1.5 text-[10px] dark:border-red-900/50 dark:bg-zinc-950/30"> {/* 单个死信任务。 */}
                <div className="flex items-center justify-between gap-2"> {/* 死信任务头部。 */}
                  ...
                  <div className="flex gap-1"> {/* 操作按钮组。 */}
                    {(job.status === "dead_letter" || job.status === "timeout") ? <button type="button" disabled={queueLoading} onClick={() => void handleRequeueQueueJob(job.id)} className={requeueButtonClass}>Requeue</button> : null} {/* 原地重新入队按钮。 */}
                    <button type="button" disabled={queueLoading} onClick={() => void handleRestartQueueJob(job.id)} className={requeueButtonClass}>Restart</button> {/* 第37天：克隆为新 Job 的重启按钮。 */}
                  </div> {/* 结束操作按钮组。 */}
                </div> {/* 结束死信任务头部。 */}
                <p className="mt-1 break-words text-red-800 dark:text-red-200">type: {job.type} | workflow: {job.workflowId ?? DASH} | resource: {job.resourceType ?? DASH} | attempts: {job.attempts}/{job.maxAttempts}</p> {/* 可重启任务摘要。 */}
      ...
      {latestJob ? ( // 判断是否有最新任务。
        <div className="mt-3 rounded-lg border border-cyan-200/70 bg-white/60 px-2 py-2 dark:border-cyan-900/40 dark:bg-zinc-950/25"> {/* 时间线卡片。 */}
          <h3 className="text-xs font-semibold text-cyan-950 dark:text-cyan-100">Unified V7 Timeline | {latestJob.id}</h3> {/* 时间线标题。 */}
