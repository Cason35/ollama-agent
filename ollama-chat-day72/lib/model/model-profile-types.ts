...
export type ModelRole = "reasoning" | "writing" | "evaluation" | "json" | "embedding" | "summary"; /* 第56天：定义模型在多模型协作中可承担的角色集合。 */
...
export type ModelProfile = { /* 第51天：定义 ModelProfile（模型档案），描述一个可路由且可降级的逻辑模型。 */
  ...
  roles: ModelRole[]; /* 第56天：保存模型在协作团队中可承担的角色列表，同一模型可以承担多个角色。 */
...
export type ModelTaskType = "chat" | "summary" | "research" | "planning" | "reflection" | "evaluation" | "embedding" | "json"; /* 第56天：在原有任务类型基础上补充 research（研究）任务，用于触发多模型协作计划。 */
