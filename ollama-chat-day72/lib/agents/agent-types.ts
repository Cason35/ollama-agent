...
import type { StorageObjectMetadata } from "@/lib/storage/storage-types"; /* 第61天：引入对象存储元数据，用于 Trace Attachment（追踪附件）只保存对象引用。 */
...
export type TraceAttachment = { /* 第61天：定义 Trace Attachment（追踪附件）引用结构。 */
  id: string; /* 第61天：保存附件唯一标识。 */
  name: string; /* 第61天：保存附件名称，例如 prompt-snapshot 或 output-artifact。 */
  storage: StorageObjectMetadata; /* 第61天：保存附件在 Object Storage（对象存储）里的元数据引用。 */
  createdAt: number; /* 第61天：保存附件创建时间戳。 */
}; /* 第61天：结束 TraceAttachment 类型定义。 */
...
export type Trace = { /* 第44天：定义一次完整用户请求的 Trace（追踪记录）。 */
  ...
  attachments?: TraceAttachment[]; /* 第61天：保存 Trace Attachment（追踪附件）的对象存储引用列表。 */
  evaluation?: import("@/lib/evaluation/evaluation-platform-types").EvaluationTraceLink; /* 第71天：自动关联 EvaluationRun、多维评分和评估器版本质量链路。 */
...
export type WorkspaceExportReference = { /* 第61天：定义工作区导出文件对象存储引用。 */
  workspaceId: string; /* 第61天：保存被导出的工作区 ID。 */
  markdownObject: StorageObjectMetadata; /* 第61天：保存 Markdown 导出文件的对象存储元数据。 */
  signedUrl: string; /* 第61天：保存可下载签名链接或本地访问链接。 */
}; /* 第61天：结束 WorkspaceExportReference 类型定义。 */
...
export type AgentCollaborationSnapshot = { /* 定义第40天 Supervisor 计划运行快照结构。 */
  ...
  workspaceExport?: WorkspaceExportReference; /* 第61天：保存 Workspace Export（工作区导出）上传到 Object Storage 后的引用。 */
