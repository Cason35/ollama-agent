...
import type { Job, JobType, QueueMetrics } from "@/lib/queue/queue-types"; // 第31天：Queue Runtime 任务与指标类型
...
export default function HomePage() {
  ...
  const [queueJobs, setQueueJobs] = useState<Job[]>([]); // 第31天：Queue Dashboard 任务列表
  const [queueMetrics, setQueueMetrics] = useState<QueueMetrics | null>(null); // 第31天：Queue Metrics 指标
  const [queueLoading, setQueueLoading] = useState(false); // 第31天：队列 API 加载状态
  ...
  /** 第31天：加载队列快照（JobStore + Queue Metrics + Job Timeline）。 */
  ...
  useEffect(() => {
    void refreshQueuePanel(); // 挂载后立即拉取一次队列状态
    const timer = window.setInterval(() => {
      void refreshQueuePanel(); // 每秒刷新任务状态与指标
    }, 1000); // 轮询间隔 1 秒
    return () => window.clearInterval(timer); // 卸载时清理轮询
  }, []); // 仅挂载一次

  /** 第31天：创建后台 Job 并交给 Worker 异步执行。 */
