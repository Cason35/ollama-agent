...
import { useEffect, useState } from "react"; // 第64天：引入状态与生命周期 Hook。
...
export function RuntimeContextExplorer() { // 第64天：定义统一运行时上下文浏览器。
  ...
  const refresh = async () => { setLoading(true); const response = await fetch("/api/runtime/context", { cache: "no-store" }); setSnapshot(await response.json()); setLoading(false); }; // 第64天：请求新的统一上下文研究任务快照。
  useEffect(() => { void refresh(); }, []); // 第64天：面板首次打开时自动执行完整链路。
