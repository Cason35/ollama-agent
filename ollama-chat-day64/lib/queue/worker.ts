...
import type { LockProvider, LockToken } from "@/lib/lock/lock-types"; // 第60天：引入锁提供者和锁令牌类型。

const DEFAULT_TIMEOUT_MS = 30 * 1000; // 第36天：定义 Worker 侧默认超时时间。
const CANCEL_CHECK_INTERVAL_MS = 200; // 第36天：定义长任务协作取消检查间隔。
const DEFAULT_LOCK_TTL_MS = 30 * 1000; // 第60天：定义 Worker 执行业务资源锁的默认 TTL。
const LOCK_RENEW_INTERVAL_MS = 10 * 1000; // 第60天：定义 Worker 执行长任务时的锁续期间隔。
const LOCK_BUSY_RETRY_DELAY_MS = 1000; // 第60天：定义获取不到锁时重新入队的短延迟。
...
async function executeJob(job: Job, ctx: JobExecutionContext): Promise<Record<string, unknown>> { // 第36天：定义支持协作取消的任务执行器。
  if (job.type === "workflow") { // 第37天：判断是否为 Workflow as Job 任务。
    ...
    const { rt, errorResponse } = await buildModelRuntime(job.payload.provider, job.payload.mimoModel); // 第63天：异步恢复模型运行时，并通过 SecretsManager 读取敏感密钥。
...
export class Worker { // 定义第35天本地 Worker。
  ...
  private lastQueueErrorAt = 0; // 第59天：记录最近一次 Redis Queue 认领错误日志时间，避免 Redis 未启动时刷屏。
  ...
  constructor( // 定义 Worker 构造函数。
    ...
    private readonly onJobFinished?: (job: Job) => void, // 第60天：注入任务完成回调用于统计吞吐。
    private readonly lockProvider: LockProvider = getLockRuntime() // 第60天：注入分布式锁提供者，默认使用 RedisLockProvider。
  ) { // 构造函数主体开始。
  ...
  private async tick(): Promise<void> { // 定义轮询执行方法。
    ...
    let job: Job | undefined; // 第59天：准备保存 Redis Queue 异步认领结果。
    try { // 第59天：捕获 Redis 未启动或队列命令失败，避免 Worker 轮询产生未处理异常。
      job = await this.queue.claimNextJob(this.id); // 第59天：从 Redis Queue 异步认领一个满足限制条件的任务。
    } catch (error) { // 第59天：处理 Redis Queue 认领失败。
      const now = Date.now(); // 第59天：记录本次错误时间。
      if (process.env.REDIS_QUEUE_DEBUG === "1" && now - this.lastQueueErrorAt > 10_000) { // 第59天：仅在调试开关开启时输出队列错误，并限制频率。
        console.warn("[RedisQueue] claim failed", error instanceof Error ? error.message : String(error)); // 第59天：写入简短警告，页面仍可继续展示健康检查失败。
        this.lastQueueErrorAt = now; // 第59天：更新时间戳，限制后续日志频率。
      } // 第59天：结束日志限频判断。
      return; // 第59天：本轮跳过，等待下一次轮询。
    } // 第59天：结束 Redis Queue 认领异常处理。
    ...
    try { // 开始执行任务。
      this.currentRun = this.processWithDistributedLock(job).then((finished) => { // 第60天：保存带分布式锁保护的当前任务 Promise 供优雅关闭等待。
  ...
  private resolveJobLockKey(job: Job): string { // 第60天：根据任务载荷解析要保护的分布式锁 Key。
    const resourceKey = typeof job.payload.resourceKey === "string" ? job.payload.resourceKey : undefined; // 第60天：优先读取显式业务资源 Key。
    if (resourceKey) return `resource:${resourceKey}`; // 第60天：显式资源 Key 存在时保护同一业务资源。
    const workflowId = job.workflowId ?? (typeof job.payload.workflowId === "string" ? job.payload.workflowId : undefined); // 第60天：读取 WorkflowJob 关联的工作流 ID。
    if (job.type === "workflow" && workflowId) return `workflow:${workflowId}`; // 第60天：Workflow Job 使用工作流级别锁，避免同一工作流并发执行。
    return `job:${job.id}`; // 第60天：普通任务默认退回 Job 级别锁。
  } // 第60天：结束任务锁 Key 解析方法。
  private createLockOwner(job: Job): string { // 第60天：生成锁 owner 标识。
    return `${this.id}:${job.id}:${Date.now()}`; // 第60天：组合 Worker、Job 和时间戳，避免同一 Worker 旧令牌误释放新锁。
  } // 第60天：结束锁 owner 生成方法。
  private async requeueBecauseLockBusy(claimedJob: Job, lockKey: string): Promise<Job> { // 第60天：处理获取不到锁时的重新入队。
    const now = Date.now(); // 第60天：获取当前时间戳。
    const retrying: Job = { // 第60天：构造因锁繁忙而等待重试的任务。
      ...claimedJob, // 第60天：继承已认领任务字段。
      status: "retrying", // 第60天：把状态改为等待重试。
      error: `Distributed Lock ${lockKey} 正被其他 Worker 持有`, // 第60天：写入锁竞争原因。
      workerId: undefined, // 第60天：释放 Worker 归属。
      lockedAt: undefined, // 第60天：释放队列可见性锁时间。
      nextRunAt: now + LOCK_BUSY_RETRY_DELAY_MS, // 第60天：短暂延迟后再尝试获取锁。
      updatedAt: now, // 第60天：更新时间戳。
      timeline: [...claimedJob.timeline, timeline("Blocked", `Worker ${this.id} 未获得 Distributed Lock ${lockKey}，任务等待锁释放后重试`)], // 第60天：写入锁竞争时间线。
    }; // 第60天：结束锁繁忙重试任务构造。
    await this.store.update(retrying); // 第60天：保存锁繁忙重试状态。
    await this.queue.retryJob(retrying).catch((queueError) => console.warn("[RedisLock] retry after lock busy failed", queueError)); // 第60天：把任务放回 Redis Waiting Queue。
    return retrying; // 第60天：返回重试任务。
  } // 第60天：结束锁繁忙重新入队方法。
  async processWithDistributedLock(claimedJob: Job): Promise<Job> { // 第60天：定义带分布式锁保护的任务处理入口。
    const lockKey = this.resolveJobLockKey(claimedJob); // 第60天：解析当前任务要保护的锁 Key。
    const owner = this.createLockOwner(claimedJob); // 第60天：生成当前锁持有者标识。
    const token = await this.lockProvider.acquire({ key: lockKey, owner, ttlMs: DEFAULT_LOCK_TTL_MS }).catch(() => null); // 第60天：尝试获取 Redis Distributed Lock。
    if (!token) return await this.requeueBecauseLockBusy(claimedJob, lockKey); // 第60天：获取失败时不执行任务，直接延迟重试。
    let activeToken: LockToken = token; // 第60天：保存当前有效锁令牌，续期后会更新。
    let renewFailed = false; // 第60天：记录续期是否曾失败，便于排查长任务风险。
    const renewalTimer = setInterval(() => { // 第60天：启动锁心跳续期计时器。
      void this.lockProvider.extend(activeToken, { ttlMs: DEFAULT_LOCK_TTL_MS }).then((next) => { // 第60天：异步续期当前锁令牌。
        if (next) activeToken = next; // 第60天：续期成功时替换为新令牌。
        if (!next) renewFailed = true; // 第60天：续期失败时标记风险。
      }).catch(() => { renewFailed = true; }); // 第60天：续期异常时标记风险但不打断当前 Promise。
    }, LOCK_RENEW_INTERVAL_MS); // 第60天：按固定心跳间隔续期。
    const lockedJob: Job = { ...claimedJob, timeline: [...claimedJob.timeline, timeline("Claimed", `Worker ${this.id} 已获得 Distributed Lock ${activeToken.key}，Workflow Runtime 或 Job Runtime 可安全执行`)] }; // 第60天：把锁获取事件写入任务时间线。
    try { // 第60天：开始执行被锁保护的真实任务。
      const finished = await this.process(lockedJob); // 第60天：复用原有任务处理逻辑。
      if (renewFailed) console.warn("[RedisLock] lock renewal failed during job", { jobId: claimedJob.id, lockKey }); // 第60天：如果续期失败过则输出警告。
      return finished; // 第60天：返回真实任务处理结果。
    } finally { // 第60天：无论任务成功、失败、取消或超时都清理锁。
      clearInterval(renewalTimer); // 第60天：停止锁续期心跳。
      await this.lockProvider.release(activeToken).catch((lockError) => console.warn("[RedisLock] release failed", lockError)); // 第60天：校验 owner 后释放 Redis Distributed Lock。
    } // 第60天：结束锁清理逻辑。
  } // 第60天：结束带分布式锁保护的任务处理入口。
  async process(claimedJob: Job): Promise<Job> { // 定义处理单个已认领任务的方法。
    ...
    try { // 开始调用任务执行器。
      ...
      await this.queue.ackJob(success).catch((queueError) => console.warn("[RedisQueue] ack failed", queueError)); // 第59天：ACK 成功任务，Redis 归档失败时只记录警告。
      ...
      if (error instanceof JobCancelledError) { // 第36天：取消错误不进入 retry。
        ...
        await this.queue.failJob(cancelled).catch((queueError) => console.warn("[RedisQueue] cancel archive failed", queueError)); // 第59天：取消终态任务归档失败时只记录警告。
      ...
      if (error instanceof JobTimeoutError) { // 第36天：超时错误按可重试错误处理。
        ...
        if (attemptNumber < running.maxAttempts) { // 判断超时后是否还允许重试。
          ...
          await this.queue.retryJob(retrying).catch((queueError) => console.warn("[RedisQueue] timeout retry failed", queueError)); // 第59天：超时重试回队失败时只记录警告。
        ...
        await this.queue.failJob(timedOut).catch((queueError) => console.warn("[RedisQueue] timeout archive failed", queueError)); // 第59天：最终超时归档失败时只记录警告。
      ...
      if (attemptNumber < running.maxAttempts) { // 判断是否还允许重试。
        ...
        await this.store.update(retrying); // 保存等待重试状态。
        await this.queue.retryJob(retrying).catch((queueError) => console.warn("[RedisQueue] retry failed", queueError)); // 第59天：重试回队失败时只记录警告。
        return retrying; // 返回等待重试任务。
      } // 结束重试判断。
      ...
      await this.queue.failJob(deadLetter).catch((queueError) => console.warn("[RedisQueue] dead letter archive failed", queueError)); // 第59天：死信归档失败时只记录警告。
