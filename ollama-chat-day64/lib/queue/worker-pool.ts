...
import type { LockProvider } from "@/lib/lock/lock-types"; // 第60天：引入分布式锁提供者类型，供 WorkerPool 注入 Worker。
import type { QueueManager } from "@/lib/queue/queue-manager"; // 引入队列管理器类型。
import { Worker } from "@/lib/queue/worker"; // 引入单个 Worker 实现。

const DEFAULT_GRACE_PERIOD_MS = 10 * 1000; // 第36天：定义默认优雅关闭宽限期为 10 秒。
...
export class WorkerPool { // 第35天：沿用 WorkerPool 工作池，并配合资源限制器控制任务认领。
  ...
  constructor( // 定义 WorkerPool 构造函数。
    ...
    private readonly retryPolicy: RetryPolicy, // 第60天：注入共享重试策略。
    private readonly lockProvider?: LockProvider // 第60天：注入共享分布式锁提供者。
  ) { // 构造函数主体开始。
    this.workers = Array.from({ length: concurrency }, (_, index) => new Worker(`worker_${index + 1}`, queue, store, retryPolicy, (job) => this.recordFinishedJob(job), lockProvider)); // 按并发数创建 Worker。
  } // 结束构造函数。
  ...
  async stopGracefully(options: { gracePeriodMs?: number } = {}): Promise<void> { // 第36天：定义优雅关闭工作池方法。
    const gracePeriodMs = options.gracePeriodMs ?? DEFAULT_GRACE_PERIOD_MS; // 读取宽限期配置。
    const shutdownAt = Date.now(); // 记录优雅关闭开始时间。
    this.workers.forEach((worker) => worker.stopAccepting()); // 先让所有 Worker 停止认领新任务。
    const waitAllWorkers = Promise.all(this.workers.map((worker) => worker.waitUntilIdle())); // 创建等待所有运行任务结束的 Promise。
    await Promise.race([waitAllWorkers, new Promise((resolve) => setTimeout(resolve, gracePeriodMs))]); // 等待任务自然结束或宽限期耗尽。
    const jobs = await this.store.list(); // 从 Store 读取最新任务状态。
    const stillRunning = jobs.filter((job) => job.status === "running" || job.status === "cancelling"); // 找出宽限期后仍未结束的任务。
    for (const job of stillRunning) { // 遍历仍在运行的任务。
      const now = Date.now(); // 获取当前时间。
      const retrying: Job = { // 构造恢复为 retrying 的任务。
        ...job, // 继承原任务字段。
        status: "retrying", // 状态恢复为等待重试。
        error: "Graceful shutdown grace period exceeded", // 写入优雅关闭超时说明。
        workerId: undefined, // 清空 Worker 归属。
        lockedAt: undefined, // 清空任务锁。
        nextRunAt: now + 1000, // 一秒后允许重新认领。
        updatedAt: now, // 更新时间。
        timeline: [...job.timeline, { label: "GracefulShutdownStarted", at: shutdownAt, note: `WorkerPool 开始优雅关闭，宽限期 ${gracePeriodMs}ms` }, { label: "WorkerStopped", at: now, note: "宽限期结束后任务仍未完成，已恢复为 retrying 等待重新认领" }], // 记录优雅关闭生命周期事件。
      }; // 结束 retrying 任务。
      this.queue.releaseJobResource(job); // 释放该任务占用的资源额度。
      await this.store.update(retrying); // 保存恢复后的任务状态。
      await this.queue.retryJob(retrying); // 第59天：放回 Redis Waiting Queue 等待后续 Worker 处理。
    } // 结束仍在运行任务遍历。
    this.workers.forEach((worker) => worker.stop()); // 最后彻底停止全部 Worker 计时器。
  } // 结束 stopGracefully。
