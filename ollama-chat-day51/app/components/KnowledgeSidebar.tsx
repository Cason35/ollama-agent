...
export function KnowledgeSidebar({
  ...
  const [activePanel, setActivePanel] = useState<"runtime" | "model" | "memory" | "cache" | "usage" | "knowledge" | "records">("model"); /* 第51天：默认打开模型健康面板，突出 fallback 与熔断器能力。 */
      ...
      <div className="z-10 shrink-0 border-b border-zinc-200/80 bg-white px-3 py-3 dark:border-zinc-800 dark:bg-zinc-900">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-0.5 text-[11px] text-zinc-500 dark:text-zinc-400">按主题查看模型健康、运行、用量、知识与历史记录。</p> {/* 第51天：更新控制台标签页说明，突出模型健康。 */}
          </div>
          <span className="rounded-full bg-emerald-500/15 px-2.5 py-1 text-[10px] font-semibold text-emerald-800 ring-1 ring-emerald-500/20 dark:text-emerald-200">
            Day 51 {/* 第51天：更新控制台日期徽标。 */}
          </span>
        </div>
        <div className="mt-3 grid grid-cols-7 gap-1" role="tablist" aria-label="Day 51 侧边栏面板"> {/* 第51天：扩展为包含模型健康与熔断分析的七个标签页。 */}
