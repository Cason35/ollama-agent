...
export class AgentRuntime { /* 定义第40天基于 Supervisor 的智能体运行时。 */
  ...
  constructor(private readonly registry: AgentRegistry, private readonly workspaceStore: WorkspaceStore = new MemoryWorkspaceStore(), private readonly usageStore: UsageManager = usageManager, private readonly cacheStore: SemanticCache = semanticCache, private readonly memoryStore: LongTermMemoryStore = longTermMemory, private readonly modelRouterStore: ModelRouter = modelRouter) {} /* 第50天：在第49天依赖注入基础上追加共享模型路由器，让运行时为不同阶段按任务选择不同模型。 */
  ...
  async executeAgent(agentId: string, task: AgentTask, context?: AgentContext, rt?: ModelRuntime): Promise<AgentResult> { /* 定义执行单个智能体任务的方法。 */
    ...
    try { /* 第44天：捕获 Agent 执行异常并写入 failed span。 */
      ...
      while (attempt <= this.maxReflectionRetries) { /* 第43天：执行生成、反思、必要时重试的闭环。 */
        ...
        this.recordComponentUsage(traceId, agentSpanId, "agent", agentId, `${agent.systemPrompt}\n${prompt}`, output, generationStartedAt); /* 第47天：记录本轮 Agent 输入、输出、费用和耗时并关联 Trace/Span。 */
        ...
        const reflectionUsage = this.recordComponentUsage(traceId, reflectionSpanId, "reflection", "reflection", `${assignedTask.goal}\n${output}`, JSON.stringify(finalReflection), reflectionStartedAt); /* 第47天：记录反思输入输出并把费用关联到反思 Span。 */
        this.traceManager.endSpan(traceId, reflectionSpanId, "success", { score: finalReflection.score, shouldRetry, issuesCount: finalReflection.issues.length, suggestionsCount: finalReflection.suggestions.length, totalTokens: reflectionUsage.totalTokens, estimatedCost: reflectionUsage.estimatedCost }); /* 第47天：结束反思跨度并写入质量判断与用量摘要。 */
      ...
      const evaluationUsage = this.recordComponentUsage(traceId, evaluationSpanId, "evaluation", "evaluation", `${assignedTask.goal}\n${output}`, JSON.stringify(finalEvaluation), evaluationStartedAt); /* 第47天：记录评估输入输出并把费用关联到评估 Span。 */
      this.traceManager.endSpan(traceId, evaluationSpanId, "success", { score: finalEvaluation.score, completeness: finalEvaluation.dimensions.completeness, correctness: finalEvaluation.dimensions.correctness, relevance: finalEvaluation.dimensions.relevance, coverage: finalEvaluation.dimensions.coverage, suggestionsCount: finalEvaluation.suggestions.length, totalTokens: evaluationUsage.totalTokens, estimatedCost: evaluationUsage.estimatedCost }); /* 第47天：结束评估跨度并写入分数与用量摘要。 */
      ...
      this.traceManager.endSpan(traceId, agentSpanId, "success", { duration, reflectionScore: finalReflection?.score, evaluationScore: finalEvaluation.score, totalTokens: agentUsage.totalTokens, estimatedCost: agentUsage.estimatedCost }); /* 第47天：结束 Agent Span 并写入质量分与用量成本摘要。 */
      if (standaloneTrace) this.closeActiveTrace(); /* 第44天：独立调用结束时关闭 Trace（追踪记录）。 */
  ...
  private recordComponentUsage(traceId: string, spanId: string, componentType: UsageComponentType, componentId: string, inputText: string, outputText: string, startedAt: number): UsageRecord { /* 第47天：定义各运行阶段共享的用量写入方法。 */
    const model = this.routeComponentModel(componentType, componentId); /* 第50天：先用 ModelRouter 为当前组件选择最合适的模型。 */
    return this.usageStore.addRecord({ traceId, spanId, componentType, componentId, inputTokens: estimateTokenCount(inputText), outputTokens: estimateTokenCount(outputText), durationMs: Math.max(1, Date.now() - startedAt), modelId: model.id, provider: model.provider, modelName: model.model }); /* 第50天：在第47天基础上把路由出的模型 id、提供方与模型名一并写入用量记录，实现成本归因。 */
  } /* 第47天：结束运行阶段用量写入方法。 */
  ...
  private async recordDeclaredToolSpans(traceId: string, parentSpanId: string, agentId: string, tools: string[], taskId: string, attempt: number): Promise<void> { /* 第44天：把 Agent 声明工具记录成可观测 Tool Span（工具跨度）。 */
    for (const toolName of tools) { /* 第44天：遍历当前 Agent 声明的工具名称。 */
      ...
      const toolSpanId = this.traceManager.startSpan(traceId, { parentSpanId, name: toolName, type: spanType, metadata: { agentId, taskId, attempt, simulated: true } }); /* 第44天：创建工具或检索跨度。 */
      ...
      const toolUsage = this.recordComponentUsage(traceId, toolSpanId, "tool", toolName, `${agentId}\n${taskId}\n${attempt}`, toolOutput, toolStartedAt); /* 第47天：把 Retrieval、Summary 等工具统一接入 Usage。 */
      this.traceManager.endSpan(traceId, toolSpanId, "success", { note: "当前演示运行时记录声明式工具链路，真实 ToolRegistry 调用也支持 Trace 接入。", totalTokens: toolUsage.totalTokens, estimatedCost: toolUsage.estimatedCost }); /* 第47天：结束工具跨度并附加词元与费用摘要。 */
    } /* 第44天：结束工具名称遍历。 */
  ...
  async evaluateOutput(task: AgentTask, output: string, agentId = task.assignedAgentId ?? "unknown", rt?: ModelRuntime): Promise<EvaluationResult> { /* 第45天：实现 evaluateOutput（评估输出）函数，输入任务和输出并返回评估结果。 */
    ...
    if (!rt || !evaluationAgent) return this.createRuleBasedEvaluation(task, output); /* 第45天：没有模型或评估智能体时使用规则型评估兜底。 */
    const prompt = this.buildEvaluationPrompt(task, output, agentId); /* 第45天：构造要求模型返回结构化 JSON 的评估提示词。 */
    const result = await invokeChatModel(rt, [{ role: "system", content: evaluationAgent.systemPrompt }, { role: "user", content: prompt }]); /* 第45天：调用 Evaluation Agent（评估智能体）量化业务 Agent 输出。 */
    const parsed = result.ok ? this.parseEvaluationFromText(result.text) : null; /* 第45天：尝试解析模型返回的 EvaluationResult（评估结果）。 */
    return parsed ?? this.createRuleBasedEvaluation(task, output); /* 第45天：解析失败时回退到规则型评估，保证本地演示稳定。 */
