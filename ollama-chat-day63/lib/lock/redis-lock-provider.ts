...
import { configManager } from "@/lib/config/config-runtime"; // 第62天：引入配置中心，统一读取分布式锁 TTL。
...
export class RedisLockProvider implements LockProvider { // 第60天：定义基于 Redis String 的分布式锁实现。 
  ...
  constructor(private readonly client: RedisClient = redisClient, private readonly defaultTtlMs = configManager.getNumber("redis.lockTtlMs", DEFAULT_LOCK_TTL_MS)) {} // 第62天：注入 RedisClient 并从配置中心读取默认 TTL。 
