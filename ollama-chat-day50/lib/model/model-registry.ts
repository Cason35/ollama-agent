...
export class ModelRegistry { /* 第50天：定义 ModelRegistry（模型注册表），负责登记与查询逻辑模型。 */
  ...
  register(model: ModelProfile): ModelProfile { /* 第50天：定义注册单个模型档案的方法。 */
    this.models.set(model.id, { ...model, capabilities: [...model.capabilities] }); /* 第50天：写入模型副本，避免外部引用被意外修改。 */
  ...
  get(id: string): ModelProfile | undefined { /* 第50天：定义按 id 读取模型档案的方法。 */
    ...
    return model ? { ...model, capabilities: [...model.capabilities] } : undefined; /* 第50天：命中时返回副本，未命中时返回 undefined。 */
  } /* 第50天：结束读取模型方法。 */

  list(): ModelProfile[] { /* 第50天：定义列出全部模型档案的方法。 */
    return Array.from(this.models.values()).map((model) => ({ ...model, capabilities: [...model.capabilities] })); /* 第50天：返回按注册顺序排列的模型副本数组。 */
  } /* 第50天：结束列出模型方法。 */
