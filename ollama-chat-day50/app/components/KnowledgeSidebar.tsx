...
export function KnowledgeSidebar({
  ...
  const [activePanel, setActivePanel] = useState<"runtime" | "model" | "memory" | "cache" | "usage" | "knowledge" | "records">("model"); /* 第50天：新增模型路由面板并默认打开，突出第50天 Model Router 能力。 */
  ...
  return (
    <aside className="flex min-h-0 w-full shrink-0 flex-col overflow-hidden rounded-lg border border-zinc-200/80 bg-white shadow-sm shadow-zinc-900/5 dark:border-zinc-800 dark:bg-zinc-900/80 dark:shadow-black/30 lg:h-full lg:w-[400px] xl:w-[430px]">
      <div className="z-10 shrink-0 border-b border-zinc-200/80 bg-white px-3 py-3 dark:border-zinc-800 dark:bg-zinc-900">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-0.5 text-[11px] text-zinc-500 dark:text-zinc-400">按主题查看运行、用量、知识与历史记录。</p> {/* 第47天：更新控制台标签页说明。 */}
          </div>
          <span className="rounded-full bg-emerald-500/15 px-2.5 py-1 text-[10px] font-semibold text-emerald-800 ring-1 ring-emerald-500/20 dark:text-emerald-200">
            Day 50 {/* 第50天：更新控制台日期徽标。 */}
          </span>
        </div>
        <div className="mt-3 grid grid-cols-7 gap-1" role="tablist" aria-label="Day 50 侧边栏面板"> {/* 第50天：扩展为包含模型路由分析的七个标签页。 */}
          <button type="button" className={tabClass("runtime")} onClick={() => setActivePanel("runtime")}>
            运行
          </button>
          <button type="button" className={tabClass("model")} onClick={() => setActivePanel("model")}> {/* 第50天：新增模型路由标签页入口。 */}
            模型 {/* 第50天：显示紧凑的模型路由标签名称。 */}
          </button>
      ...
      <div className={activePanel === "model" ? "contents" : "hidden"}> {/* 第50天：仅在模型标签页展示 Model Explorer。 */}
        <ModelExplorer /> {/* 第50天：展示模型档案、注册表指标与按任务路由预览。 */}
      </div> {/* 第50天：结束模型标签页内容。 */}
