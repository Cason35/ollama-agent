...
export function UsageExplorer() { /* 第47天：定义 Usage Explorer（用量浏览器）主组件。 */
  ...
  return ( /* 第47天：返回用量浏览器完整视图。 */
    <section className="shrink-0 border-b border-violet-200/70 px-4 py-3 dark:border-violet-900/40"> {/* 第47天：定义用量浏览器外层容器。 */}
      ...
      {snapshot && activeTab === "records" ? <div className="mt-3 overflow-x-auto rounded-lg border border-zinc-200 dark:border-zinc-800"><table className="w-full min-w-[620px] text-left text-[9px]"><thead className="bg-zinc-100 text-zinc-500 dark:bg-zinc-900"><tr><th className="px-2 py-1.5">Component</th><th className="px-2 py-1.5">Input</th><th className="px-2 py-1.5">Output</th><th className="px-2 py-1.5">Cost</th><th className="px-2 py-1.5">Duration</th><th className="px-2 py-1.5">Trace / Span</th></tr></thead><tbody>{snapshot.records.map((record, index) => <tr key={`${record.traceId}-${record.spanId}-${record.createdAt}-${index}`} className="border-t border-zinc-100 dark:border-zinc-800"><td className="px-2 py-1.5"><p className="font-semibold">{record.componentId}</p><p className="text-zinc-500">{componentLabels[record.componentType]}</p></td><td className="px-2 py-1.5 font-mono">{record.inputTokens}</td><td className="px-2 py-1.5 font-mono">{record.outputTokens}</td><td className="px-2 py-1.5 font-mono text-violet-700 dark:text-violet-300">{formatCost(record.estimatedCost)}</td><td className="px-2 py-1.5 font-mono">{formatDuration(record.durationMs)}</td><td className="max-w-48 px-2 py-1.5 font-mono text-[8px] text-zinc-500"><p className="truncate" title={record.traceId}>{record.traceId}</p><p className="truncate" title={record.spanId}>{record.spanId}</p></td></tr>)}</tbody></table></div> : null} {/* 第47天：展示每次调用的输入输出词元、费用、耗时和 Trace/Span 关联。 */}
