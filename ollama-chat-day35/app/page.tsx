...
export default function HomePage() {
  ...
  /** 第35天：创建后台 Job，并携带 priority / resourceType / scheduledAt 交给 V5 调度。 */
  const handleCreateQueueJob = async (input: CreateJobInput) => {
    ...
    try {
      const payloadByType: Record<JobType, Record<string, unknown>> = {
        chat: { prompt: "第35天 chat 任务，用于测试 llm 资源速率限制" }, // chat 任务载荷
        ...
        reminder: { text: "第35天 Reminder Job 已按 scheduledAt 自动触发" }, // reminder 定时提醒任务载荷
      }; // 任务载荷映射结束
