...
export function QueueDashboard({ // 导出队列看板组件。
  ...
  const createTenEmbeddingJobs = () => { // 第35天：定义 10 个 embedding 资源并发压测入口。
    Array.from({ length: 10 }, (_, index) => index + 1).forEach((index) => { // 循环创建 10 个 embedding 长任务。
      void handleCreateQueueJob({ type: "embedding", priority: 1, payload: { durationMs: 5000, document: `Day35 embedding 资源限制压测任务 ${index}` } }); // 创建 5 秒 embedding 任务。
    }); // 结束循环。
  }; // 结束 createTenEmbeddingJobs。
  const createFiveChatJobs = () => { // 第35天：定义 llm 速率限制压测入口。
    Array.from({ length: 5 }, (_, index) => index + 1).forEach((index) => { // 循环创建 5 个 chat 任务。
      void handleCreateQueueJob({ type: "chat", priority: 10, payload: { prompt: `Day35 llm rate limit burst ${index}` } }); // 创建 chat 任务观察每秒速率限制。
    }); // 结束循环。
  }; // 结束 createFiveChatJobs。
  return ( // 返回队列看板 UI。
    <div className="shrink-0 border-b border-cyan-200/70 px-4 py-3 dark:border-cyan-900/40"> {/* 队列看板容器。 */}
      <h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">Queue Runtime V5</h2> {/* 看板标题。 */}
      <p className="mt-1 text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">第35天：Rate Limit + Resource Control，在 WorkerPool 并发基础上限制 llm、embedding、database、workflow 与 tool 资源。</p> {/* 看板说明。 */}
      ...
      <div className="mt-3 overflow-hidden rounded-lg border border-cyan-200/70 dark:border-cyan-900/40"> {/* 任务表格外框。 */}
        <div className="grid grid-cols-[1fr_0.62fr_0.62fr_0.65fr_0.75fr_0.75fr] bg-cyan-50/80 px-2 py-1 font-mono text-[9px] text-cyan-900 dark:bg-cyan-950/25 dark:text-cyan-100"> {/* 表头。 */}
        ...
        <ul className="max-h-44 overflow-y-auto"> {/* 任务列表。 */}
          {jobs.length === 0 ? ( // 判断是否暂无任务。
            <li className="px-2 py-3 text-center text-[11px] text-zinc-400">暂无任务；点击上方按钮创建 Day 35 Job。</li> // 空状态。
          ) : ( // 有任务时渲染列表。
            jobs.map((job) => ( // 遍历任务。
              <li key={job.id} className="grid grid-cols-[1fr_0.62fr_0.62fr_0.65fr_0.75fr_0.75fr] items-center gap-1 border-t border-cyan-100 px-2 py-1.5 text-[10px] dark:border-cyan-900/30"> {/* 单行任务。 */}
      ...
      {latestJob ? ( // 判断是否有最新任务。
        <div className="mt-3 rounded-lg border border-cyan-200/70 bg-white/60 px-2 py-2 dark:border-cyan-900/40 dark:bg-zinc-950/25"> {/* 时间线卡片。 */}
          <h3 className="text-xs font-semibold text-cyan-950 dark:text-cyan-100">V5 Timeline | {latestJob.id}</h3> {/* 时间线标题。 */}
