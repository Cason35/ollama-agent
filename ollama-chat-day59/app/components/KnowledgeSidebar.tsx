...
type KnowledgeSidebarProps = {
  ...
  handleDeleteQueueJob: (jobId: string) => Promise<void>; // 第59天：从 Redis Queue Explorer 删除任务动作
...
export function KnowledgeSidebar({
  ...
  const [activePanel, setActivePanel] = useState<"queue" | "runtime" | "redis" | "experiment" | "prompt" | "collaboration" | "model" | "memory" | "cache" | "usage" | "knowledge" | "records">("queue"); /* 第59天：默认打开 Queue 面板，突出 Distributed Queue。 */
  ...
  return (
    <aside className="flex min-h-0 w-full shrink-0 flex-col overflow-hidden rounded-lg border border-zinc-200/80 bg-white shadow-sm shadow-zinc-900/5 dark:border-zinc-800 dark:bg-zinc-900/80 dark:shadow-black/30 lg:h-full lg:w-[400px] xl:w-[430px]">
      <div className="z-10 shrink-0 border-b border-zinc-200/80 bg-white px-3 py-3 dark:border-zinc-800 dark:bg-zinc-900">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-0.5 text-[11px] text-zinc-500 dark:text-zinc-400">按主题查看 Queue、Redis、运行时决策、模型协作、提示词实验、模型健康、用量、知识与历史记录。</p> {/* 第59天：更新控制台标签页说明，突出 Redis Queue 任务调度。 */}
          </div>
          <span className="rounded-full bg-emerald-500/15 px-2.5 py-1 text-[10px] font-semibold text-emerald-800 ring-1 ring-emerald-500/20 dark:text-emerald-200">
            Day 59 {/* 第59天：更新控制台日期徽标。 */}
          </span>
        </div>
        <div className="mt-3 grid grid-cols-12 gap-1" role="tablist" aria-label="Day 59 侧边栏面板"> {/* 第59天：扩展为包含 Queue 浏览器的十二个标签页。 */}
          <button type="button" className={tabClass("queue")} onClick={() => setActivePanel("queue")}> {/* 第59天：Queue 分布式任务队列标签页入口。 */}
            队列 {/* 第59天：显示紧凑的 Queue 标签名称。 */}
          </button> {/* 第59天：结束 Queue 标签页按钮。 */}
      ...
      <div className={activePanel === "queue" ? "contents" : "hidden"}> {/* 第59天：仅在队列标签页展示 Redis Queue Explorer。 */}
      <QueueDashboard
        jobs={queueJobs}
      ...
      /> {/* 第59天：展示 Waiting、Processing、Completed、Dead Letter 与 Queue Trace。 */}
      </div> {/* 第59天：结束 Queue 标签页内容。 */}
