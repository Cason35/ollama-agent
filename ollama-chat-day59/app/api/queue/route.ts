...
export async function PATCH(req: NextRequest) { // 定义第36天队列生命周期动作接口。
  let body: { action?: string; jobId?: string; gracePeriodMs?: number }; // 声明请求体变量。
  try { // 开始解析 JSON。
    body = (await req.json()) as { action?: string; jobId?: string; gracePeriodMs?: number }; // 读取请求体。
  } catch { // 捕获非法 JSON。
    return apiJsonError(API_CODE.BAD_REQUEST, API_CODE.BAD_REQUEST, "请求体必须是 JSON"); // 返回格式错误。
  } // 结束 try/catch。
  try { // 开始重新入队。
    ...
    if (body.action === "delete" && body.jobId) { // 第59天：处理 Queue Explorer 删除任务动作。
      const snapshot = await getQueueRuntime().deleteJob(body.jobId); // 第59天：调用运行时同时清理 Redis Queue 与 JobStore。
      return apiJsonSuccess(snapshot, "job deleted"); // 第59天：返回删除结果。
    } // 第59天：结束删除任务动作。
    ...
    return apiJsonError(API_CODE.BAD_REQUEST, API_CODE.BAD_REQUEST, "请提供 requeue/restart/cancel/delete/shutdown 动作以及必要参数"); // 第59天：返回动作参数错误。
  } catch (error) { // 捕获业务错误。
