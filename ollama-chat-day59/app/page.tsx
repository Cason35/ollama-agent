...
export default function HomePage() {
  ...
  /** 第59天：从 Redis Queue Explorer 删除指定任务。 */
  const handleDeleteQueueJob = async (jobId: string) => {
    setQueueLoading(true); // 第59天：开始队列请求。
    setErrorText(""); // 第59天：清空旧错误。
    try {
      const res = await fetch("/api/queue", {
        method: "PATCH", // 第59天：发送删除动作。
        headers: { "Content-Type": "application/json" }, // 第59天：JSON 请求头。
        body: JSON.stringify({ action: "delete", jobId }), // 第59天：请求体包含 delete 动作与任务 ID。
      }); // 第59天：fetch 调用结束。
      const payload = await readApiData<JobStoreSnapshot>(res); // 第59天：解析删除结果。
      setQueueJobs(payload.jobs ?? []); // 第59天：立即更新任务列表。
      setQueueMetrics(payload.metrics ?? null); // 第59天：立即更新队列指标。
      setWorkerPool(payload.workerPool ?? null); // 第59天：立即更新 WorkerPool 快照。
      setQueueRuntimeSnapshot(payload); // 第59天：立即更新 Queue Explorer 快照。
    } catch (err) {
      setErrorText(err instanceof ApiClientError ? err.message : "删除任务失败"); // 第59天：展示错误。
    } finally {
      setQueueLoading(false); // 第59天：结束队列请求。
    } // 第59天：结束 finally。
  }; // 第59天：结束 handleDeleteQueueJob。
