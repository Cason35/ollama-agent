...
export class QueueRuntime { // 定义第35天队列运行时。
  ...
  async snapshot(): Promise<JobStoreSnapshot> { // 定义读取队列快照的方法。
    ...
    return { jobs, metrics: calculateQueueMetrics(jobs), redisQueue, redisQueueMetrics: redisQueue?.metrics, queueOperations: redisQueue?.operations, workerPool: this.workerPool.getStats(jobs), resourceUsage: this.resourceLimiter.snapshot(), rateLimitUsage: this.rateLimiter.snapshot(), rateLimitMetrics: this.limitMetrics.snapshot() }; // 第59天：返回任务、Redis Queue 指标、WorkerPool 指标和限制指标。
  } // 结束 snapshot。
} // 结束 QueueRuntime。

const globalForQueue = globalThis as typeof globalThis & { __day59QueueRuntime?: QueueRuntime }; // 第59天：扩展 globalThis 保存 Day59 Redis Queue 单例。

export function getQueueRuntime(): QueueRuntime { // 定义获取运行时单例的方法。
  if (!globalForQueue.__day59QueueRuntime) { // 第59天：判断是否已有 Day59 Redis Queue 单例。
    globalForQueue.__day59QueueRuntime = new QueueRuntime(); // 第59天：没有则创建新的运行时。
  } // 结束单例判断。
  return globalForQueue.__day59QueueRuntime; // 第59天：返回运行时单例。
} // 结束 getQueueRuntime。
