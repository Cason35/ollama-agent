...
import type { CostBreakdownItem, ModelUsageAggregate, PromptROIResult, PromptROIVariant, PromptROIVariantInput, PromptUsageAggregate, UsageAggregate, UsageComponentType, UsageMetrics, UsageRecord, UsageRecordInput } from "@/lib/usage/usage-types"; /* 第52天：引入用量管理器需要的全部结构类型（追加提示词用量聚合类型）。 */
...
export class UsageManager { /* 第47天：定义用量记录、聚合、指标和成本分析管理器。 */
  ...
  addRecord(input: UsageRecordInput): UsageRecord { /* 第47天：定义写入并规范化单条用量记录的方法。 */
    ...
    const record: UsageRecord = { ...input, inputTokens, outputTokens, fallbackChain: input.fallbackChain ? [...input.fallbackChain] : undefined, totalTokens: inputTokens + outputTokens, estimatedCost: input.estimatedCost ?? estimateUsageCost(input.componentType, inputTokens, outputTokens), durationMs: Math.max(0, Math.round(input.durationMs)), createdAt: input.createdAt ?? Date.now() }; /* 第51天：统一补齐总词元、费用、耗时、创建时间并复制备用链。 */
  ...
  listRecords(): UsageRecord[] { /* 第47天：定义读取全部用量记录的方法。 */
    return this.records.map((record) => ({ ...record, fallbackChain: record.fallbackChain ? [...record.fallbackChain] : undefined })); /* 第51天：返回按写入顺序排列且备用链已复制的记录副本。 */
  } /* 第47天：结束列出用量记录方法。 */
  ...
  getModelUsage(): ModelUsageAggregate[] { /* 第50天：定义按模型聚合用量与成本的方法，支撑模型级成本归因。 */
    const withModel = this.records.filter((record) => typeof record.modelId === "string" && record.modelId.length > 0); /* 第50天：只统计经 ModelRouter 标注了模型的记录。 */
    return this.groupBy(withModel, (record) => record.modelId as string).map(([modelId, records]) => ({ modelId, provider: records[0].provider ?? "unknown", modelName: records[0].modelName ?? modelId, recordCount: records.length, totalTokens: records.reduce((sum, record) => sum + record.totalTokens, 0), estimatedCost: round(records.reduce((sum, record) => sum + record.estimatedCost, 0)), durationMs: records.reduce((sum, record) => sum + record.durationMs, 0), fallbackUsedCount: records.filter((record) => record.fallbackUsed).length })).sort((left, right) => right.estimatedCost - left.estimatedCost); /* 第51天：按模型分组求和并追加备用链触发次数，再按累计成本降序返回。 */
  } /* 第50天：结束按模型聚合用量方法。 */

  getPromptUsage(): PromptUsageAggregate[] { /* 第52天：定义按提示词版本聚合用量与成本的方法。 */
    const withPrompt = this.records.filter((record) => typeof record.promptId === "string" && typeof record.promptVersion === "string"); /* 第52天：只统计已经标注提示词版本的记录。 */
    return this.groupBy(withPrompt, (record) => record.promptId as string).map(([promptId, records]) => ({ promptId, promptVersion: records[0].promptVersion ?? "unknown", recordCount: records.length, totalTokens: records.reduce((sum, record) => sum + record.totalTokens, 0), estimatedCost: round(records.reduce((sum, record) => sum + record.estimatedCost, 0)), durationMs: records.reduce((sum, record) => sum + record.durationMs, 0) })).sort((left, right) => right.estimatedCost - left.estimatedCost); /* 第52天：按提示词分组求和并按累计成本降序返回。 */
  } /* 第52天：结束按提示词聚合用量方法。 */
