...
import { promptRegistry } from "@/lib/prompts/default-prompts"; /* 第52天：引入共享 PromptRegistry（提示词注册表）用于回归关联。 */
import type { PromptRegressionLink } from "@/lib/prompts/prompt-types"; /* 第52天：引入提示词回归关联类型。 */

const BASELINE_VERSION: EvaluationVersion = { label: "baseline-research-v2", model: "qwen2.5:7b", promptVersion: "research.v2", workflowVersion: "workflow-v46" }; /* 第52天：定义绑定 research.v2 的稳定基线版本。 */
const CANDIDATE_VERSION: EvaluationVersion = { label: "candidate-research-v3", model: "qwen2.5:7b", promptVersion: "research.v3", workflowVersion: "workflow-v46" }; /* 第52天：定义绑定 research.v3 的候选版本。 */
...
function buildPromptRegressionLink(result: "passed" | "failed"): PromptRegressionLink { /* 第52天：定义基线与候选提示词版本的回归关联构造函数。 */
  const baseline = promptRegistry.getVersion("research", "v2"); /* 第52天：读取 research.v2 基线提示词。 */
  const candidate = promptRegistry.getVersion("research", "v3"); /* 第52天：读取 research.v3 候选提示词。 */
  const baselineScore = baseline?.score ?? 0; /* 第52天：读取基线提示词评估分。 */
  const candidateScore = candidate?.score ?? baselineScore; /* 第52天：读取候选提示词评估分。 */
  const baselineCost = baseline?.costEstimate || 1; /* 第52天：读取基线成本并避免除零。 */
  const candidateCost = candidate?.costEstimate || baselineCost; /* 第52天：读取候选成本，缺失时使用基线。 */
  return { componentId: "research", baselinePromptId: baseline?.id ?? "research.v2", candidatePromptId: candidate?.id ?? "research.v3", baselineVersion: "v2", candidateVersion: "v3", result, scoreDelta: Number((candidateScore - baselineScore).toFixed(2)), costDeltaPercent: Number(((candidateCost - baselineCost) / baselineCost * 100).toFixed(2)) }; /* 第52天：返回可被回归看板与 Prompt Explorer 共同解释的关联对象。 */
} /* 第52天：结束提示词回归关联构造函数。 */
...
async function buildRegressionSnapshot(): Promise<RegressionDashboardSnapshot> { /* 第46天：执行一次完整持续评估闭环。 */
  ...
  const promptRegressionLink = buildPromptRegressionLink(qualityGate.status); /* 第52天：把质量门禁结果关联到 research.v2 与 research.v3 的提示词版本对比。 */
  ...
  return { dataset: DAY46_EVALUATION_DATASET, badCases, baseline, candidate, comparison, qualityGate, workspace: storedWorkspace, timeline, trace: traceManager.getTrace(trace.traceId) ?? trace, promptRegressionLink, generatedAt: Date.now() }; /* 第52天：返回回归看板所需的完整快照并附带提示词回归关联。 */
} /* 第46天：结束持续评估闭环执行函数。 */
