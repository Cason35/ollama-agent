...
export function KnowledgeSidebar({
  ...
  const [activePanel, setActivePanel] = useState<"registry" | "events" | "context" | "secrets" | "config" | "storage" | "lock" | "queue" | "runtime" | "redis" | "experiment" | "prompt" | "collaboration" | "model" | "memory" | "cache" | "usage" | "knowledge" | "records">("prompt"); /* 第67天：默认打开提示词面板，突出 Production Prompt Platform。 */
      ...
      <div className="z-10 shrink-0 border-b border-zinc-200/80 bg-white px-3 py-3 dark:border-zinc-800 dark:bg-zinc-900">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-0.5 text-[11px] text-zinc-500 dark:text-zinc-400">第67天默认进入提示词运营，可继续查看统一注册、事件、上下文、模型、知识与历史能力。</p> {/* 第67天：更新控制台说明并突出生产提示词运营入口。 */}
          </div>
          <span className="rounded-full bg-emerald-500/15 px-2.5 py-1 text-[10px] font-semibold text-emerald-800 ring-1 ring-emerald-500/20 dark:text-emerald-200">
            Day 67 {/* 第67天：更新控制台日期徽标。 */}
          </span>
        </div>
        <div className="mt-3 grid grid-cols-12 gap-1" role="tablist" aria-label="Day 67 生产提示词平台侧边栏面板"> {/* 第67天：更新侧边栏标签页名称为生产提示词平台主题。 */}
