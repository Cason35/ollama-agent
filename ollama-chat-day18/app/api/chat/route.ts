...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!Array.isArray(messages) || messages.length === 0) {
      return Response.json({ error: "messages is required" }, { status: 400 }); // 缺少 messages 则 400
    }
    ...
    if (useWorkflow) {
      ...
      if (!validation.ok) {
        ...
        return Response.json(withMemory({ type: "workflow", workflow, finalSummary })); // 早返回：避免 executeWorkflow
      } // 校验短路返回结束
    ...
    if (!routeResult.ok) {
      const errStatus =
        routeResult.status >= 400 && routeResult.status < 600 ? routeResult.status : 502; // 上游 4xx/5xx 透传，否则 502
      return Response.json(
