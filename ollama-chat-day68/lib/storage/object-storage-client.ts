...
import { configManager } from "@/lib/config/config-runtime"; // 第62天：引入配置中心，统一读取对象存储 Provider 配置。
...
export function createObjectStorageClient(providerKind: StorageProviderKind = configManager.getString("storage.provider", "local") as StorageProviderKind): ObjectStorageClient { // 第62天：按配置中心创建对象存储客户端。
