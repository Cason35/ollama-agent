...
import { configManager } from "@/lib/config/config-runtime"; /* 第62天：引入配置中心，统一读取缓存后端选择。 */
...
export function createDefaultCacheStore<TValue>(namespace: string): CacheStore<TValue> { /* 第58天：定义默认缓存 Store 工厂。 */
  return configManager.getString("feature.cacheStore", "redis") === "memory" ? new MemoryCacheStore<TValue>() : new RedisCacheStore<TValue>(namespace); /* 第62天：默认使用 Redis，可通过配置中心切回内存。 */
