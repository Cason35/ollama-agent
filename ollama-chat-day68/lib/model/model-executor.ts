...
import { configManager } from "@/lib/config/config-runtime"; /* 第62天：引入配置中心，统一读取模型执行器兜底运行时配置。 */
...
import { secretsManager } from "@/lib/secrets/secrets-runtime"; /* 第63天：引入密钥管理器，统一读取模型执行器所需敏感凭证。 */
...
async function buildDefaultRuntime(profile: ModelProfile, runtime?: ModelRuntime): Promise<ModelRuntime> { /* 第63天：根据模型档案与可选运行时异步构造本次真实调用运行时。 */
  const ollamaUrl = runtime?.ollamaUrl || configManager.getString("model.ollamaApiUrl", DEFAULT_OLLAMA_API_URL).trim() || DEFAULT_OLLAMA_API_URL; /* 第62天：从配置中心读取 Ollama API 兜底地址。 */
  const ollamaModel = runtime?.ollamaModel || configManager.getString("model.default", DEFAULT_OLLAMA_MODEL).trim() || DEFAULT_OLLAMA_MODEL; /* 第62天：从配置中心读取默认 Ollama 模型。 */
  const mimoBaseUrl = runtime?.mimoBaseUrl || configManager.getString("model.mimoBaseUrl", DEFAULT_MIMO_BASE_URL).trim() || DEFAULT_MIMO_BASE_URL; /* 第62天：从配置中心读取 MiMo Base URL。 */
  const mimoApiKey = runtime?.mimoApiKey || (await secretsManager.get("XIAOMI_MIMO_API_KEY"))?.trim() || ""; /* 第63天：从 SecretsManager 读取 MiMo API Key。 */
  if (profile.provider === "ollama" || profile.provider === "local") return { provider: "local", ollamaUrl, ollamaModel: profile.model || ollamaModel, mimoBaseUrl, mimoApiKey, mimoModel: runtime?.mimoModel || "" }; /* 第62天：Ollama 或 local 档案统一走配置中心模型运行时。 */
  return { provider: "mimo", ollamaUrl, ollamaModel, mimoBaseUrl, mimoApiKey, mimoModel: profile.model || runtime?.mimoModel || "" }; /* 第62天：非本地档案统一映射为配置中心驱动的 MiMo 兼容调用。 */
} /* 第51天：结束默认运行时构造。 */
...
export const defaultModelCaller: ModelCaller = async (profile, messages, runtime, timeoutMs) => { /* 第51天：定义默认底层模型调用函数。 */
  ...
  const rt = await buildDefaultRuntime(profile, runtime); /* 第63天：按模型档案异步得到本次调用运行时。 */
