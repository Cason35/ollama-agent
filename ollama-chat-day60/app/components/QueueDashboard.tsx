...
import { useRef, useState } from "react"; // 第60天：引入 useRef 和 useState，分别保存点击防抖与 Inspect 选中任务。
...
type QueueDashboardProps = { // 定义队列看板组件参数。
  ...
  handleDeleteQueueJob: (jobId: string) => Promise<void>; // 第60天：接收删除 Redis Queue 任务回调。
...
export function QueueDashboard({ // 导出队列看板组件。
  ...
  handleDeleteQueueJob, // 第60天：接收删除任务回调。
  handleGracefulShutdown, // 第36天：接收优雅关闭回调。
}: QueueDashboardProps) { // 结束组件参数。
  const [inspectedJobId, setInspectedJobId] = useState<string | null>(null); // 第60天：保存用户正在 Inspect 的 Job ID。
  const latestJob = jobs[0] ?? null; // 取最新任务作为 Inspect 默认值。
  const inspectedJob = jobs.find((job) => job.id === inspectedJobId) ?? latestJob; // 第60天：优先展示用户选中的任务，否则展示最新任务。
  ...
  const redisQueue = runtimeSnapshot?.redisQueue ?? null; // 第60天：读取 Redis Queue Explorer 快照。
  const redisQueueMetrics = runtimeSnapshot?.redisQueueMetrics ?? redisQueue?.metrics ?? null; // 第60天：读取 Redis Queue Metrics。
  const queueOperations = runtimeSnapshot?.queueOperations ?? redisQueue?.operations ?? []; // 第60天：读取 Queue Operation Trace。
  ...
  const createTenEmbeddingJobs = () => { // 第36天：定义 10 个 embedding 资源并发与取消压测入口。
    Array.from({ length: 10 }, (_, index) => index + 1).forEach((index) => { // 循环创建 10 个 embedding 长任务。
      void handleCreateQueueJob({ type: "embedding", priority: 1, timeoutMs: 30000, payload: { durationMs: 5000, document: `Day60 Redis Queue embedding 生命周期压测任务 ${index}` } }); // 第60天：创建 5 秒 embedding 任务。
    }); // 结束循环。
  ...
  const createFiveChatJobs = () => { // 第36天：定义 llm 速率限制压测入口。
    Array.from({ length: 5 }, (_, index) => index + 1).forEach((index) => { // 循环创建 5 个 chat 任务。
      void handleCreateQueueJob({ type: "chat", priority: 10, payload: { prompt: `Day60 Redis Queue llm rate limit burst ${index}` } }); // 第60天：创建 chat 任务观察每秒速率限制。
    }); // 结束循环。
  }; // 结束 createFiveChatJobs。
  const createTimeoutEmbeddingJob = () => handleDebouncedCreateJob({ type: "embedding", priority: 10, timeoutMs: 1200, payload: { durationMs: 5000, document: "Day60 Redis Queue visibility timeout 测试任务" } }); // 第60天：创建一个会触发超时的 embedding 任务。
  const createManualWorkflowJob = () => { // 第37天：定义无需聊天入口也能创建的最小 WorkflowJob。
    const workflowId = `manual_wf_${Date.now()}`; // 生成手动工作流 ID。
    handleDebouncedCreateJob({ type: "workflow", priority: 8, timeoutMs: 120000, payload: { workflowId, workflow: { id: workflowId, goal: "手动创建 Day60 Redis Queue WorkflowJob 示例", status: "queued", steps: [], executionTimeline: [{ ts: Date.now(), message: "手动 WorkflowJob 已在 Queue Explorer 创建" }] }, memory: { shortTerm: [], items: [] }, provider: "local" } }); // 第60天：创建包含完整 workflow 与 memory 的合法 WorkflowJob。
  }; // 结束 createManualWorkflowJob。
  return ( // 返回队列看板 UI。
    <div className="shrink-0 border-b border-cyan-200/70 px-4 py-3 dark:border-cyan-900/40"> {/* 队列看板容器。 */}
      <h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">Queue Explorer（队列浏览器）Day 60</h2> {/* 第60天：看板标题。 */}
      <p className="mt-1 text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">第60天：Production Infrastructure V3（生产基础设施第3版），在 Redis Queue 基础上加入 Lock Guard（锁保护），观察任务入队、认领、重试、死信与 Worker 获取分布式锁后的执行链路。</p> {/* 第60天：看板说明。 */}
      <div className="mt-2 grid grid-cols-2 gap-2 text-[10px]"> {/* 队列指标网格。 */}
        <div className="rounded-lg border border-sky-200/70 bg-sky-50/60 px-2 py-1.5 dark:border-sky-900/40 dark:bg-sky-950/20">waiting（等待）: {redisQueueMetrics?.waiting ?? metrics?.queuedJobs ?? 0}</div> {/* 第60天：Redis Waiting Queue 指标。 */}
        <div className="rounded-lg border border-amber-200/70 bg-amber-50/60 px-2 py-1.5 dark:border-amber-900/40 dark:bg-amber-950/20">processing（处理中）: {redisQueueMetrics?.processing ?? metrics?.runningJobs ?? 0}</div> {/* 第60天：Redis Processing Queue 指标。 */}
        <div className="rounded-lg border border-emerald-200/70 bg-emerald-50/60 px-2 py-1.5 dark:border-emerald-900/40 dark:bg-emerald-950/20">completed（已完成）: {redisQueueMetrics?.completed ?? metrics?.successJobs ?? 0}</div> {/* 第60天：Redis Completed Queue 指标。 */}
        <div className="rounded-lg border border-red-200/70 bg-red-50/60 px-2 py-1.5 dark:border-red-900/40 dark:bg-red-950/20">dead letter（死信）: {redisQueueMetrics?.failed ?? metrics?.deadLetterJobs ?? 0}</div> {/* 第60天：Redis Dead Letter Queue 指标。 */}
        ...
        <div className="rounded-lg border border-cyan-200/70 bg-cyan-50/60 px-2 py-1.5 dark:border-cyan-900/40 dark:bg-cyan-950/20">avg wait（平均等待）: {redisQueueMetrics?.avgWaitTime ?? 0}ms</div> {/* 第60天：展示平均等待时长。 */}
        <div className="rounded-lg border border-orange-200/70 bg-orange-50/60 px-2 py-1.5 dark:border-orange-900/40 dark:bg-orange-950/20">avg processing（平均处理）: {redisQueueMetrics?.avgProcessingTime ?? 0}ms</div> {/* 第60天：展示平均处理时长。 */}
      </div> {/* 结束队列指标网格。 */}
      <p className="mt-2 font-mono text-[10px] text-cyan-900 dark:text-cyan-100">backend(后端): {redisQueue?.backend ?? "redis-list"} | namespace(命名空间): {redisQueue?.namespace ?? "Redis 未连接"} | active(活跃): {workerPool?.metrics.activeWorkers ?? 0} | idle(空闲): {workerPool?.metrics.idleWorkers ?? 0} | allowed(放行): {rateLimitMetrics?.allowedCount ?? 0} | blocked(阻塞): {rateLimitMetrics?.blockedCount ?? 0}</p> {/* 第60天：展示 Redis Queue 后端、命名空间、WorkerPool 与限制器摘要。 */}
      <div className="mt-2 rounded-lg border border-cyan-200/70 bg-cyan-50/50 px-2 py-2 dark:border-cyan-900/40 dark:bg-cyan-950/20"> {/* 第60天：Queue Explorer 四桶面板。 */}
        <h3 className="text-xs font-semibold text-cyan-950 dark:text-cyan-100">Redis Queue Buckets（Redis 队列桶）</h3> {/* 第60天：四桶面板标题。 */}
        <div className="mt-2 grid grid-cols-2 gap-2"> {/* 第60天：四桶网格布局。 */}
          {(redisQueue?.buckets ?? []).map((bucket) => ( // 第60天：遍历 Waiting、Processing、Completed 和 Dead Letter。 
            <div key={bucket.name} className="min-w-0 rounded-md border border-cyan-100 bg-white/70 px-2 py-1.5 dark:border-cyan-900/40 dark:bg-zinc-950/30"> {/* 第60天：单个队列桶卡片。 */}
              <p className="font-mono text-[10px] font-semibold text-cyan-950 dark:text-cyan-100">{bucket.name} · {bucket.jobs.length}</p> {/* 第60天：展示队列桶名称与任务数量。 */}
              <ul className="mt-1 max-h-20 space-y-1 overflow-y-auto"> {/* 第60天：队列桶内任务列表。 */}
                {bucket.jobs.length === 0 ? <li className="text-[10px] text-zinc-400">empty（空）</li> : null} {/* 第60天：空桶占位。 */}
                {bucket.jobs.slice(0, 5).map((job) => ( // 第60天：最多展示前五个任务，完整数据仍在任务表中。 
                  <li key={`${bucket.name}-${job.id}`} className="truncate font-mono text-[9px] text-cyan-800 dark:text-cyan-200" title={`${job.id} | ${job.status}`}>{job.id} · {job.status}</li> // 第60天：展示队列桶内 Job 摘要。
                ))} {/* 第60天：结束桶内任务遍历。 */}
              </ul> {/* 第60天：结束队列桶内任务列表。 */}
            </div> // 第60天：结束单个队列桶卡片。
          ))} {/* 第60天：结束四桶遍历。 */}
          {!redisQueue ? <p className="col-span-2 rounded border border-dashed border-cyan-200 px-2 py-3 text-center text-[11px] text-cyan-700 dark:border-cyan-900/50 dark:text-cyan-300">Redis Queue 暂不可用；请确认 Redis 已启动。</p> : null} {/* 第60天：Redis 不可用时展示友好提示。 */}
        </div> {/* 第60天：结束四桶网格布局。 */}
      </div> {/* 第60天：结束 Queue Explorer 四桶面板。 */}
      ...
      <div className="mt-3 overflow-hidden rounded-lg border border-cyan-200/70 dark:border-cyan-900/40"> {/* 任务表格外框。 */}
        <div className="grid grid-cols-[1fr_0.58fr_0.58fr_0.65fr_0.65fr_0.65fr_1.05fr] bg-cyan-50/80 px-2 py-1 font-mono text-[9px] text-cyan-900 dark:bg-cyan-950/25 dark:text-cyan-100"> {/* 第60天：扩展 Action 列以容纳 Inspect、Cancel 和 Delete。 */}
        ...
        <ul className="max-h-44 overflow-y-auto"> {/* 任务列表。 */}
          {jobs.length === 0 ? ( // 判断是否暂无任务。
            <li className="px-2 py-3 text-center text-[11px] text-zinc-400">暂无任务；点击上方按钮创建 Day 60 Redis Queue Job（任务）。</li> // 第60天：空状态。
          ) : ( // 有任务时渲染列表。
            jobs.map((job) => ( // 遍历任务。
              <li key={job.id} className="grid grid-cols-[1fr_0.58fr_0.58fr_0.65fr_0.65fr_0.65fr_1.05fr] items-center gap-1 border-t border-cyan-100 px-2 py-1.5 text-[10px] dark:border-cyan-900/30"> {/* 第60天：单行任务。 */}
                ...
                <span className="flex flex-wrap gap-1"> {/* 第60天：Action 按钮组。 */}
                  <button type="button" disabled={queueLoading} onClick={() => setInspectedJobId(job.id)} className={inspectButtonClass}>Inspect（检查）</button> {/* 第60天：选中任务并在下方展示详情。 */}
                  {canCancel(job) ? <button type="button" disabled={queueLoading} onClick={() => void handleCancelQueueJob(job.id)} className={cancelButtonClass}>Cancel（取消）</button> : null} {/* 第60天：取消 queued/retrying/running 任务。 */}
                  <button type="button" disabled={queueLoading} onClick={() => void handleDeleteQueueJob(job.id)} className={requeueButtonClass}>Delete（删除）</button> {/* 第60天：从 Redis Queue 与 JobStore 删除任务。 */}
                </span> {/* 第60天：结束 Action 按钮组。 */}
              </li> // 结束单行任务。
            )) // 结束任务遍历。
          )} {/* 结束条件渲染。 */}
        </ul> {/* 结束任务列表。 */}
      ...
      <div className="mt-3 rounded-lg border border-cyan-200/70 bg-cyan-50/40 px-2 py-2 dark:border-cyan-900/40 dark:bg-cyan-950/15"> {/* 第60天：Queue Operation Trace 面板。 */}
        <h3 className="text-xs font-semibold text-cyan-950 dark:text-cyan-100">Queue Operation Trace（队列操作追踪）</h3> {/* 第60天：队列追踪标题。 */}
        <ul className="mt-2 max-h-32 space-y-1 overflow-y-auto"> {/* 第60天：队列追踪列表。 */}
          {queueOperations.length === 0 ? <li className="text-[11px] text-zinc-400">暂无 Queue Trace；创建任务后会出现 enqueue、dequeue、ack、retry、fail、delete。</li> : null} {/* 第60天：队列追踪空状态。 */}
          {queueOperations.slice(0, 10).map((op) => ( // 第60天：展示最近 10 条队列操作。
            <li key={op.id} className="rounded border border-cyan-100 bg-white/70 px-2 py-1 text-[10px] dark:border-cyan-900/40 dark:bg-zinc-950/30"> {/* 第60天：单条队列追踪。 */}
              <p className="font-mono font-semibold text-cyan-950 dark:text-cyan-100">{op.operation} · {op.status} · {op.latencyMs}ms</p> {/* 第60天：展示操作名、状态和耗时。 */}
              <p className="break-words text-cyan-800 dark:text-cyan-200">job(任务): {op.jobId ?? DASH} | worker(工作线程): {op.workerId ?? DASH} | bucket(队列桶): {op.bucket ?? DASH}</p> {/* 第60天：展示操作上下文。 */}
              <p className="break-words text-zinc-500 dark:text-zinc-400">{op.note} · {new Date(op.createdAt).toLocaleTimeString("zh-CN")}</p> {/* 第60天：展示中文说明和时间。 */}
            </li> // 第60天：结束单条队列追踪。
          ))} {/* 第60天：结束队列追踪遍历。 */}
        </ul> {/* 第60天：结束队列追踪列表。 */}
      </div> {/* 第60天：结束 Queue Operation Trace 面板。 */}
      {inspectedJob ? ( // 第60天：判断是否有可检查任务。
        <div className="mt-3 rounded-lg border border-cyan-200/70 bg-white/60 px-2 py-2 dark:border-cyan-900/40 dark:bg-zinc-950/25"> {/* 时间线卡片。 */}
          <h3 className="text-xs font-semibold text-cyan-950 dark:text-cyan-100">Inspect Job（检查任务） | {inspectedJob.id}</h3> {/* 第60天：检查任务标题。 */}
          <p className="mt-1 font-mono text-[10px] text-cyan-800 dark:text-cyan-200">status(状态): {inspectedJob.status} | duration(耗时): {formatDuration(inspectedJob)} | resource(资源): {inspectedJob.resourceType ?? DASH} | attempts(尝试): {inspectedJob.attempts}/{inspectedJob.maxAttempts}</p> {/* 第60天：检查任务摘要。 */}
          <p className="mt-1 break-words font-mono text-[10px] text-zinc-500 dark:text-zinc-400">payload(载荷): {JSON.stringify(inspectedJob.payload)}</p> {/* 第60天：展示任务载荷，完成 Inspect 要求。 */}
          <ol className="mt-2 space-y-1"> {/* 时间线列表。 */}
            {inspectedJob.timeline.map((item) => ( // 第60天：遍历被检查任务的时间线节点。
