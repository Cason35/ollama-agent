      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-50 sm:text-3xl">
            Evaluation Framework（评估框架） · Production Runtime V2（生产运行时第2版）
          </h1>
          <p className="mt-2 max-w-2xl text-sm leading-relaxed text-zinc-600 dark:text-zinc-400">
            第45天：在 Day44 的 Observability & Tracing（可观测性与链路追踪）基础上加入 EvaluationResult（评估结果）、Evaluation Agent（评估智能体）、evaluateOutput（评估输出函数）、Evaluation Metrics（评估指标）和 Prompt A/B Test（提示词 A/B 测试），让 Agent 系统不只知道内部发生了什么，也能量化输出到底好不好。
            Queue（队列）、Workflow（工作流）、RAG（检索增强生成）、Tool Registry（工具注册表）、WorkerPool（工作线程池）、Trace（追踪）与 Reflection（反思）链路继续保留，侧栏仍可 Import（导入）笔记、观察向量写入并调试检索；需
            Ollama{" "}
