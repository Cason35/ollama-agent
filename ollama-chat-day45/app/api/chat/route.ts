 ...
 * 3. 否则走 Day40 Supervisor Agent → AgentPlan → validate → executeAgentPlan；
...
import { executeSupervisorAgentCollaboration } from "@/lib/agents/agent-executor"; // 第40天：引入 Supervisor Agent 动态协作入口。
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { memory } = await buildMemory(messages, incomingMemory, rt); // 构建记忆供 Supervisor 和各 Agent 使用
    ...
    if (useWorkflow) {
      ...
      pushTimeline("Workflow 已通过校验，准备创建 Queue Job，由 Worker 异步执行", undefined); // 第37天：写入 Workflow as Job 分界点。
    ...
    // ---------- 第40天 Supervisor Agent 分支 ----------
    const supervisorStart = Date.now(); // 记录 Supervisor 协作链开始时间。
    const latestUser = getLatestUserText(memory.shortTerm); // 读取最新用户目标。
    const collaboration = await executeSupervisorAgentCollaboration(latestUser, { memory, workflow: null, tools: null }, rt); // 让 Supervisor 规划并执行真实 Agent 协作链。
    const finalWriterOutput = collaboration.result.childResults?.at(-1)?.output ?? collaboration.result.output; // 读取最后一个 Agent 输出作为聊天最终答案。
    logAgent("result", { action: "supervisor", durationMs: Date.now() - supervisorStart, success: true, selectedAgents: collaboration.plan.selectedAgents }); // 记录 Supervisor 协作完成日志。
    return apiJsonSuccess(withMemory({ type: "chat", content: finalWriterOutput })); // 以普通聊天气泡返回最终 Agent 输出。
  } catch (error) {
    logAgent("error", {
      success: false, // 失败
