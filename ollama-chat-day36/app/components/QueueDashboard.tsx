...
function blockedText(job: Job): string { // 第35天：定义阻塞原因展示函数。
  if (job.blockedReason === "resource_limit") return `waiting ${job.resourceType ?? "resource"}`; // 资源并发满时展示等待资源。
  if (job.blockedReason === "rate_limit") return `rate limited ${job.resourceType ?? "resource"}`; // 速率超限时展示速率限制。
...
export function QueueDashboard({ // 导出队列看板组件。
  ...
  const deadLetterJobs = jobs.filter((job) => job.status === "dead_letter" || job.status === "timeout"); // 第36天：找出死信与最终超时任务列表。
  ...
  const createTenEmbeddingJobs = () => { // 第36天：定义 10 个 embedding 资源并发与取消压测入口。
    Array.from({ length: 10 }, (_, index) => index + 1).forEach((index) => { // 循环创建 10 个 embedding 长任务。
      void handleCreateQueueJob({ type: "embedding", priority: 1, timeoutMs: 30000, payload: { durationMs: 5000, document: `Day36 embedding 生命周期压测任务 ${index}` } }); // 创建 5 秒 embedding 任务。
    }); // 结束循环。
  ...
  const createFiveChatJobs = () => { // 第36天：定义 llm 速率限制压测入口。
    Array.from({ length: 5 }, (_, index) => index + 1).forEach((index) => { // 循环创建 5 个 chat 任务。
      void handleCreateQueueJob({ type: "chat", priority: 10, payload: { prompt: `Day36 llm rate limit burst ${index}` } }); // 创建 chat 任务观察每秒速率限制。
    }); // 结束循环。
  }; // 结束 createFiveChatJobs。
  const createTimeoutEmbeddingJob = () => handleDebouncedCreateJob({ type: "embedding", priority: 10, timeoutMs: 1200, payload: { durationMs: 5000, document: "Day36 timeout wrapper 测试任务" } }); // 第36天：创建一个会触发超时的 embedding 任务。
  return ( // 返回队列看板 UI。
    <div className="shrink-0 border-b border-cyan-200/70 px-4 py-3 dark:border-cyan-900/40"> {/* 队列看板容器。 */}
      <h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">Queue Runtime V6</h2> {/* 看板标题。 */}
      <p className="mt-1 text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">第36天：Cancellation + Timeout + Graceful Shutdown，在 V5 资源与速率控制基础上管理任务生命周期。</p> {/* 看板说明。 */}
      <div className="mt-2 grid grid-cols-2 gap-2 text-[10px]"> {/* 队列指标网格。 */}
        ...
        <div className="rounded-lg border border-zinc-200/70 bg-zinc-50/60 px-2 py-1.5 dark:border-zinc-800 dark:bg-zinc-950/20">cancelled: {metrics?.cancelledJobs ?? 0}</div> {/* 第36天：已取消指标。 */}
        <div className="rounded-lg border border-rose-200/70 bg-rose-50/60 px-2 py-1.5 dark:border-rose-900/40 dark:bg-rose-950/20">timeout: {metrics?.timeoutJobs ?? 0}</div> {/* 第36天：超时指标。 */}
        <div className="rounded-lg border border-cyan-200/70 bg-cyan-50/60 px-2 py-1.5 dark:border-cyan-900/40 dark:bg-cyan-950/20">allowed: {rateLimitMetrics?.allowedCount ?? 0}</div> {/* 限制器放行指标。 */}
        <div className="rounded-lg border border-orange-200/70 bg-orange-50/60 px-2 py-1.5 dark:border-orange-900/40 dark:bg-orange-950/20">blocked: {rateLimitMetrics?.blockedCount ?? 0}</div> {/* 限制器阻塞指标。 */}
      </div> {/* 结束队列指标网格。 */}
      <p className="mt-2 font-mono text-[10px] text-cyan-900 dark:text-cyan-100">active: {workerPool?.metrics.activeWorkers ?? 0} | idle: {workerPool?.metrics.idleWorkers ?? 0} | throughput/min: {workerPool?.metrics.throughputPerMinute ?? 0}</p> {/* WorkerPool 摘要。 */}
      <div className="mt-2 rounded-lg border border-zinc-200/70 bg-zinc-50/70 px-2 py-2 dark:border-zinc-800 dark:bg-zinc-950/30"> {/* 资源占用面板。 */}
        <h3 className="text-xs font-semibold text-zinc-900 dark:text-zinc-100">Resource Usage</h3> {/* 资源占用标题。 */}
      ...
      <div className="mt-2 rounded-lg border border-zinc-200/70 bg-zinc-50/70 px-2 py-2 dark:border-zinc-800 dark:bg-zinc-950/30"> {/* 速率限制面板。 */}
        <h3 className="text-xs font-semibold text-zinc-900 dark:text-zinc-100">Rate Limit Window</h3> {/* 速率限制标题。 */}
      ...
      <div className="mt-2 rounded-lg border border-zinc-200/70 bg-zinc-50/70 px-2 py-2 dark:border-zinc-800 dark:bg-zinc-950/30"> {/* WorkerPool 面板。 */}
        <h3 className="text-xs font-semibold text-zinc-900 dark:text-zinc-100">Worker Pool</h3> {/* WorkerPool 标题。 */}
      ...
      <div className="mt-2 grid grid-cols-2 gap-2"> {/* 创建任务按钮区域。 */}
        <button type="button" disabled={queueLoading} onClick={() => create("chat", 10)} className={createJobButtonClass}>High Chat P10</button> {/* 创建高优先级 chat 任务。 */}
        <button type="button" disabled={queueLoading} onClick={() => create("workflow", 8)} className={createJobButtonClass}>Workflow P8</button> {/* 创建 workflow 任务。 */}
        <button type="button" disabled={queueLoading} onClick={() => create("retrieval", 5)} className={createJobButtonClass}>Retrieval DB P5</button> {/* 创建 database 资源任务。 */}
        <button type="button" disabled={queueLoading} onClick={() => create("reindex", 3)} className={createJobButtonClass}>Reindex Embedding</button> {/* 创建 reindex 任务。 */}
        ...
        <button type="button" disabled={queueLoading} onClick={createTenEmbeddingJobs} className={`${createJobButtonClass} col-span-2`}>Create 10 Embedding Jobs</button> {/* 创建 10 个 embedding 资源限制压测任务。 */}
        <button type="button" disabled={queueLoading} onClick={createFiveChatJobs} className={`${createJobButtonClass} col-span-2`}>Burst 5 Chat Jobs</button> {/* 创建 5 个 chat 速率限制压测任务。 */}
        <button type="button" disabled={queueLoading} onClick={createTimeoutEmbeddingJob} className={`${createJobButtonClass} col-span-2`}>Timeout Embedding Job</button> {/* 第36天：创建会触发 timeout wrapper 的任务。 */}
        <button type="button" disabled={queueLoading} onClick={() => void handleGracefulShutdown()} className={`${cancelButtonClass} col-span-2`}>Graceful Shutdown</button> {/* 第36天：触发 WorkerPool 优雅关闭。 */}
      </div> {/* 结束创建任务按钮区域。 */}
      <div className="mt-3 overflow-hidden rounded-lg border border-cyan-200/70 dark:border-cyan-900/40"> {/* 任务表格外框。 */}
        ...
        <ul className="max-h-44 overflow-y-auto"> {/* 任务列表。 */}
          {jobs.length === 0 ? ( // 判断是否暂无任务。
            <li className="px-2 py-3 text-center text-[11px] text-zinc-400">暂无任务；点击上方按钮创建 Day 36 Job。</li> // 空状态。
          ) : ( // 有任务时渲染列表。
            jobs.map((job) => ( // 遍历任务。
              <li key={job.id} className="grid grid-cols-[1fr_0.58fr_0.58fr_0.65fr_0.65fr_0.65fr_0.62fr] items-center gap-1 border-t border-cyan-100 px-2 py-1.5 text-[10px] dark:border-cyan-900/30"> {/* 单行任务。 */}
                <span className="truncate font-mono" title={`${job.id} | ${priorityLabel(job.priority)} | wait ${formatWait(job)}`}>{job.id}</span> {/* 任务 ID。 */}
                ...
                {canCancel(job) ? ( // 第36天：判断是否展示取消按钮。
                  <button type="button" disabled={queueLoading} onClick={() => void handleCancelQueueJob(job.id)} className={cancelButtonClass}>Cancel</button> // 第36天：取消 queued/retrying/running 任务。
                ) : (
      ...
      {deadLetterJobs.length ? ( // 判断是否存在死信任务。
        <div className="mt-3 rounded-lg border border-red-200/80 bg-red-50/60 px-2 py-2 dark:border-red-900/50 dark:bg-red-950/20"> {/* 死信任务区域。 */}
          <h3 className="text-xs font-semibold text-red-950 dark:text-red-100">Dead Letter / Timeout Jobs</h3> {/* 第36天：死信和最终超时标题。 */}
          <ul className="mt-2 max-h-28 space-y-1 overflow-y-auto"> {/* 死信任务列表。 */}
            {deadLetterJobs.map((job) => ( // 遍历死信任务。
              <li key={job.id} className="rounded border border-red-200/80 bg-white/70 px-2 py-1.5 text-[10px] dark:border-red-900/50 dark:bg-zinc-950/30"> {/* 单个死信任务。 */}
                ...
                <p className="mt-1 break-words text-red-800 dark:text-red-200">type: {job.type} | resource: {job.resourceType ?? DASH} | attempts: {job.attempts}/{job.maxAttempts}</p> {/* 死信摘要。 */}
      ...
      {latestJob ? ( // 判断是否有最新任务。
        <div className="mt-3 rounded-lg border border-cyan-200/70 bg-white/60 px-2 py-2 dark:border-cyan-900/40 dark:bg-zinc-950/25"> {/* 时间线卡片。 */}
          <h3 className="text-xs font-semibold text-cyan-950 dark:text-cyan-100">V6 Timeline | {latestJob.id}</h3> {/* 时间线标题。 */}
          <p className="mt-1 font-mono text-[10px] text-cyan-800 dark:text-cyan-200">duration: {formatDuration(latestJob)} | resource: {latestJob.resourceType ?? DASH} | attempts: {latestJob.attempts}/{latestJob.maxAttempts}</p> {/* 最新任务摘要。 */}
