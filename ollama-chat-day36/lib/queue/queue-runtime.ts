...
function createJob(input: CreateJobInput): Job { // 定义任务对象创建函数。
  ...
  const items: JobTimelineItem[] = [timeline("Created", `用户请求已创建 Job，并进入 Queue Runtime V6 生命周期控制队列，资源类型为 ${resourceType}，超时时间为 ${timeoutMs}ms`)]; // 写入创建时间线。
...
export class QueueRuntime { // 定义第35天队列运行时。
  ...
  async requeue(jobId: string): Promise<JobStoreSnapshot & { requeued: Job }> { // 定义手动重新入队方法。
    ...
    const requeued: Job = { // 构造重新入队任务。
      ...
      updatedAt: now, // 写入更新时间。
      timeline: [...current.timeline, timeline("Requeued", "用户手动重新入队，任务将重新参与 V6 生命周期、资源与速率调度")], // 追加重新入队节点。
    }; // 结束 requeued 任务。
  ...
  async cancelJob(jobId: string): Promise<JobStoreSnapshot & { cancelled: Job }> { // 第36天：定义取消任务方法。
    ...
    if (current.status === "queued" || current.status === "retrying") { // 判断是否为尚未运行的任务。
      const queuedCancelled = this.queue.cancelQueuedJob({ ...current, cancelRequestedAt: now }); // 尝试从内存队列中直接取消。
...
const globalForQueue = globalThis as typeof globalThis & { __day36QueueRuntime?: QueueRuntime }; // 扩展 globalThis 保存第36天单例。

export function getQueueRuntime(): QueueRuntime { // 定义获取运行时单例的方法。
  if (!globalForQueue.__day36QueueRuntime) { // 判断是否已有第36天单例。
    globalForQueue.__day36QueueRuntime = new QueueRuntime(); // 没有则创建新的运行时。
  } // 结束单例判断。
  return globalForQueue.__day36QueueRuntime; // 返回运行时单例。
} // 结束 getQueueRuntime。
