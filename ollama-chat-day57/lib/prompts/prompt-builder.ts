...
function canSkip(block: PromptBlock): boolean { /* 第54天：定义提示词块是否允许条件跳过的函数。 */
  return block.skipIfMissing === true || block.type === "memory" || block.type === "workspace" || block.type === "knowledge" || block.type === "tool" || block.type === "citation"; /* 第55天：显式可跳过或上下文型块缺失时允许跳过。 */
} /* 第54天：结束条件跳过判断函数。 */
...
export class PromptBuilder { /* 第54天：定义 PromptBuilder（提示词构建器）。 */
  ...
  buildPromptWithReport(blocks: PromptBlock[], variables: PromptVariables): PromptBuildResult { /* 第54天：定义带渲染报告的提示词构建入口。 */
    ...
    const text = usedRecords.map((record) => `${sectionTitle({ id: record.blockId, name: record.blockName, type: record.blockType, template: record.renderedText, enabled: record.enabled, order: 0, weight: 0 })}\n${record.renderedText}`).join("\n\n"); /* 第55天：把命中块按标题和正文组合成最终 Prompt，并为标题辅助对象补齐 weight。 */
