...
import { configManager } from "@/lib/config/config-runtime"; // 第62天：引入配置中心，统一读取 Redis Queue 可见性超时。
...
export class RedisQueueStore implements QueueStore { // 第59天：实现基于 Redis List 的 QueueStore。
  ...
  constructor(private readonly client: RedisClient = redisClient, private readonly visibilityTimeoutMs = configManager.getNumber("redis.queueVisibilityTimeoutMs", DEFAULT_VISIBILITY_TIMEOUT_MS)) {} // 第62天：默认使用共享 RedisClient，并从配置中心读取可见性超时。
