  ...
  return ( // 第62天：返回 Storage Explorer 视图。
    <section className="space-y-3"> {/* 第62天：定义面板整体间距。 */}
      <div className="rounded-lg border border-teal-200/80 bg-teal-50/70 px-3 py-3 dark:border-teal-900/50 dark:bg-teal-950/20"> {/* 第62天：定义对象存储概览区域。 */}
        <div className="flex items-start justify-between gap-3"> {/* 第62天：排列标题和刷新按钮。 */}
          <div> {/* 第62天：定义标题文案容器。 */}
            ...
            <p className="mt-1 text-xs leading-relaxed text-teal-800/80 dark:text-teal-200/80">Day 63：Object Storage 继续管理 Knowledge、Workspace Export 与 Trace Attachment，MinIO 凭证改由 SecretsManager 读取。</p> {/* 第63天：展示对象存储在密钥管理项目中的凭证接入方式。 */}
          </div> {/* 第62天：结束标题文案容器。 */}
      ...
      <div className="grid grid-cols-3 gap-2"> {/* 第62天：定义演示动作按钮区。 */}
        <button type="button" disabled={loading} onClick={() => void runAction("upload-demo", "Day63 Secrets-safe storage upload chain demo")} className="rounded-md bg-teal-600 px-2 py-1.5 text-[11px] font-semibold text-white disabled:opacity-50">上传</button> {/* 第63天：触发不暴露密钥的对象存储上传演示。 */}
      ...
      <ul className="max-h-[36rem] space-y-2 overflow-y-auto pr-1"> {/* 第62天：定义对象列表。 */}
        {objects.length === 0 ? ( // 第62天：判断当前是否没有对象。
          <li className="rounded-lg border border-dashed border-zinc-200 px-3 py-6 text-center text-xs text-zinc-400 dark:border-zinc-700">暂无对象；点击上传、导出或附件生成 Day63 测试对象。</li> // 第63天：展示对象存储空状态。
        ) : ( // 第62天：存在对象时展示列表。
