...
export default function HomePage() {
  ...
  const handleCreateQueueJob = async (input: CreateJobInput) => {
    ...
    try {
      const payloadByType: Record<JobType, Record<string, unknown>> = {
        chat: { prompt: "第42天 chat 任务，用于在 Agent Workspace Runtime 项目中继续测试 llm 资源速率限制与生命周期控制" }, // 第42天：chat 任务载荷
        ...
        reminder: { text: "第42天 Reminder Job 已按 scheduledAt 自动触发" }, // 第42天：reminder 定时提醒任务载荷
      }; // 任务载荷映射结束
  ...
  /** 第60天：从 Redis Queue Explorer 删除指定任务。 */
  const handleDeleteQueueJob = async (jobId: string) => {
    setQueueLoading(true); // 第60天：开始队列请求。
    setErrorText(""); // 第60天：清空旧错误。
    try {
      const res = await fetch("/api/queue", {
        method: "PATCH", // 第60天：发送删除动作。
        headers: { "Content-Type": "application/json" }, // 第60天：JSON 请求头。
        body: JSON.stringify({ action: "delete", jobId }), // 第60天：请求体包含 delete 动作与任务 ID。
      }); // 第60天：fetch 调用结束。
      const payload = await readApiData<JobStoreSnapshot>(res); // 第60天：解析删除结果。
      setQueueJobs(payload.jobs ?? []); // 第60天：立即更新任务列表。
      setQueueMetrics(payload.metrics ?? null); // 第60天：立即更新队列指标。
      setWorkerPool(payload.workerPool ?? null); // 第60天：立即更新 WorkerPool 快照。
      setQueueRuntimeSnapshot(payload); // 第60天：立即更新 Queue Explorer 快照。
    } catch (err) {
      setErrorText(err instanceof ApiClientError ? err.message : "删除任务失败"); // 第60天：展示错误。
    } finally {
      setQueueLoading(false); // 第60天：结束队列请求。
    } // 第60天：结束 finally。
  }; // 第60天：结束 handleDeleteQueueJob。

  /** 第60天：从 Lock Explorer 强制释放指定 Redis 分布式锁。 */
  const handleForceUnlock = async (lockKey: string) => {
    setQueueLoading(true); // 第60天：开始锁操作请求。
    setErrorText(""); // 第60天：清空旧错误。
    try {
      const res = await fetch("/api/queue", {
        method: "PATCH", // 第60天：发送强制解锁动作。
        headers: { "Content-Type": "application/json" }, // 第60天：JSON 请求头。
        body: JSON.stringify({ action: "forceUnlock", lockKey }), // 第60天：请求体包含 forceUnlock 动作与锁 Key。
      }); // 第60天：fetch 调用结束。
      const payload = await readApiData<JobStoreSnapshot>(res); // 第60天：解析强制解锁结果。
      setQueueJobs(payload.jobs ?? []); // 第60天：立即更新任务列表。
      setQueueMetrics(payload.metrics ?? null); // 第60天：立即更新队列指标。
      setWorkerPool(payload.workerPool ?? null); // 第60天：立即更新 WorkerPool 快照。
      setQueueRuntimeSnapshot(payload); // 第60天：立即更新包含 Lock Explorer 的运行时快照。
    } catch (err) {
      setErrorText(err instanceof ApiClientError ? err.message : "强制解锁失败"); // 第60天：展示强制解锁错误。
    } finally {
      setQueueLoading(false); // 第60天：结束锁操作请求。
    } // 第60天：结束 finally。
  }; // 第60天：结束 handleForceUnlock。
  ...
  return (
    <main className="h-screen overflow-hidden bg-[#f4f6f8] font-sans text-zinc-950 dark:bg-zinc-950 dark:text-zinc-50">
      <div className="mx-auto flex h-full max-w-[1680px] flex-col px-3 py-3 sm:px-4">
          <Header
            provider={provider}
          ...
          <div className="flex min-h-0 flex-1 flex-col gap-3 pt-3 lg:flex-row">
            {/* 对话主栏 */}
            <section className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-zinc-200/80 bg-white shadow-sm shadow-zinc-900/5 dark:border-zinc-800 dark:bg-zinc-900/80 dark:shadow-black/30">
              <ChatToolbar
                useWorkflow={useWorkflow}
