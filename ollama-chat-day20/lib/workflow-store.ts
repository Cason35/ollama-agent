/**
 * 第20天：WorkflowStore 存储接口与工厂（Runtime 不直接依赖 localStorage）。
 * 每行带中文行尾注释。
 */

import type { WorkflowState } from "@/lib/workflow-types"; // 持久化快照类型
import { BackendWorkflowStore } from "@/lib/backend-workflow-store"; // 后端内存 Map 实现
...
export function createWorkflowStore(mode: WorkflowStorageMode): WorkflowStore {
  if (mode === "backend") {
    return new BackendWorkflowStore(); // 走 /api/workflows mock
  } // backend 分支结束
