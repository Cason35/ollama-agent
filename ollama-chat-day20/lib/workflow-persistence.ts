/**
 * 第19–20天：Workflow 持久化辅助（build/summary/恢复）；读写经 WorkflowStore，不直接 localStorage。
 * 每行带中文行尾注释。
 */
...
/** 第20天：经 Store 保存完整 WorkflowState。 */
export async function saveWorkflowState(
  ...
  await store.save(state); // 委托 Store
...
export async function persistWorkflowFromApi(
  ...
  const prev = await store.get(args.workflow.id); // 读取旧记录保留 createdAt
  ...
  await saveWorkflowState(store, state); // 写入 Store
  return state; // 返回
} // persistWorkflowFromApi 结束

/** 第20天：加载所有可恢复的 paused 状态（任务 5）。 */
export async function loadResumablePausedStates(store: WorkflowStore): Promise<WorkflowState[]> {
