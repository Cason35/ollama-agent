    ...
    <section className="shrink-0 border-b border-cyan-200/70 px-4 py-3 dark:border-cyan-900/40"> {/* 第46天：定义回归看板外层容器。 */}
      ...
      {snapshot && activeTab === "overview" ? ( /* 第46天：判断是否展示回归概览标签页。 */
        <div className="mt-3 space-y-2"> {/* 第46天：定义回归概览内容容器。 */}
          ...
          <div className="rounded-lg border border-teal-200 bg-teal-50/70 p-2.5 text-[10px] text-teal-900 dark:border-teal-900/50 dark:bg-teal-950/25 dark:text-teal-100"><p className="text-[11px] font-semibold">Prompt Regression Link（提示词回归关联）</p><p className="mt-1 font-mono">{snapshot.promptRegressionLink.baselinePromptId} → {snapshot.promptRegressionLink.candidatePromptId} · {snapshot.promptRegressionLink.result}</p><p className="mt-1">Score Δ（分数变化）: {snapshot.promptRegressionLink.scoreDelta >= 0 ? "+" : ""}{snapshot.promptRegressionLink.scoreDelta} · Cost Δ（成本变化）: {snapshot.promptRegressionLink.costDeltaPercent >= 0 ? "+" : ""}{snapshot.promptRegressionLink.costDeltaPercent}%</p></div> {/* 第52天：展示回归评估与 Prompt Version（提示词版本）的显式关联。 */}
