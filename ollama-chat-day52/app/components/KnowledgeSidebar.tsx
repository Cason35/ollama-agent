...
export function KnowledgeSidebar({
  ...
  const [activePanel, setActivePanel] = useState<"runtime" | "prompt" | "model" | "memory" | "cache" | "usage" | "knowledge" | "records">("prompt"); /* 第52天：默认打开提示词面板，突出 Prompt Registry 与版本回滚能力。 */
      ...
      <div className="z-10 shrink-0 border-b border-zinc-200/80 bg-white px-3 py-3 dark:border-zinc-800 dark:bg-zinc-900">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-0.5 text-[11px] text-zinc-500 dark:text-zinc-400">按主题查看提示词版本、模型健康、运行、用量、知识与历史记录。</p> {/* 第52天：更新控制台标签页说明，突出提示词版本管理。 */}
          </div>
          <span className="rounded-full bg-emerald-500/15 px-2.5 py-1 text-[10px] font-semibold text-emerald-800 ring-1 ring-emerald-500/20 dark:text-emerald-200">
            Day 52 {/* 第52天：更新控制台日期徽标。 */}
          </span>
        </div>
        <div className="mt-3 grid grid-cols-8 gap-1" role="tablist" aria-label="Day 52 侧边栏面板"> {/* 第52天：扩展为包含提示词版本管理的八个标签页。 */}
