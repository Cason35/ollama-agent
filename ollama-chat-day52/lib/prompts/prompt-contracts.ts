...
export function buildSamplePromptVariables(prompt: PromptTemplate): Record<string, string> { /* 第52天增强：定义提示词预览样例变量构造函数。 */
  ...
  const samples: Record<string, string> = { task: "分析 Day 52 Prompt Registry 的完整提示词治理能力。", tools: "retrieval, ragAnswer, summary", memory: "长期记忆：Day 51 已具备 fallback、timeout 和 circuit breaker。", workspace: "共享工作空间：上一个 step 总结了模型稳定性风险和后续优化方向。", agentId: prompt.componentId, output: "上游输出：已经完成资料检索、风险整理和下一步建议。", threshold: "80", agents: "research, planner, writer, critic" }; /* 第52天增强：定义所有内置变量的样例值。 */
