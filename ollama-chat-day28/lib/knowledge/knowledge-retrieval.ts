...
/** 第28天：按给定 queries 执行 Multi-Query Retrieval，供 RetrievalPipeline 复用。 */
export async function retrieveWithQueries(
  ...
  const minScore = typeof options.minScore === "number" ? options.minScore : DEFAULT_MIN_SCORE; // 读取最低分阈值
  const mode = normalizeRetrievalMode(options.mode); // 规整检索模式
  const normalizedQueries = rewrittenQueries.length > 0 ? rewrittenQueries : [q]; // 防御性兜底原始 query
  const recalledRows: ScoredChunkRow[] = []; // 汇总所有 query 的第一阶段候选
  for (const rewrittenQuery of normalizedQueries) {
  ...
  return {
    ...
    rewrite: {
      ...
      rewrittenQueries: normalizedQueries, // 实际检索 query
      rewriteCount: normalizedQueries.length, // query 数量
    },
  }; // 返回结果与调试信息
}

/** 第27天兼容入口：规则 Query Rewrite + Multi-Query Retrieval 主流程。 */
