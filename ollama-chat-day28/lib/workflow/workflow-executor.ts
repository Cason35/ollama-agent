...
export async function executeWorkflow(
  ...
  async function runOneStepWithRetries(step: WorkflowStep, chainSnapshot: string): Promise<void> {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      try {
        const toolInput = buildWorkflowToolInput({
          step,
          memory,
          rt,
          chainSnapshot,
        ...
        const stepOutputs = buildSuccessStepOutputsRecord(workflow.steps); // 第23天：供 critic 等读取
        const out = await workflowToolRegistry.execute(step.action, toolInput, {
          workflowId: workflow.id, // 工作流 id 注入 context
          stepOutputs, // 已成功步骤输出
          depth: 0, // 顶层执行深度为 0
        });
        ...
        if (err instanceof ToolValidationError) {
          console.warn("[ToolRegistry] validation failed", {
            tool: err.toolName,
            stepId: step.id,
            message: err.message,
          });
