...
import { getDefaultMaxQueries, retrieveWithQueries } from "@/lib/knowledge/knowledge-retrieval"; // 第62天：引入可控 queries 的多查询检索入口和配置中心默认查询数。
...
export async function runRetrievalPipeline(input: RetrievalPipelineInput): Promise<RetrievalPipelineResult> {
  ...
  const maxQueries = Math.max(1, Math.min(8, Math.floor(input.options?.maxQueries ?? getDefaultMaxQueries()))); // 第62天：从配置中心限制 query 数。
