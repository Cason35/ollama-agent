/** 第61天：RAG Runtime V8 的知识库、向量库与 Object Storage 共享类型。 */

import type { StorageObjectMetadata } from "@/lib/storage/storage-types"; // 第61天：引入对象存储元数据，用于让数据库只保存文件引用信息。
...
export type KnowledgeDocument = {
  ...
  title: string; // 文档标题
  content: string; // 原始全文
  storage?: StorageObjectMetadata; // 第61天：保存原始文档在 Object Storage（对象存储）中的引用元数据。
...
export type KnowledgeDocumentSummary = {
  ...
  title: string; // 标题
  version: number; // 版本
  storage?: StorageObjectMetadata; // 第61天：展示知识文档对应的对象存储元数据。
