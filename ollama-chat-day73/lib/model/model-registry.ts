...
import { modelRegistryItemId, syncModelToUnifiedRegistry } from "@/lib/registry/registry-adapters"; /* 第66天：引入模型统一注册同步适配器和稳定标识生成器。 */
import type { UnifiedRegistry } from "@/lib/registry/unified-registry"; /* 第66天：引入可选统一注册中心类型以保持模型注册表兼容。 */
...
export class ModelRegistry { /* 第50天：定义 ModelRegistry（模型注册表），负责登记与查询逻辑模型。 */
  ...
  register(model: ModelProfile): ModelProfile { /* 第50天：定义注册单个模型档案的方法。 */
    this.models.set(model.id, { ...model, capabilities: [...model.capabilities], roles: [...model.roles], fallbackModelIds: model.fallbackModelIds ? [...model.fallbackModelIds] : undefined }); /* 第56天：写入模型副本并复制能力、角色与备用链数组，避免外部引用被意外修改。 */
    syncModelToUnifiedRegistry(model, this.unifiedRegistry); /* 第66天：同步模型提供方、上下文、成本、延迟和健康元数据。 */
    return this.get(model.id) as ModelProfile; /* 第50天：返回已登记的模型副本。 */
  } /* 第50天：结束注册模型方法。 */

  unregister(id: string): void { /* 第66天：新增兼容注销方法同步删除模型档案与统一注册项。 */
    this.models.delete(id); /* 第66天：从旧模型注册表删除目标模型档案。 */
    this.unifiedRegistry?.unregister(modelRegistryItemId(id)); /* 第66天：从统一注册中心删除对应模型能力。 */
  } /* 第66天：结束模型兼容注销方法。 */
  ...
  get(id: string): ModelProfile | undefined { /* 第50天：定义按 id 读取模型档案的方法。 */
    ...
    return model ? { ...model, capabilities: [...model.capabilities], roles: [...model.roles], fallbackModelIds: model.fallbackModelIds ? [...model.fallbackModelIds] : undefined } : undefined; /* 第56天：命中时返回含角色与备用链副本的模型档案，未命中时返回 undefined。 */
  } /* 第50天：结束读取模型方法。 */

  list(): ModelProfile[] { /* 第50天：定义列出全部模型档案的方法。 */
    return Array.from(this.models.values()).map((model) => ({ ...model, capabilities: [...model.capabilities], roles: [...model.roles], fallbackModelIds: model.fallbackModelIds ? [...model.fallbackModelIds] : undefined })); /* 第56天：返回按注册顺序排列且角色与备用链已复制的模型副本数组。 */
  } /* 第50天：结束列出模型方法。 */
  ...
  clear(): void { /* 第50天：定义清空注册表的方法，便于测试隔离。 */
    this.list().forEach((model) => this.unifiedRegistry?.unregister(modelRegistryItemId(model.id))); /* 第66天：清空旧注册表前同步注销全部统一模型注册项。 */
