...
export type RuntimeContextV2 = { // 第64天：定义统一运行时上下文第二版，供所有运行模块共享。
  ...
  identityContext?: import("@/lib/governance/types").IdentityContext; // 第73天：保存当前用户、租户和成员关系等统一身份上下文。
  securityContext?: import("@/lib/governance/types").SecurityContext; // 第73天：保存当前请求已经解析出的角色与权限安全上下文。
  sessionId: string; // 第64天：串联同一用户的多轮会话。
  workflowId?: string; // 第64天：关联当前工作流。
  workflowContext?: import("@/lib/workflow/durable-workflow-types").WorkflowContextV1; // 第70天：保存工作流定义、执行实例、版本与最近检查点信息。
  ...
  evaluationContext?: import("@/lib/evaluation/evaluation-platform-types").EvaluationContextV2; // 第71天：保存评估运行、数据集、评估器版本和多维评分链路信息。
  observabilityContext?: import("@/lib/observability/types").ObservabilityContext; // 第72天：保存链路标识、指标开关、采样比例和采样原因等可观测上下文。
...
export class RuntimeContextBuilder { // 第64天：实现统一运行时上下文构建器。
  build(request: RuntimeContextRequest = {}): RuntimeContextV2 { // 第64天：统一补齐请求、追踪、会话和各模块上下文。
    const traceId = request.traceId?.trim() || `trace_${randomUUID()}`; // 第72天：提前生成唯一链路标识保证根上下文与可观测上下文完全一致。
    return { // 第64天：返回一份可沿全链路共享的上下文对象。
      ...
      identityContext: request.identityContext ? { ...request.identityContext, roles: [...request.identityContext.roles], permissions: [...request.identityContext.permissions] } : undefined, // 第73天：复制用户、租户、角色和权限身份上下文避免共享可变数组。
      securityContext: request.securityContext ? { ...request.securityContext, roles: [...request.securityContext.roles], permissions: [...request.securityContext.permissions] } : undefined, // 第73天：复制安全上下文供网关之后的全部运行模块安全读取。
      sessionId: request.sessionId?.trim() || `session_${randomUUID()}`, // 第64天：优先复用会话标识，否则生成新会话。
      workflowId: request.workflowId, // 第64天：透传可选工作流标识。
      workflowContext: request.workflowContext ? { ...request.workflowContext } : undefined, // 第70天：复制持久化工作流上下文并避免共享可变对象。
      ...
      traceId, // 第72天：复用提前生成的链路标识串联全部运行模块。
      ...
      observabilityContext: request.observabilityContext ?? { traceId, metricsEnabled: true, samplingRate: 1, sampled: true, samplingReason: "full" }, // 第72天：初始化统一可观测上下文并复用根上下文链路标识。
