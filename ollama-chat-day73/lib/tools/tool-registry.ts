...
import { syncToolToUnifiedRegistry, toolRegistryItemId } from "@/lib/registry/registry-adapters"; // 第66天：引入工具统一注册同步适配器和稳定标识生成器。
import type { UnifiedRegistry } from "@/lib/registry/unified-registry"; // 第66天：引入可选统一注册中心类型以保持原有工具调用兼容。
...
export class ToolRegistry {
  ...
  constructor(private readonly unifiedRegistry?: UnifiedRegistry) {} // 第66天：允许可选注入统一注册中心并保留历史无参构造方式。

  register(tool: Tool): void {
    console.log("[ToolRegistry] register", tool.name); // 第22天调试日志
    this.tools.set(tool.name, tool); // 写入 Map
    syncToolToUnifiedRegistry(tool, this.unifiedRegistry); // 第66天：同步工具描述、Schema、能力、超时和重试元数据。
  ...
  unregister(name: string): void { // 第66天：新增兼容注销方法同步移除旧工具注册表和统一注册项。
    this.tools.delete(name); // 第66天：从旧工具注册表删除目标工具实现。
    this.metrics.delete(name); // 第66天：同时删除目标工具的运行指标桶。
    this.unifiedRegistry?.unregister(toolRegistryItemId(name)); // 第66天：从统一注册中心删除对应工具能力。
  } // 第66天：结束工具兼容注销方法。
