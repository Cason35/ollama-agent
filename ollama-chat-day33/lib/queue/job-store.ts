...
const STORE_FILE = path.join(STORE_DIR, "jobs-v3.json"); // 第33天：使用 V3 持久化文件，避免污染 day32 数据。
...
export function calculateQueueMetrics(jobs: Job[]): QueueMetrics { // 定义队列指标计算函数。
  ...
  return { // 返回指标对象。
    ...
    highPriorityJobs: jobs.filter((job) => job.priority >= 10).length, // 第33天：统计高优先级任务。
    normalPriorityJobs: jobs.filter((job) => job.priority > 1 && job.priority < 10).length, // 第33天：统计普通优先级任务。
    lowPriorityJobs: jobs.filter((job) => job.priority <= 1).length, // 第33天：统计低优先级任务。
    scheduledJobs: jobs.filter((job) => job.status === "queued" && typeof job.scheduledAt === "number" && job.scheduledAt > now).length, // 第33天：统计未来定时任务。
  }; // 结束指标对象。
