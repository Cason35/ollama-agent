...
import type { CreateJobInput, Job, JobType, QueueMetrics } from "@/lib/queue/queue-types"; // 第33天：Queue Runtime V3 任务、创建输入与指标类型
...
export default function HomePage() {
  ...
  const [queueJobs, setQueueJobs] = useState<Job[]>([]); // 第33天：Queue Dashboard 任务列表
  const [queueMetrics, setQueueMetrics] = useState<QueueMetrics | null>(null); // 第33天：Queue Metrics V3 指标
  const [queueLoading, setQueueLoading] = useState(false); // 第33天：队列 API 加载状态
  ...
  /** 第33天：加载队列快照（Priority + Schedule + Retry + DLQ + Queue Metrics V3）。 */
  const refreshQueuePanel = async () => {
    try {
      const res = await fetch("/api/queue"); // GET 队列快照
      const payload = await readApiData<{ jobs: Job[]; metrics: QueueMetrics }>(res); // 解析队列响应
  ...
  /** 第33天：创建后台 Job，并携带 priority / scheduledAt 交给 Worker 异步调度。 */
  const handleCreateQueueJob = async (input: CreateJobInput) => {
    ...
    try {
      const payloadByType: Record<JobType, Record<string, unknown>> = {
        ...
        reminder: { text: "第33天 Reminder Job 已按 scheduledAt 自动触发" }, // reminder 定时提醒任务载荷
      }; // 任务载荷映射结束
      ...
      const payload = await readApiData<{ jobs: Job[]; metrics: QueueMetrics }>(res); // 解析创建结果
  ...
  /** 第33天：从 Dead Letter Queue 手动重新入队。 */
  const handleRequeueQueueJob = async (jobId: string) => {
    ...
    try {
      ...
      const payload = await readApiData<{ jobs: Job[]; metrics: QueueMetrics }>(res); // 解析恢复结果
      setQueueJobs(payload.jobs ?? []); // 立即更新任务列表
      setQueueMetrics(payload.metrics ?? null); // 立即更新指标
    } catch (err) {
