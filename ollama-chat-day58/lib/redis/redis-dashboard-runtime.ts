...
const DEMO_KEYS = ["day58:demo:cache", "day58:demo:session", "day58:demo:ratelimit"]; /* 第58天：定义 Redis Explorer 演示 Key，全部在 Day58 命名空间内。 */
export async function seedRedisDemoKeys(): Promise<void> { /* 第58天：定义写入 Redis 演示 Key 的函数。 */
  await redisClient.set(DEMO_KEYS[0], JSON.stringify({ purpose: "semantic-cache", day: 58 }), 3600); /* 第58天：写入模拟 Semantic Cache（语义缓存）Key 并设置一小时 TTL。 */
  await redisClient.set(DEMO_KEYS[1], JSON.stringify({ sessionId: "day58-session", status: "active" }), 1800); /* 第58天：写入模拟 Session（会话）Key 并设置半小时 TTL。 */
  await redisClient.set(DEMO_KEYS[2], JSON.stringify({ remaining: 42, window: "1m" }), 60); /* 第58天：写入模拟 RateLimit（限流）Key 并设置一分钟 TTL。 */
} /* 第58天：结束 Redis 演示 Key 写入函数。 */
