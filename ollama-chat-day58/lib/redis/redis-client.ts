...
export class RedisClient { /* 第58天：定义统一 RedisClient（Redis 客户端封装），业务代码不直接调用 ioredis。 */
  ...
  private readonly keyPrefix: string; /* 第58天：保存统一 Key Prefix（键前缀）。 */
  ...
  constructor(options: RedisClientOptions = {}) { /* 第58天：构造 RedisClient 并只保存配置，不在模块加载时立即连接 Redis。 */
    ...
    this.keyPrefix = options.keyPrefix ?? process.env.REDIS_KEY_PREFIX ?? "ollama:day58:"; /* 第58天：设置 Day58 默认命名空间，保护共享 Redis 实例。 */
