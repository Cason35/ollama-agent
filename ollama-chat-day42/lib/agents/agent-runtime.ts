import type { AgentCallEdge, AgentCollaborationSnapshot, AgentContext, AgentDAGMetrics, AgentPlan, AgentPlanStep, AgentPlanValidation, AgentResult, AgentTask, AgentTimelineEvent, Workspace, WorkspaceEntryType } from "@/lib/agents/agent-types"; /* 引入第42天 DAG 与共享工作空间运行时需要的类型。 */
...
import { createWorkspace, MemoryWorkspaceStore, type WorkspaceStore } from "@/lib/agents/workspace-store"; /* 第42天：引入共享工作空间创建函数和默认内存存储。 */
...
export class AgentRuntime { /* 定义第40天基于 Supervisor 的智能体运行时。 */
  ...
  constructor(private readonly registry: AgentRegistry, private readonly workspaceStore: WorkspaceStore = new MemoryWorkspaceStore()) {} /* 第42天：注入智能体注册表和共享工作空间存储。 */
  ...
  async executeAgent(agentId: string, task: AgentTask, context?: AgentContext, rt?: ModelRuntime): Promise<AgentResult> { /* 定义执行单个智能体任务的方法。 */
    ...
    const prompt = await this.buildAgentUserPrompt(assignedTask, context); /* 第42天：构造包含任务、前置结果和共享工作空间的用户提示词。 */
    ...
    await this.writeWorkspaceEntry(agentId, assignedTask.id, output, context); /* 第42天：把当前 Agent 的输出沉淀到共享工作空间。 */
  ...
  async executeAgentPlan(plan: AgentPlan, context?: AgentContext, rt?: ModelRuntime): Promise<AgentCollaborationSnapshot> { /* 定义按 AgentPlan DAG 并行执行智能体的方法。 */
    ...
    const workspace = context?.workspace ?? createWorkspace(safePlan.goal); /* 第42天：为本次多 Agent 协作创建共享工作空间。 */
    await this.workspaceStore.create(workspace); /* 第42天：把工作空间写入存储，供后续 Agent 读写。 */
    const workspaceContext: AgentContext = { memory: context?.memory, workflow: context?.workflow, tools: context?.tools, workspace }; /* 第42天：把共享工作空间注入 AgentContext。 */
    this.pushTimeline("supervisor", "day42-workspace-plan-task", validation.ok ? "supervisor planned workspace DAG" : "supervisor fallback planned workspace DAG"); /* 第42天：记录 Supervisor 完成带工作空间 DAG 规划事件。 */
    ...
    while (pendingSteps.size > 0) { /* 循环寻找当前批次可运行节点，直到所有节点完成。 */
      ...
      this.pushTimeline("supervisor", `day42-workspace-batch-${orderedResults.length + 1}`, `parallel workspace batch: ${runnableSteps.map((step) => step.id).join(", ")}`); /* 第42天：记录本轮带工作空间的并行批次。 */
      const batchResults = await Promise.all(runnableSteps.map((step) => this.executeDAGStep(step, resultsByStepId, workspaceContext, rt))); /* 第42天：使用带共享工作空间的上下文并行执行当前批次节点。 */
    ...
    const rootResult = finalResults[0] ?? orderedResults[0] ?? await this.executeAgent("writer", { id: "day42-empty-workspace-fallback", goal: safePlan.goal, assignedAgentId: "writer" }, workspaceContext, rt); /* 第42天：获取最终结果或空 DAG 兜底结果。 */
    ...
    await this.summarizeWorkspace(workspace.id); /* 第42天：在协作结束后把工作空间压缩为摘要条目。 */
    const resultStore = Object.fromEntries(resultsByStepId.entries()); /* 将 Map 结果存储转换为可序列化对象。 */
    const dagMetrics = this.calculateDAGMetrics(safePlan.steps); /* 第42天：计算 Agent Workspace DAG 指标。 */
    const finalWorkspace = await this.workspaceStore.get(workspace.id) ?? workspace; /* 第42天：读取包含全部 Agent 写入内容的最终工作空间快照。 */
    const workspaceMetrics = await this.workspaceStore.getMetrics(workspace.id); /* 第42天：计算共享工作空间指标。 */
    return { result, callGraph: this.callGraph, timeline: this.timeline, metrics: this.getRuntimeMetrics(), dagMetrics, resultStore, workspace: finalWorkspace, workspaceMetrics, plan: safePlan, validation: safeValidation }; /* 第42天：返回包含共享工作空间的 DAG 计划执行快照。 */
  ...
  private async executeDAGStep(step: AgentPlanStep, resultsByStepId: Map<string, AgentResult>, context?: AgentContext, rt?: ModelRuntime): Promise<{ step: AgentPlanStep; result: AgentResult }> { /* 定义执行单个 DAG 节点并返回步骤配对结果的方法。 */
    ...
    const workspace = context?.workspace ? await this.refreshWorkspace(context.workspace) : undefined; /* 第42天：执行节点前刷新共享工作空间，让下游 Agent 读取最新条目。 */
    const nextContext = context && workspace ? { ...context, workspace } : context; /* 第42天：把刷新后的工作空间重新注入当前节点上下文。 */
    const result = await this.delegateTask(fromAgentId, step.agentId, task, nextContext, rt); /* 第42天：通过带工作空间的委派入口执行当前 DAG 节点。 */
  ...
  private normalizePlanStep(step: Partial<AgentPlanStep>, index: number): AgentPlanStep { /* 定义计划步骤标准化方法。 */
    const fallbackId = `day42-step-${index + 1}-${typeof step.agentId === "string" ? step.agentId : "agent"}`; /* 第42天：生成缺省步骤 ID。 */
  ...
  private buildPlanSteps(goal: string, selectedAgents: string[]): AgentPlanStep[] { /* 定义根据智能体列表生成计划步骤的方法。 */
    ...
    return selectedAgents.map((agentId, index) => ({ id: `day42-step-${index + 1}-${agentId}`, agentId, task: this.buildTaskForAgent(agentId, goal), dependsOn: index === 0 ? [] : [`day42-step-${index}-${selectedAgents[index - 1]}`] })); /* 第42天：对简单任务保留可执行的线性 Workspace DAG。 */
  ...
  private createFallbackPlan(goal: string, errors: string[]): AgentPlan { /* 定义计划校验失败时的兜底计划。 */
    return { goal, selectedAgents: ["writer"], reason: `原计划校验失败，降级为 Writer Agent 兜底输出：${errors.join("；")}`, steps: [{ id: "day42-fallback-writer", agentId: "writer", task: `整理用户目标并输出可读结果：${goal}`, dependsOn: [] }] }; /* 第42天：返回 Writer 兜底计划。 */
  ...
  private findOrphanSteps(steps: AgentPlanStep[]): string[] { /* 第42天：定义孤儿节点检测方法。 */
  ...
  private calculateDAGMetrics(steps: AgentPlanStep[]): AgentDAGMetrics { /* 第42天：定义 Workspace DAG 指标计算方法。 */
  ...
  private async buildAgentUserPrompt(task: AgentTask, context?: AgentContext): Promise<string> { /* 第42天：定义单个 Agent 的用户提示词生成方法。 */
    ...
    const workspaceEntries = context?.workspace ? await this.workspaceStore.listEntries(context.workspace.id) : []; /* 第42天：读取共享工作空间已有条目。 */
    const workspaceText = workspaceEntries.length ? workspaceEntries.map((entry) => `[${entry.type}] ${entry.agentId}: ${entry.content}`).join("\n\n") : "无"; /* 第42天：格式化共享工作空间条目文本。 */
    return `当前任务：\n${task.goal}\n\n前置 Agent 输出：\n${previousText}\n\n共享工作空间：\n${workspaceText}\n\n请只完成当前 Agent 职责范围内的工作，并把结果写清楚。`; /* 第42天：返回包含 Workspace 的 Agent 执行提示词。 */
  ...
  private extractPreviousResults(taskContext?: unknown): AgentResult[] { /* 定义从任务上下文提取前置结果的方法。 */
    ...
    const parentResults = (taskContext as { parentResults?: unknown }).parentResults; /* 第42天：读取 DAG parentResults 字段并配合 Workspace 使用。 */
  ...
  private describeContext(context?: AgentContext, taskContext?: unknown): string { /* 定义上下文说明生成方法。 */
    ...
    const workspaceReady = context?.workspace ? `已接收共享工作空间 ${context.workspace.id}` : "暂无共享工作空间"; /* 第42天：生成共享工作空间上下文状态。 */
    return `${memoryReady}，${workflowReady}，${toolsReady}，${taskReady}，${workspaceReady}`; /* 第42天：返回合并后的上下文说明。 */
  } /* 结束 describeContext 方法。 */

  private async refreshWorkspace(workspace: Workspace): Promise<Workspace> { /* 第42天：定义刷新共享工作空间快照的方法。 */
    return await this.workspaceStore.get(workspace.id) ?? workspace; /* 第42天：优先返回存储中的最新工作空间。 */
  } /* 第42天：结束 refreshWorkspace 方法。 */

  private async writeWorkspaceEntry(agentId: string, taskId: string, output: string, context?: AgentContext): Promise<void> { /* 第42天：定义 Agent 写入共享工作空间的方法。 */
    if (!context?.workspace) return; /* 第42天：没有工作空间时保持兼容旧调用。 */
    await this.workspaceStore.addEntry(context.workspace.id, { id: `${taskId}-${agentId}-${Date.now()}`, type: this.getWorkspaceEntryType(agentId, taskId), agentId, content: output, tags: [agentId, taskId], createdAt: Date.now() }); /* 第42天：把 Agent 输出保存为工作空间条目。 */
  } /* 第42天：结束 writeWorkspaceEntry 方法。 */

  private getWorkspaceEntryType(agentId: string, taskId: string): WorkspaceEntryType { /* 第42天：定义 Agent 到工作空间条目类型的映射。 */
    if (agentId === "research") return "finding"; /* 第42天：Research Agent 输出记录为研究发现。 */
    if (agentId === "planner") return "draft"; /* 第42天：Planner Agent 输出记录为草稿。 */
    if (agentId === "critic") return taskId.includes("question") ? "question" : "decision"; /* 第42天：Critic Agent 输出记录为决策或问题。 */
    if (agentId === "writer") return "final"; /* 第42天：Writer Agent 输出记录为最终结果。 */
    return "note"; /* 第42天：其他 Agent 输出记录为普通笔记。 */
  } /* 第42天：结束 getWorkspaceEntryType 方法。 */

  private async summarizeWorkspace(workspaceId: string): Promise<void> { /* 第42天：定义共享工作空间摘要器。 */
    const entries = await this.workspaceStore.listEntries(workspaceId); /* 第42天：读取当前工作空间全部条目。 */
    if (entries.length === 0) return; /* 第42天：没有条目时不生成摘要。 */
    const byType = entries.reduce<Record<string, number>>((acc, entry) => ({ ...acc, [entry.type]: (acc[entry.type] ?? 0) + 1 }), {}); /* 第42天：统计摘要需要的条目类型分布。 */
    const summary = `Workspace Summary（工作空间摘要）：共 ${entries.length} 条记录；类型分布 ${Object.entries(byType).map(([type, count]) => `${type}:${count}`).join("，")}；最终协作目标已沉淀到共享工作空间。`; /* 第42天：生成短摘要文本，避免后续 prompt 过长。 */
    await this.workspaceStore.addEntry(workspaceId, { id: `workspace-summary-${Date.now()}`, type: "note", agentId: "workspace-summarizer", content: summary, tags: ["workspace", "summary"], createdAt: Date.now() }); /* 第42天：把摘要作为 note 条目写回工作空间。 */
  } /* 第42天：结束 summarizeWorkspace 方法。 */
