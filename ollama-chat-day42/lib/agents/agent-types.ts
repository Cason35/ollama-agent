...
export type AgentContext = { /* 定义第40天智能体执行上下文。 */
  ...
  workspace?: Workspace; /* 第42天：注入当前共享工作空间，让每个 Agent 都能读取团队协作现场。 */
}; /* 结束 AgentContext 类型定义。 */

export type WorkspaceEntryType = "note" | "finding" | "draft" | "decision" | "question" | "final"; /* 第42天：定义工作空间条目的六种业务类型。 */

export type WorkspaceEntry = { /* 第42天：定义单个 Agent 写入共享工作空间的结构。 */
  id: string; /* 第42天：保存工作空间条目的唯一标识。 */
  type: WorkspaceEntryType; /* 第42天：保存条目类型，用于前端分组和过滤。 */
  agentId: string; /* 第42天：保存写入该条目的 Agent ID。 */
  content: string; /* 第42天：保存 Agent 写入共享工作空间的正文内容。 */
  tags?: string[]; /* 第42天：保存可选标签，用于按主题筛选协作记录。 */
  createdAt: number; /* 第42天：保存条目创建时间戳。 */
}; /* 第42天：结束 WorkspaceEntry 类型定义。 */

export type Workspace = { /* 第42天：定义一次多 Agent 协作共享工作空间。 */
  id: string; /* 第42天：保存工作空间唯一标识。 */
  goal: string; /* 第42天：保存本次协作的用户目标。 */
  entries: WorkspaceEntry[]; /* 第42天：保存所有 Agent 写入的协作条目。 */
  createdAt: number; /* 第42天：保存工作空间创建时间戳。 */
  updatedAt: number; /* 第42天：保存工作空间最近更新时间戳。 */
}; /* 第42天：结束 Workspace 类型定义。 */

export type WorkspaceMetrics = { /* 第42天：定义工作空间可观测指标。 */
  entryCount: number; /* 第42天：记录当前工作空间条目总数。 */
  entriesByType: Record<string, number>; /* 第42天：记录不同条目类型的数量。 */
  entriesByAgent: Record<string, number>; /* 第42天：记录不同 Agent 写入的数量。 */
  lastUpdatedAt: number; /* 第42天：记录工作空间最后更新时间。 */
}; /* 第42天：结束 WorkspaceMetrics 类型定义。 */
...
export type AgentDAGMetrics = { /* 第42天：继续定义 Agent DAG 指标结构，用于观察图式计划的复杂度。 */
...
export type AgentCollaborationSnapshot = { /* 定义第40天 Supervisor 计划运行快照结构。 */
  ...
  dagMetrics: AgentDAGMetrics; /* 第42天：保存 DAG 总步数、并行步数、最大深度和关键路径指标。 */
  resultStore: Record<string, AgentResult>; /* 保存按 stepId 索引的 Agent Result Store，便于前端和测试查看依赖结果。 */
  workspace: Workspace; /* 第42天：保存本次协作产生的共享工作空间快照。 */
  workspaceMetrics: WorkspaceMetrics; /* 第42天：保存共享工作空间的条目数量和分布指标。 */
